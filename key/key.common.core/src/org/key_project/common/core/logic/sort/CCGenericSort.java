...
public class CCGenericSort extends AbstractSort {
     ...
     * means that every sort may be used
     */
    protected final ImmutableSet<Sort> oneOf;
    ...
    /**
     * @return true if "p_s" is a possible instantiation of this sort. This
     *         method does not check the instantiations of other generic sorts,
     *         i.e. the return value true is possible even if "p_s" is not a
     *         valid instantiation.
     *
    ...
    /**
     * @return true iff "p_s" is subsort of every non-generic supersort of this
     *         sort
     */
    protected boolean checkNonGenericSupersorts(Sort p_s) {
        ...
        while (it.hasNext()) {
            ss = it.next();
            if (ss instanceof CCGenericSort) {
                if (!((CCGenericSort) ss).checkNonGenericSupersorts(p_s))
                    return false;
            }
            else {
