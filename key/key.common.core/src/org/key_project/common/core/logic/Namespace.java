 ...
 * {@link Named}. It is possible to have nested namespaces in order to represent
 * different visibility scopes. An instance of Namespace can operate in normal
...
public class Namespace {
    ...
    /**
     * The hashmap that maps a name to a symbols of that name if it is defined
     * in this Namespace.
     */
    protected HashMap<Name, Named> symbols = null;

    /**
     * One defined symbol. Many Namespaces, e.g. those generated when a
     * quantified formula is parsed, define only one new symbol, and it would be
     * a waste of time and space to create a hashmap for that. So
     * {@link #symbols} is only initialized when there is more than one symbol
     * in this namespace. Otherwise, <code>localSym</code> contains that symbol.
     */
    protected Named localSym = null;

    /**
     * The number of symbols defined in this namespace. This is different from
     * <code>symbols.size()</code> because symbols might be null if there is
     * only one symbol in this Namespace.
     */
    ...
    /**
     * Construct an empty Namespace with protocol <code>protocol</code> and
     * without a parent namespace.
     */
    ...
    /**
     * Construct a Namespace that uses <code>parent</code> as a fallback for
     * finding symbols not defined in this one.
     */
    ...
    /**
     * Construct a Namespace that uses <code>parent</code> as a fallback for
     * finding symbols not defined in this one. Put an entry for
    ...
    /**
     * Adds the object <code>sym</code> to this Namespace. If an object with the
     * same name is already there, it is quietly replaced by <code>sym</code>.
     * Use addSafely() instead if possible. TODO:The problem of saving to
     * localSym, symbols, and symbolRefs is not solved yet.
     */
    ...
    /**
     * Adds the object <code>sym</code> to this namespace. Throws a runtime
     * exception if an object with the same name is already there.
     */
    ...
    /**
     * gets symbols added since last <code>startProtocol()</code>; resets the
     * protocol
     */
    ...
    /**
     * creates a new Namespace that has this as parent, and contains an entry
     * for <code>sym</code>.
     * 
    ...
    /**
     * looks if a registered object is declared in this namespace, if negative
     * it asks its parent
     * 
     * @param name
     *            a Name representing the name of the symbol to look for
     * @return Object with name "name" or null if no such an object has been
     *         found
     */
    ...
    /**
     * returns list of the elements (not the keys) in this namespace (not about
    ...
    /**
     * returns the fall-back Namespace of this Namespace, i.e. the one where
     * symbols are looked up that are not found in this one.
     */
