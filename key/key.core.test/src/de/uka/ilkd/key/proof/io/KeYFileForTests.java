...
/**
 * Used for TESTS ONLY as we allow there to declare program variables global in
 * rule files.
 */
public class KeYFileForTests extends KeYFile {
    ...
    public ImmutableSet<PositionedString> read() throws ProofInputException {
        ...
        try {
            cinp = new CountingBufferedReader(getNewStream(), monitor,
                    getNumberOfChars() / 100);
            final ParserConfig pc = new ParserConfig(initConfig.getServices(),
                    initConfig.namespaces());
            KeYParserF problemParser = new KeYParserF(ParserMode.PROBLEM,
                    new KeYLexerF(cinp, file.toString()), pc, pc,
