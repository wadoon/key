...
/**
 * Simple implementation of a non-destructive (unmodifiable) list. The list
 * implementation allows list sharing of sublists.
 *
 * The costs of the different operations are O(1) for prepending one element,
 * O(m) for prepending a list with m elements, O(n) for appending an element to
 * this list(size n) O(n) for appending a list with m elements to this list
 * (size n) head() has O(1) tail has O(1) size has O(1) ATTENTION appending and
 * prepending and element can be realized with O(1) costs (see Osaka) then
 * having tail and head with amortized O(1). This will be done later (if
 * necessary).
 */
...
public abstract class ImmutableSLList<T> implements ImmutableList<T> {
    ...
    public ImmutableList<T> take(int n) {
        if (n < 0 || n > size())
            throw new IndexOutOfBoundsException("Unable to take " + n
                    + " elements from list " + this);
	 ...
	 */
        ...
        /**
         * creates a new list with element as head and the momentan list as tail
         * (O(1))
         * 
         * @param e
        ...
        /**
         * hashcode for collections, implemented similar (just reverse)
         * algorithm as java.util.Collections use
         * 
        ...
        public int hashCode() {
            ...
            while (!crt.isEmpty()) {
                ...
                hashCode =
                        (element == null ? 0 : element.hashCode()) + 31
                                * hashCode;
                crt = crt.tail();
            }
            return hashCode;
    ...
    private static class SLListIterator<T> implements Iterator<T> {
        ...
        /**
         * throws an unsupported operation exception as removing elements is not
         * allowed on immutable lists
         */
        @Override
        public void remove() {
    ...
    private static class NIL<S> extends ImmutableSLList<S> {
        ...
        private class SLNilListIterator implements Iterator<S> {
            ...
            /**
             * throws an unsupported operation exception as removing elements is
             * not allowed on immutable lists
             */
            @Override
            public void remove() {
                throw new UnsupportedOperationException(
