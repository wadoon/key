...
public class IOUtilTest extends TestCase {
    ...
    public void testReadBytesFrom() throws IOException {
        File f = new File("../LICENSE.TXT");
        byte[] expected = IOUtil.readFrom(f).getBytes(StandardCharsets.UTF_8);
        try (FileInputStream s = new FileInputStream(f)) {
            byte[] actual = IOUtil.readBytesFrom(s, 20000);
