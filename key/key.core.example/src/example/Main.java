   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         try {
            ...
            final List<DisplayableSpecificationElement> proofContracts =
                    new LinkedList<DisplayableSpecificationElement>();
            ...
            // Perform proofs
            for (DisplayableSpecificationElement contract : proofContracts) {
               Proof proof = null;
               final boolean isContract = contract instanceof Contract;
               try {
                  // Create proof
                  proof = env.createProof(contract.createProofObl(env.getInitConfig()));
                  ...
                  System.out.println("Contract '" + contract.getDisplayName()
                          + (isContract ? "' of " + ((Contract)contract).getTarget() : "")
                          + " is " + (closed ? "verified" : "still open") + ".");
               }
               catch (ProofInputException e) {
                  System.out.println("Exception at '" + contract.getDisplayName()
                          + (isContract ? "' of " + ((Contract)contract).getTarget() : "") + ":");
                  e.printStackTrace();
               }
               finally {
