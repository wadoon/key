...
class TranslationOptions extends SettingsPanel implements SettingsProvider {
    ...
    private static final String infoUseExplicitTypeHierarchy =
        "If this option is selected, the transitive inheritance between classes is modeled by "
            ...
            + "If the option is selected, the following assumption is additionally added to the "
            + "assumptions above:\n\\forall x; (type_of_C(x)->type_of_A(x))\n";
    private static final String infoUseNullInstantiation =
        "At the moment this option has only effect on hierarchy assumptions regarding the null "
            ...
            + "If the option is selected, those assumptions are instantiated with a concrete null "
            + "object:\ntype_of_A(null)\n" + "type_of_B(null)";
    private static final String infoUseBuiltInUniqueness =
        "Some solvers support the uniqueness of functions by built-in mechanisms. If this option "
            + "is selected those mechanisms are used, otherwise some assumptions are added by "
            + "using normal FOL.\nNote: The uniqueness of functions is needed for translating "
            + "attributes and arrays.";
    private static final String infoUseUIMultiplication =
        "Some solvers support only simple multiplications. For example Yices supports only"
            + " multiplications of type a*b where a or b must be a number.\n"
            + "In order to support more complex multiplications, this option can be activated: If"
            + " the solver does not support a certain kind of multiplication, the occurence of this"
            + " multiplication is translated into the uninterpreted function mult. Furthermore some"
            ...
            + "1. If this option is not selected, an exception is thrown in the case that a"
            + " unsupported multiplication occurs.\n"
            + "2. The translation makes the uninterpreted function mult unique, so that there"
            + " cannot be any clashes with functions that are introduced by the user.";
    private static final String infoUseConstantsForIntegers =
        "Some solvers support only a certain interval of integers (e.g. Simplify). If this option"
            + " is activated, numbers that are not supported by the used solver are translated into"
            + " uninterpreted constants. Furthermore an asumption is added that states that the"
            + " introduced constant is greater than the maximum of the supported numbers.\n\n"
            ...
            + "Note: If this option is not selected, an exception is thrown in the case that a not"
            + " supported number occurs.\n";
