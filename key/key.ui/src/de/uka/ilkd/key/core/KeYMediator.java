...
public class KeYMediator {
    ...
    /**
     * current proof and node the user works with. All user interaction is
    ...
    /**
     * creates the KeYMediator with a reference to the application's main frame
     * and the current proof settings
     */
    ...
    /**
     * Returns a filter that is used for filtering taclets that should not be
     * showed while interactive proving.
     */
    ...
    private void finishSetBack(final Proof proof) {
        TaskFinishedInfo info = new DefaultTaskFinishedInfo(this, null, proof,
                0, 0, getNrGoalsClosedByAutoMode()) {
            @Override
            public String toString() {
                return "Proof has been pruned: "
                        + (proof.openGoals().size() == 1
        ...
        if (!proof.isDisposed()) {
            ...
            caches.getBetaCandidates().clear(); // TODO: Is this required since
                                                // the strategy is instantiated
                                                // everytime again?
            caches.getIfThenElseMalusCache().clear(); // TODO: Is this required
                                                      // since the strategy is
                                                      // instantiated everytime
                                                      // again?
        }
    }
    ...
    public void setMaxAutomaticSteps(int steps) {
        if (getSelectedProof() != null) {
            getSelectedProof().getSettings().getStrategySettings()
                    .setMaxSteps(steps);
        }
    ...
    public int getMaxAutomaticSteps() {
        if (getSelectedProof() != null) {
            return getSelectedProof().getSettings().getStrategySettings()
                    .getMaxSteps();
        }
        else {
    ...
    /**
     * fires that a GUI component that has asked for modal access has been
     * closed, so views can be enabled again
     */
    ...
    public void interrupt() {
        for (InterruptListener listener : listenerList
                .getListeners(InterruptListener.class)) {
            ...
            public void run() {
                ...
                if (b) {
                    ...
                    getUI().getProofControl().fireAutoModeStarted(
                            new ProofEvent(getSelectedProof())); // TODO: Is
                                                                 // this wrong
                                                                 // use of auto
                                                                 // mode really
                                                                 // required?
    ...
    public void startInterface(boolean fullStop) {
        ...
        Runnable interfaceSignaller = new Runnable() {
            public void run() {
                if (b) {
                    inAutoMode = false;
                    getUI().getProofControl().fireAutoModeStopped(
                            new ProofEvent(getSelectedProof())); // TODO: Is
                                                                 // this wrong
                                                                 // use of auto
                                                                 // mode really
    ...
    class KeYMediatorProofTreeListener extends ProofTreeAdapter {
        ...
        public void proofClosed(ProofTreeEvent e) {
            ...
            assert p.closed();
            KeYMediator.this
                    .notify(new ProofClosedNotificationEvent(e.getSource()));
        }
    ...
    private final class KeYMediatorProofListener
            implements RuleAppListener, AutoModeListener {
    ...
    /**
     * Disable certain actions until a proof is loaded. This is a workaround for
     * a broken proof macro menu in the GUI. Remove this method as soon as
    ...
    /**
     * besides the number of rule applications it is possible to define a
     * timeout after which rule application shall be terminated
     * 
    ...
    public boolean processDelayedCut(final Node invokedNode) {
        if (ensureProofLoaded()) {
            final String result = CheckedUserInput.showAsDialog("Cut Formula",
                    ...
                    new InspectorForDecisionPredicates(
                            getSelectedProof().getServices(), invokedNode,
            ...
            DelayedCutProcessor processor = new DelayedCutProcessor(
                    getSelectedProof(), invokedNode, formula,
                    DelayedCut.DECISION_PREDICATE_IN_ANTECEDENT);
            processor.add(new DelayedCutListener() {
                ...
                public void eventRebuildingTree(final int currentTacletNumber,
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        ...
                        public void run() {
                            ui.taskStarted(
                                    new DefaultTaskStartedInfo(TaskKind.Other,
                ...
                public void eventCutting() {
                    SwingUtilities.invokeLater(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            ui.taskStarted(new DefaultTaskStartedInfo(
                                    TaskKind.Other, "Cutting...", 0));
                        }
                    });
     ...
     * 
     * @return The {@link AutoSaver} to use or {@code null} if no
     *         {@link AutoSaver} should be used.
     */
