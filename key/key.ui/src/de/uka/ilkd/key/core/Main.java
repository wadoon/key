...
public final class Main {
	...
	// deprecated
	// public static final String JPATH_OF_RULE_FILE = JKEY_PREFIX +
	// "pathOfRuleFile";
	 ...
	 * replaced in Eclipse. For this reason the {@link Desktop} should never be
	 * used directly.
	 */
	...
	/**
	 * Save all contracts in selected location to automate the creation of
	 * multiple ".key"-files
	 */
	 ...
	 * {@link #examplesDir} is defined (not {@code null}). It is set in the
	 * Eclipse integration to {@code false}, because it is required to define
	 * the path to a different one without showing the chooser.
	 * </p>
	 * <p>
	...
	private static CommandLine createCommandLine() {
		...
		cl.addOption(TIMEOUT, "<timeout>", "timeout for each automatic proof of a problem in ms (default: "
		...
		cl.addText("\n", true);
		cl.addOption(JMAX_RULES, "<number>", "maximum number of rule application to perform (default: "
				+ LemmataAutoModeOptions.DEFAULT_MAXRULES + ")");
		cl.addOption(JPATH_OF_RESULT, "<path>", "store proofs to this folder");
		cl.addOption(JTIMEOUT, "<timeout>",
				"the timeout for proof of a taclet in ms (default: " + LemmataAutoModeOptions.DEFAULT_TIMEOUT + ")");
	...
	public static void evaluateOptions(CommandLine cl) {
		...
		if (cl.isSet(RIFL)) {
			...
			List<File> newargs = preProcessingRIFL(cl.getFileArguments());
			cl.getFileArguments().clear();
			cl.getFileArguments().addAll(newargs);
		...
		if (cl.isSet(TACLET_DIR)) {
			System.setProperty(RuleSourceFactory.STD_TACLET_DIR_PROP_KEY, cl.getString(TACLET_DIR, ""));
		}
	 ...
	 * <p>
	 * {@link ConsoleUserInterfaceControl} will be used if {@link Main#uiMode}
	 * is {@link UiMode#AUTO} and {@link WindowUserInterfaceControl} otherwise.
	 *
	...
	private static AbstractMediatorUserInterfaceControl createUserInterface(List<File> fileArguments) {
		...
		if (uiMode == UiMode.AUTO) {
			// terminate immediately when an uncaught exception occurs (e.g.,
			// OutOfMemoryError), see bug #1216
			Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
				@Override
				public void uncaughtException(Thread t, Throwable e) {
					if (verbosity > Verbosity.SILENT) {
						System.out.println(
								"Auto mode was terminated by an exception:" + e.getClass().toString().substring(5));
	...
	public static void ensureExamplesAvailable() {
		File examplesDir = getExamplesDir() == null ? ExampleChooser.lookForExamples() : new File(getExamplesDir());
	...
	private static void evaluateLemmataOptions(CommandLine options) {
		...
		try {
			...
			opt = new LemmataAutoModeOptions(options, KeYConstants.INTERNAL_VERSION, PathConfig.getKeyConfigDir());
	 ...
	 * the list of arguments. Otherwise, value of
	 * System.getProperty("user.home") is used to determine working directory.
	 *
	...
	/**
	 * Perform necessary actions before loading any problem files. Currently
	 * only performs RIFL to JML transformation.
	 */
	private static List<File> preProcessingRIFL(final List<File> filesOnStartup) {
		List<File> result = new ArrayList<File>();
		// RIFL to JML transformation
		if (riflFileName == null) {
			System.err.println("[RIFL] No rifl file is given. Please specify a XML file with: `--rifl <file>`");
			System.exit(-131615);
		}
		...
		if (filesOnStartup.size() > 1) {
			System.out.println("[RIFL] Warning: Multiple Java files are specified, we just process the first one.");			
		}

		// only use one input file
		File javaFile = filesOnStartup.get(0).getAbsoluteFile();

		try {
			RIFLTransformer transformer = new RIFLTransformer();
			transformer.setVerbosity(verbosity > Verbosity.SILENT);
			transformer.doTransform(riflFileName, javaFile);

			if (verbosity > Verbosity.SILENT) {
				System.out.println("[RIFL] Writing transformed Java files to " + javaFile + " ...");
			}
	 ...
	 * integration (KeY4Eclipse) to use the examples extract from the plug-in
	 * into the workspace.
	 *
	...
	/**
	 * Returns the {@link KeYDesktop} to use. Never use {@link Desktop} directly
	 * because the {@link KeYDesktop} is different in Eclipse.
	 *
	 ...
	 *
	 * @param keyDesktop
	 *            The new {@link KeYDesktop} to use.
	 */
