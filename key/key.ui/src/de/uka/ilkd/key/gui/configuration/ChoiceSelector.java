...
public class ChoiceSelector extends JDialog {
    ...
    /**
     * creates a new ChoiceSelector, using the <code>ChoiceSettings</code> from
     * <code>settings</code>
     */
    ...
    protected void layoutChoiceSelector() {
        ...
        String[] cats = category2DefaultChoice.keySet()
                .toArray(new String[category2DefaultChoice.size()]);
        Arrays.sort(cats);
        {
            ...
            JScrollPane catListScroll = new JScrollPane(
        ...
        }
        {
            ...
            choiceList.setSelectedValue(category2DefaultChoice.get(cats[0]),
                true);
            choiceList.addListSelectionListener(new ListSelectionListener() {
                ...
                public void valueChanged(ListSelectionEvent e) {
                    ...
                    if (selectedValue instanceof ChoiceEntry) {
                        setDefaultChoice(
                            ((ChoiceEntry) selectedValue).getChoice());
                    } else {
        ...
        }
        {
            ...
            cancel.registerKeyboardAction(escapeListener, "ESC",
    ...
    /**
     * is called if the selection of left list has changed, and causes the right
     * one to display the possible choices for the category chosen on the left
     * side
     */
    private void setChoiceList() {
        String selection = catList.getSelectedValue();
        ChoiceEntry[] choices = createChoiceEntries(
            category2Choices.get(selection));
        choiceList.setListData(choices);
        ChoiceEntry selectedChoice = findChoice(choices,
            category2DefaultChoice.get(selection));
     ...
     * </p>
     * <p>
     * This method should be public and static because it is independent from
     * the {@link JDialog} and it is also used by the eclipse projects.
     * </p>
     *
     * @param category
    ...
    public static String getExplanation(String category) {
        synchronized (ChoiceSelector.class) {
            if (explanationMap == null) {
                ...
                InputStream is = ChoiceSelector.class
                        .getResourceAsStream(EXPLANATIONS_RESOURCE);
                try {
                    if (is == null) {
                        throw new FileNotFoundException(
                            EXPLANATIONS_RESOURCE + " not found");
                    }
                    explanationMap.loadFromXML(is);
                } catch (InvalidPropertiesFormatException e) {
                    System.err.println(
                        "Cannot load help message in rule view (malformed XML).");
                    e.printStackTrace();
                } catch (IOException e) {
                    System.err
                            .println("Cannot load help messages in rule view.");
                    e.printStackTrace();
                }
            }
        }
     ...
     * @param choice
     *            The choice to check.
     * @return {@code true} proof will be incomplete, {@code false} proof will
     *         be complete as long as all other choices are complete.
     */
    public static boolean isIncomplete(String choice) {
        return "runtimeExceptions:ban".equals(choice)
                || "Strings:off".equals(choice)
                || "lazySymbExec:on".equals(choice)
     ...
     * @param choice
     *            The choice to check.
     * @return The additional information or {@code null} if no information are
     *         available.
     */
    public static String getInformation(String choice) {
        ...
        } else if ("lazySymbExec:on".equals(choice)) {
            return "complete with post-processing step";
     ...
     * @return The found {@link ChoiceEntry} for the given choice or
     *         {@code null} otherwise.
     */
    public static ChoiceEntry findChoice(ChoiceEntry[] choices,
            final String choice) {
    ...
    public static class ChoiceEntry {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof ChoiceEntry) {
                ...
                return choice.equals(other.getChoice())
                        ...
                        && unsound == other.isUnsound() && ObjectUtil
                                .equals(information, other.getInformation());
            } else {
                return false;
            }
        }
        ...
        public String toString() {
            if (unsound && incomplete) {
                if (information != null) {
                    return choice + " (" + UNSOUND_TEXT + " and "
