...
public class TacletMenu extends JMenu {
    ...
    public static final int TOO_MUCH_TACLETS = 15;
...
//            addSeparator();
    ...
    private void createMenuItems(ImmutableList<TacletApp> taclets, MenuControl control) {
        List<TacletApp> tacletAppList = new ArrayList<>(taclets.size());
        for (TacletApp a : taclets) tacletAppList.add(a);
        createMenuItems(this, tacletAppList, control);
    }


    private void createMenuItems(JMenu target, List<TacletApp> taclets, MenuControl control) {
        for (int i = 0; i < taclets.size(); i += TOO_MUCH_TACLETS) {
            addMenuItems(target, taclets, i, Math.min(i + TOO_MUCH_TACLETS, taclets.size())
                    , control);

            if (taclets.size() - i > TOO_MUCH_TACLETS) // we have a next round
            {
                JMenu restMenu = new JMenu("more ...");
                target.add(restMenu);
                target = restMenu;
            }
        }
    ...
    private void addMenuItems(JMenu target, List<TacletApp> taclets, int start, int stop,
                              MenuControl control) {
        ...
        for (int i = start; i < stop; i++) {
            final TacletApp app = taclets.get(i);
