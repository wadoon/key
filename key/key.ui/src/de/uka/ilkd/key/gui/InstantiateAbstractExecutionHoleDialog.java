 ...
 */
public class InstantiateAbstractExecutionHoleDialog extends JDialog {
    ...
    private List<AbstractExecutionHoleInstantiation> results;

    private final List<ParserProblem> problems = new ArrayList<>();
    private final List<Runnable> problemChangeListeners = new ArrayList<>();

    private final JList<AbstractExecutionHole> holesSelectionBox;
    private final JTabbedPane instantiationTabbedPane = new JTabbedPane();
    private final List<AbstractExecutionHole> lastSelection = new ArrayList<>();
    private final Map<AbstractExecutionHole, JPanel> instantiationPanels = new HashMap<>();

    public InstantiateAbstractExecutionHoleDialog(MainWindow mainWindow, Proof proof) {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                for (final AbstractExecutionHole deselected : deselectedOnes) {
                    ...
                    final Optional<ParserProblem> maybeProblem = problems
                            .stream().filter(p -> p.abstractExecutionHole.equals(deselected))
                            .findFirst();
                    problems.remove(maybeProblem.orElse(null));
                    informParserProblemChangeListeners();
                }

                for (final AbstractExecutionHole selected : newSelection) {
    ...
    private Component setupProblemsView() {
        ...
        problemChangeListeners.add(() -> {
            ...
            for (ParserProblem problem : problems) {
                ...
                sb.append(problem.abstractExecutionHole.toString())
                        .append("\n========================");
    ...
    private void collectResults() {
        final List<AbstractExecutionHoleInstantiation> result = new ArrayList<>();
        incompleteInput = lastSelection.isEmpty();

        for (AbstractExecutionHole lh : lastSelection) {
            ...
            result.add(new AbstractExecutionHoleInstantiation(lh, pcInst, symbStInst));
        }
    ...
    public List<AbstractExecutionHoleInstantiation> getUserInput() {
        return results;
    }
    ...
    private Component createInstantiationPanelFor(final AbstractExecutionHole selected) {
    ...
    private ParserProblem getParserProblemObjectFor(AbstractExecutionHole lh) {
        Optional<ParserProblem> maybeProblem = problems.stream()
                .filter(p -> p.abstractExecutionHole.equals(lh)).findFirst();
    ...
    private AbstractExecutionHole[] retrieveLoopHoles() {
        if (proof == null) {
            /* For GUI design purposes */
            return mockupRetrieveLoopHoles();
        }

        return InstantiateAbstractExecutionHoleRule.retrieveLoopHoles(proof);
    }
    ...
    private AbstractExecutionHole[] mockupRetrieveLoopHoles() {
        final AbstractExecutionHole[] result = new AbstractExecutionHole[4];
        for (int i = 0; i < 3; i++) {
            result[i] = new AbstractExecutionHole(i + 1, "C_sk_" + i, "U_sk_" + i);
        }
        return result;
    }

    private class ParserProblem {
        private AbstractExecutionHole abstractExecutionHole;
        private Optional<String> pathCondProblem = Optional.empty();
        private Optional<String> symbStProblem = Optional.empty();

        public ParserProblem(AbstractExecutionHole abstractExecutionHole) {
    ...
    public static void main(String[] args) {
        new InstantiateAbstractExecutionHoleDialog(null, null).setVisible(true);
    }
}
