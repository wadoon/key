...
public class ArrayList {
    ...
    int indexOfRange(/*@ nullable @*/ SimpleObject o, int start, int end) {
        ...
        if (o == null) {
			return indexOfNull(start, end);
        } else {
			return indexOfObject(o, start, end);
        }
		
    }
	
	
	int indexOfNull(int start, int end) {
		/*@ loop_invariant
	...
	int indexOfObject(/*@ nullable @*/ SimpleObject o, int start, int end) {
		/*@ loop_invariant
		  @  (i >= start && start >= 0 && start < elementData.length && 
		  @   i <= elementData.length &&
