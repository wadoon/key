...
public class ArrayList {
    ...
    int indexOfRange(/*@ nullable @*/ SimpleObject o, int start, int end) {
		...
		/*@ loop_invariant
		  ...
		  @   (\forall int j; j >= start && j < i; !areObjectsEqual(o, elementData[j]))) ||
		  @  (start < 0 && i == start) ||
		...
		for (int i = start; i < end; i++) {
			if(areObjectsEqual(o, elementData[i])){
				return i;
			}
        }
        return -1;
	...
	/*@ public normal_behavior
	  @ requires (object0 == null);
	  @ assignable \nothing;
	  @ ensures \result == (object1 == null);
	  @
	  @ also
	  @
	  @ public normal_behavior
	  @ requires (object0 instanceof SimpleObject);
	  @ assignable \nothing;
	  @ ensures \result == object0.equals(object1);
	  @ 
	  @ also
	  @
	  @ public normal_behavior
	  @ requires (object0 != null && !(object0 instanceof SimpleObject));
	  @ assignable \nothing;
	  @ ensures \result == false;
	  @*/
	boolean /*@ pure @*/ areObjectsEqual(/*@ nullable @*/ SimpleObject object0, /*@ nullable @*/ SimpleObject object1) {
		...
		if(object1 != null && object0 instanceof SimpleObject){
