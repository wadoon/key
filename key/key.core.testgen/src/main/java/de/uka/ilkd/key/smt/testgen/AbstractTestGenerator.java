...
public abstract class AbstractTestGenerator {
    ...
    protected Proof createProof(UserInterfaceControl ui, Proof oldProof, String newName, Sequent newSequent)
        ...
        proof.getServices().getSpecificationRepository().registerProof(proof.getServices()
                .getSpecificationRepository().getProofOblInput(oldProof), proof);
     ...
     */
    protected void informAboutNoTestResults(SolverLauncher launcher, Collection<SMTSolver> problemSolvers,
                                            TestGenerationLog log, Proof originalProof) {
    }
    ...
    public Collection<SMTSolver> filterSolverResultsAndShowSolverStatistics(Collection<SMTSolver> problemSolvers,
        ...
        if (unknown > 0) {
            log.writeln(" Adjust the SMT solver settings (e.g. timeout) in Options->SMT Solvers " +
    ...
    public void stopSMTLauncher() {
        if (launcher != null) {
            launcher.stop(SMTSolver.ReasonOfInterruption.USER);
        }
    }
