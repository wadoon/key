 ...
 */
public class BaseTestGenerator {
     ...
     */
    public BaseTestGenerator(UserInterfaceControl ui, Proof originalProof) {
        this.ui = ui;
        this.originalProof = originalProof;
    }

    public void generateTestCases(@Nullable StopRequest stopRequest, @NotNull TestGenerationLog log) {
        if (stopRequest == null) {
            stopRequest = () -> false;
        }
        TestGenerationSettings settings = getTestGenerationSettings();

        if (checkSmtRequirements(log, settings)) return;

        if (settings.getApplySymbolicExecution()) {
            applySymbolicExecution(log);
        }
        ...
        if (!proofs.isEmpty()) {
            log.writeln(format("Extracted %d test data constraints.", proofs.size()));
        } else {
        ...
        try {
            for (final Proof proof : proofs) {
                Collection<SMTProblem> ret = blastProof(stopRequest, log, proof);
                if (stopRequest.shouldStop()) {
                    return;
                }
                problems.addAll(ret);
    ...
    private Collection<SMTProblem> blastProof(@NotNull StopRequest stopRequest, @NotNull TestGenerationLog log, Proof proof) {
        ...
        try {
            ...
            ret = SMTProblem.createSMTProblems(proof);
    ...
    protected void applySymbolicExecution(TestGenerationLog log) {
        try {
    ...
    protected boolean checkSmtRequirements(TestGenerationLog log, TestGenerationSettings settings) {
        ...
        if (!SolverType.Z3_CE_SOLVER.isSupportedVersion()) {
            log.writeln(format("Warning: z3 supported versions are: %s",
     ...
     */
    public List<Proof> createProofsForTesting(boolean removeDuplicatePathConditions, boolean removePostCondition) {
