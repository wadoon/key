...
/**
 * Implementations of this class are used generate test cases or a given
 * {@link Proof}.
 * <p>
 * <b>This class provides the full logic independent from the a user
 * interface.</b> Subclasses are used to realize the user interface specific
 * functionality.
 */
public abstract class AbstractTestGenerator {
    ...
    private boolean isNoninterferenceProof;
    ...
    public AbstractTestGenerator(UserInterfaceControl ui, Proof originalProof) {
        ...
        ContractPO po = originalProof.getServices().getSpecificationRepository().
    ...
    public void generateTestCases(final StopRequest stopRequest, final TestGenerationLog log) {
        ...
        if (!SolverType.Z3_CE_SOLVER.isSupportedVersion()) {
            log.writeln("Warning: z3 supported versions are: "
                    + Arrays.toString(SolverType.Z3_CE_SOLVER.getSupportedVersions()));
        }
        ...
        // ModelGenerator mg = new
        // ModelGenerator(proofs.get(0).root().sequent(), 3, getMediator());
        // mg.launch();
        return;
    }
    ...
    * @throws ProofInputException exception for proof input
    ...
    private Proof createProofForTesting_noDuplicate(Node node, List<Proof> otherProofs, boolean removePostCondition)
        ...
        Sequent newSequent = Sequent.createSequent(Semisequent.EMPTY_SEMISEQUENT,
    ...
    protected Proof createProof(UserInterfaceControl ui, Proof oldProof, String newName, Sequent newSequent)
            throws ProofInputException {
        ...
        proof.getServices().getSpecificationRepository()
                .registerProof(proof.getServices().getSpecificationRepository().
                        getProofOblInput(oldProof), proof);
    ...
    protected void handleLauncherStarted(Collection<SMTProblem> problems, Collection<SolverType> solverTypes,
            SolverLauncher launcher, TestGenerationLog log) {
        log.writeln("Test data generation: solving " + problems.size()
            + " SMT problems... \n please wait...");
    }

    protected void handleLauncherStopped(SolverLauncher launcher, Collection<SMTSolver> problemSolvers,
    ...
    protected void generateFiles(SolverLauncher launcher, Collection<SMTSolver> problemSolvers, TestGenerationLog log,
            Proof originalProof) throws Exception {
        final TestCaseGenerator tg;
        // information flow tests or functional tests
...
//        }
     ...
     * 
     * @return {@code true} solver is available, {@code false} solver is not
     *         available.
     */
