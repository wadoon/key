 ...
 * </p>
 */
public class TermBuilder implements CCTermBuilder<JavaBlock, Term> {
    ...
    public LocationVariable excVar(String name,
        ...
        return new LocationVariable(new ProgramElementName(name),
                services.getProgramServices().getJavaInfo()
                        .getTypeByClassName(JAVA_LANG_THROWABLE));
    }
    ...
    public Term sum(ImmutableList<QuantifiableVariable> qvs, Term range, Term t) {
        final Function sum =
                (Function) services.getNamespaces().functions().lookup("sum");
        final Iterator<QuantifiableVariable> it = qvs.iterator();
        Term res =
                func(sum, new Term[] { convertToBoolean(range), t },
                        new ImmutableArray<QuantifiableVariable>(it.next()));
        while (it.hasNext()) {
            res =
                    func(sum, new Term[] { TRUE(), res },
                            new ImmutableArray<QuantifiableVariable>(it.next()));
        }
        return res;
    }
    ...
    public Term prod(
        ...
        final Iterator<QuantifiableVariable> it = qvs.iterator();
        Term res =
                func(prod, new Term[] { convertToBoolean(range), t },
                        new ImmutableArray<QuantifiableVariable>(it.next()));
        while (it.hasNext()) {
            res =
                    func(prod, new Term[] { TRUE(), res },
                            new ImmutableArray<QuantifiableVariable>(it.next()));
        }
        return res;
    }
    ...
    public Term min(
        ...
        final Iterator<QuantifiableVariable> it = qvs.iterator();
        Term res =
                func(min, new Term[] { convertToBoolean(range), t },
                        new ImmutableArray<QuantifiableVariable>(it.next()));
        while (it.hasNext()) {
            res =
                    func(min, new Term[] { TRUE(), res },
                            new ImmutableArray<QuantifiableVariable>(it.next()));
        }
        return res;
    }
    ...
    public Term max(
        ...
        final Iterator<QuantifiableVariable> it = qvs.iterator();
        Term res =
                func(max, new Term[] { convertToBoolean(range), t },
                        new ImmutableArray<QuantifiableVariable>(it.next()));
        while (it.hasNext()) {
            res =
                    func(max, new Term[] { TRUE(), res },
                            new ImmutableArray<QuantifiableVariable>(it.next()));
        }
        return res;
    }
    ...
    public Term convertToFormula(Term a) {
        ...
        if (a.sort() == Sort.FORMULA) {
            ...
            throw new TermCreationException("Term " + a
                    + " cannot be converted"
                    + " into a formula.");
        }
    }
        ...
        if (lhss.length != values.length) {
            throw new TermCreationException(
                    "Tried to create parallel update with "
                            + lhss.length + " locs and " + values.length
                            + " values");
        }
    ...
    public Term applyUpdatePairsSequential(
            ImmutableList<UpdateLabelPair<Term>> updates, Term target) {
        if (updates.isEmpty()) {
            return target;
        }
        else {
    ...
    public Term zTerm(String numberString) {
        ...
        for (int i = j, sz = numberString.length(); i < sz; i++) {
            ...
            numberLiteralTerm =
                    func(intLDT.getNumberLiteralFor(digit), numberLiteralTerm);
        }
        if (negate) {
            numberLiteralTerm =
                    func(intLDT.getNegativeNumberSign(), numberLiteralTerm);
        }
    ...
    public Term staticInv(Term[] h, KeYJavaType t) {
        return func(
                services.getProgramServices().getJavaInfo().getStaticInv(t), h);
    }
    ...
    public Term staticInv(KeYJavaType t) {
        ...
        return func(
                services.getProgramServices().getJavaInfo().getStaticInv(t), hs);
    }
    ...
    public Term select(Sort asSort, Term h, Term o, LocationVariable field) {
        final Function f =
                theories.getHeapLDT().getFieldSymbolForPV(field, services);
    ...
    public Term dot(Sort asSort, Term o, Function f) {
        final Sort fieldSort = theories.getHeapLDT().getFieldSort();
    ...
    public Term reachableValue(Term h,
        ...
        final Sort s =
                t.sort() instanceof ProgramSVSort ? kjt.getSort() : t.sort();
        final IntegerLDT intLDT = theories.getIntegerLDT();
        final LocSetLDT setLDT = theories.getLocSetLDT();
        if (s.extendsTrans(services.getProgramServices().getJavaInfo()
                .objectSort())) {
            ...
            return createdInHeap(t, h);
                ...
                && kjt.getProgramType() != PrimitiveType.JAVA_BIGINT) {
        ...
        return all(
                ...
                or(elementOf(objVarTerm,
                        ...
                        and(not(equals(objVarTerm, NULL())),
                                not(createdAtPre)),
                        equals(select(
                                permissionHeap ? theories.getPermissionLDT()
                                        .targetSort() : SortImpl.ANY,
                                heapTerm,
                                objVarTerm,
                                fieldVarTerm),
                                select(permissionHeap ? theories
    ...
    public Term frameStrictlyEmpty(Term heapTerm, Map<Term, Term> normalToAtPre) {
        ...
        return all(
                quantVars,
                equals(select(permissionHeap ? theories.getPermissionLDT()
                        .targetSort() : SortImpl.ANY,
                        heapTerm,
                        objVarTerm,
                        fieldVarTerm),
                        select(permissionHeap ? theories.getPermissionLDT()
                                .targetSort() : SortImpl.ANY,
                                or.replace(heapTerm),
                                objVarTerm,
                                fieldVarTerm)));
    ...
    public ImmutableSet<Term> unionToSet(Term s) {
        ...
        ImmutableList<Term> workingList =
                ImmutableSLList.<Term> nil().prepend(s);
     ...
     * Removes leading updates from the passed term.
     */
    public static Pair<ImmutableList<Term>, Term> goBelowUpdates2(
            Term term) {
        ImmutableList<Term> updates = ImmutableSLList.<Term> nil();
        while (term.op() instanceof UpdateApplication) {
    ...
    public Term eqAtLocs(TermServices services,
        ...
        && locset2.equals(empty()))
                ...
                : func((Function) services.getNamespaces().functions()
                        .lookup(new Name(
                                "__EQUALS__LOCS__")), // TODO: define string
                                                      // constant elsewhere
    ...
    public Term eqAtLocsPost(TermServices services,
        ...
        && locset2.equals(empty()))
                ...
                : func((Function) services.getNamespaces().functions()
                        .lookup(new Name(
                                "__EQUALS__LOCS__POST__")), // TODO: define
                                                            // string constant
                                                            // elsewhere
