...
public abstract class VariableNamer implements InstantiationProposer {
    ...
    /**
     * returns the subterm containing a java block, or null (helper for
     * getProgramFromPIO())
     */
    ...
    protected int getMaxCounterInProgram(String basename,
            ProgramElement program, PosInProgram posOfDeclaration) {
     ...
     */
    protected boolean isUniqueInGlobals(String name,
            Iterable<ProgramElementName> globals) {
        Iterator<ProgramElementName> it = globals.iterator();
        while (it.hasNext()) {
     ...
     */
    protected Iterable<ProgramElementName> wrapGlobals(
            ImmutableList<? extends Named> globals) {
        List<ProgramElementName> result = new ArrayList<ProgramElementName>(
            globals.size());
    ...
    /**
     * proposes a unique name for the instantiation of a schema variable (like
     * getProposal(), but somewhat less nicely)
     *
     * @param basename
     ...
     *            the PosInProgram where the name will be declared (or null to
     *            just be pessimistic about the scope)
    ...
    protected ProgramElementName getNameProposalForSchemaVariable(
            String basename, SchemaVariable sv, PosInOccurrence posOfFind,
            PosInProgram posOfDeclaration,
        ...
        if (svSort == ProgramSVSort.VARIABLE) {
            ...
            int cnt = getMaxCounterInProgram(basename,
                getProgramFromPIO(posOfFind), posOfDeclaration) + 1;
     ...
     */
    @Override
    public String getProposal(TacletApp app, SchemaVariable var,
            Services services, Node undoAnchor,
            ImmutableList<String> previousProposals) {
        // determine posOfDeclaration from TacletApp
        ContextInstantiationEntry cie = app.instantiations()
        ...
        if (nv != null) {
            ...
            if (type != null) {
                ...
                if (inst instanceof Expression) {
                    final ExecutionContext ec = app.instantiations()
                            .getExecutionContext();
                    if (ec != null) {
    ...
    /**
     * tells whether a name for instantiating a schema variable is unique within
     * its scope
     *
     * @param name
    ...
    public boolean isUniqueNameForSchemaVariable(String name, SchemaVariable sv,
        ...
        Sort svSort = sv.sort();
        if (svSort == ProgramSVSort.VARIABLE) {
            result = isUniqueInProgram(name, getProgramFromPIO(posOfFind),
                posOfDeclaration);
        }
    ...
    public static ProgramElementName parseName(String name,
        ...
        if (sepPos > 0) {
            ...
            result = new TempIndProgramElementName(basename, index,
            ...
            if (sepPos > 0) {
                try {
                    String basename = name.substring(0, sepPos);
                    int index = Integer.parseInt(name.substring(sepPos + 1));
                    result = new PermIndProgramElementName(basename, index,
                        creationInfo, comments);
                } catch (NumberFormatException e) {
                    result = new ProgramElementName(name, creationInfo,
                        comments);
                }
            } else {
    ...
    public String getSuggestiveNameProposalForProgramVariable(SchemaVariable sv,
            TacletApp app, Services services,
            ImmutableList<String> previousProposals) {
        if (suggestive_off) {
        ...
        String proposal;
        try {
            Iterator<TacletGoalTemplate> templs = app.taclet().goalTemplates()
                    .iterator();
            ...
            while (templs.hasNext()) {
                ...
                ContextStatementBlock c = (ContextStatementBlock) t.javaBlock()
                        .program();
                if (c.getStatementAt(0) instanceof LocalVariableDeclaration) {
                    VariableSpecification v = ((LocalVariableDeclaration) c
                            .getStatementAt(0)).getVariables().get(0);
                    ...
                    if (v.hasInitializer()) {
                        ...
                        CopyAssignment p2 = (CopyAssignment) c
                                .getStatementAt(1);
                        Expression lhs = p2.getExpressionAt(0);
                        if (lhs.equals(sv)) {
                            SchemaVariable rhs = (SchemaVariable) p2
                                    .getExpressionAt(1);
    ...
    private ProgramElement instantiateExpression(ProgramElement e,
        ...
        ProgramReplaceVisitor trans = new ProgramReplaceVisitor(e, services,
            svInst);
        trans.start();
        return trans.result();
    }
    ...
    private static class TempIndProgramElementName
        ...
        TempIndProgramElementName(String basename, int index,
                NameCreationInfo creationInfo) {
    ...
    private static class PermIndProgramElementName
        ...
        PermIndProgramElementName(String basename, int index,
                NameCreationInfo creationInfo) {
            super(basename + (index == 0 ? "" : SEPARATOR + "" + index),
                basename, index, creationInfo);
        }

        PermIndProgramElementName(String basename, int index,
                NameCreationInfo creationInfo, Comment[] comments) {
            super(basename + (index == 0 ? "" : SEPARATOR + "" + index),
                basename, index, creationInfo, comments);
        }
    }
