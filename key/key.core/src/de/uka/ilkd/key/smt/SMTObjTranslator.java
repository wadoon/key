...
public class SMTObjTranslator implements SMTTranslator {
	...
	private void initSorts() {
		...
		objectSort = services.getProgramServices().getJavaInfo().getJavaLangObject().getSort();		
	...
	private void generateWellFormedAssertions() throws IllegalFormulaException {
		...
		for (Sort s : thierarchy.getArraySortList()) {
			...
			if (single.equals("int") || single.equals("char")
				...
				typeReq = SMTTerm.call(getIsFunction(sorts.get(OBJECT_SORT)),
				        selectArr);
				Sort singleSort = services.getProgramServices().getJavaInfo().getKeYJavaType(single)
				        .getSort();
	...
	private void addSingleSort(Set<Sort> sorts, Sort s) {
	    ...
	    if(name.endsWith("[]")){
	    	String single = name.substring(0, name.length() - 2);
	    	Sort singleSort = services.getProgramServices().getJavaInfo().getKeYJavaType(single).getSort();
