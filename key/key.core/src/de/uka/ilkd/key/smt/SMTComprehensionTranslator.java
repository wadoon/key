...
public class SMTComprehensionTranslator implements SMTTranslator {
	 ...
	 */
	private static boolean guardOverflow = false;
	...
	/*----------------------------------*/
	public SMTComprehensionTranslator(SMTSettings settings, Services services, KeYJavaType typeOfClassUnderTest) {
		super();
...
//	    }
	...
	/*
	 * added by Huy: to resolve comprehensions: bsum, max, min
	 * */
	
	public static class TermUtil{
			public static String BSUM = "bsum";
		   public static String MAX = "max";
		   public static String MIN = "min";
		   
		  /*return true if Term t is ancestor of one of term listed in listSubTerms*/
		   public static boolean isDescendantQVar(Term t, QuantifiableVariable qvar, Services services){
		      
	         if(isDescendant(services.getTermBuilder().var(qvar), t))
	            return true;
		  ...
		  /**
		    ...
		    * */
		   public static boolean inArrayQVar(QuantifiableVariable qv, ImmutableArray<QuantifiableVariable> lt){
		      if(!lt.isEmpty()){
		         for(QuantifiableVariable te: lt){
		            if(qv.name().toString().equals(te.name().toString()))
		               return true;
		         }
		      }
		      return false;
		    ...
		    * otherwise return false
		    * */
		   public static boolean inListQVar(QuantifiableVariable qv, List<QuantifiableVariable> lt){
		      if(!lt.isEmpty()){
		         for(QuantifiableVariable te: lt){
		            if(qv.name().toString().equals(te.name().toString()))
		               return true;
		 ...
		 //added by Huy
		   public static boolean isMaxFunction(Term term){
		      return term.op().name().toString().equals(MAX);         
		   }
		   
		   //added by Huy
		   public static boolean isMinFunction(Term term){
		      return term.op().name().toString().equals(MIN);         
		   }
		 ...
		 * @param services TODO
		   ...
		   /*
		    * replace subst by dest in origTerm
		    * */
		   public static Term repalce(Term subst, Term dest, Term origTerm, Services services){
			   if(sameToString(subst, origTerm)){
				   return dest;
			   }else{
				   if(origTerm.subs().size()>0){
					   int size =origTerm.subs().size();
				       Term[] subs = new Term[size];
				       for(int i=0;i<size;i++){
				          subs[i] = repalce(subst, dest, origTerm.sub(i), services);
				       }
				        return services.getTermBuilder().tf().createTerm(origTerm.op(), subs, origTerm.boundVars(),origTerm.javaBlock());
				   }else
					   return origTerm;
			   }
		   }
	 ...
	 /*
	   ...
	   private SMTTerm buildAxioms4Comprehension(SMTComprehensionFunction comprehension, SMTSort boundSort, List<SMTSort> qvarSorts, String comprehensionName, SMTTerm lowerBound, SMTTerm upperBound){
	       ...
	       List<SMTTermVariable> xyp = new LinkedList<SMTTermVariable>();
	       xyp.add(x);
	       xyp.add(y);
	       xyp.addAll(params);
	       ...
	       xyminus1p.add(x);
	       xyminus1p.add(y.minus(new SMTTermNumber(1,size,sorts.get(BINT_SORT))));
	       ...
	       kplus1yp.add(k.plus(new SMTTermNumber(1,size,sorts.get(BINT_SORT))));
	       kplus1yp.add(y);
	       ...
	       xplus1yp.add(x.plus(new SMTTermNumber(1,size,sorts.get(BINT_SORT))));
	       xplus1yp.add(y);
	       ...
	       SMTTerm comprehensionAxiom = SMTTerm.forall(xyp, boundCondition.implies(SMTTerm.and(listAxioms)), null);
	   ...
	   private SMTTerm buildCalcAxiom(SMTFunction calc, SMTTerm calcTermExpr, SMTTermVariable idx, List<SMTTermVariable> qvarExprs, SMTTerm lowerBound, SMTTerm upperBound) {
	        ...
	        * */
	       List<SMTTermVariable> paramsOfCalc = new LinkedList<SMTTermVariable>();
	       ...
	       SMTTerm calcAxiom = SMTTerm.forall(paramsOfCalc, equalExpr, null);
	   ...
	   private SMTTerm createValue4EmptyRange(String comprehensionName){
		   ...
		   if(comprehensionName.equals(TermUtil.BSUM))
			   return new SMTTermNumber(0,size,sorts.get(BINT_SORT));
		   else if (comprehensionName.equals(TermUtil.MAX))
			   return new SMTTermNumber(Long.MIN_VALUE); //TODO: working with bit vector
		   else if (comprehensionName.equals(TermUtil.MIN))
	   ...
	   private Pair<Pair<Term,Term>, Pair<QuantifiableVariable,Term>> extractBoundNCaclInfo(Term term, String comprehensionName){
		   ...
		   if(comprehensionName.equals(TermUtil.BSUM)){
			   ...
			   ComprehensionFunctionResolver qfr = new ComprehensionFunctionResolver(term, services);
