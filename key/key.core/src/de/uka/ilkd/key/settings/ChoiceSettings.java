...
public class ChoiceSettings implements Settings, Cloneable {
    ...
    /**
     * maps categories to a set of Strings(representing the choices which are
    ...
    /**
     * returns a copy of the HashMap that maps categories to their default
     * choices.
     */
    @SuppressWarnings("unchecked")
    ...
    private ImmutableSet<Choice> choiceMap2choiceSet(
            HashMap<String, String> ccc) {
        ...
        for (final Map.Entry<String, String> entry : ccc.entrySet()) {
            choices = choices.prepend(
                new Choice(new Name(entry.getValue()), entry.getKey()));
        }
        return DefaultImmutableSet.fromImmutableList(choices);
    }

    /**
     * updates <code>category2Choices</code> if new entries are found in
     * <code>choiceNS</code> or if entries of <code>category2Choices</code> are
     * no longer present in <code>choiceNS</code>
     *
     * @param remove
        ...
        for (final String s : getDefaultChoices().keySet()) {
            if (category2Choices.containsKey(s)) {
                if (!category2Choices.get(s)
                    ...
                    category2Default.put(s,
                        category2Choices.get(s).iterator().next());
                    fireSettingsChanged();
                }
            } else {
    ...
    public void writeSettings(Object sender, Properties props) {
        ...
        for (final Map.Entry<String, String> entry : category2Default
                .entrySet()) {
