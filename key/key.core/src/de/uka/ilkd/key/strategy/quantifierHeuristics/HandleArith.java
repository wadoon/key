...
/**
 * This class is used to prove some simple arithmetic problem which are a==b,
 * a>=b, a<=b; Besides it can be used to prove that a>=b or a<=b by knowing that
 * c>=d or c<=d;
 * 
 */
public class HandleArith {
    ...
    public static Term provedByArith(Term problem, Services services) {
        ...
        final Term arithTerm = formatArithTerm(problem, tb, integerLDT,
                services.getCaches());
        if (arithTerm.equals(falseT)) {
            result = provedArithEqual(problem, tb, services);
            putInTermCache(provedByArithFstCache, provedByArithFstLock, problem, result);
            return result;
        }
        ...
        if (poly2.valueLeq(poly1)) {
            putInTermCache(provedByArithFstCache, provedByArithFstLock, problem, trueT);
            return trueT;
        }
        if (poly1.valueLess(poly2)) {
            putInTermCache(provedByArithFstCache, provedByArithFstLock, problem, falseT);
            return falseT;
        }
        putInTermCache(provedByArithFstCache, provedByArithFstLock, problem, problem);
    ...
    /**
     * @param problem
     * @return true if atom.sub(0) is euqual to atom.sub(1), false if not equal,
     *         else return atom
     */
    private static Term provedArithEqual(Term problem, TermBuilder tb,
            Services services) {
        final Term trueT = tb.tt();
        final Term falseT = tb.ff();
    ...
    final static ReentrantReadWriteLock formattedTermCacheLock = new ReentrantReadWriteLock();
    final static ReentrantReadWriteLock provedByArithFstLock = new ReentrantReadWriteLock();
    ...
    /**
     * Try to prove problem by know that axiom is true. The idea is that we know
     * a>=b(axiom),we want to prove c>=d(problem). It is enough to prove that
     * c+b>=a+d which means c>=d is true. or we prove that !(c>=d) which is
     * d>=c+1 is true. This means c>= d is false;
     * 
     * @param problem
     * @param axiom
    ...
    public static Term provedByArith(Term problem, Term axiom,
        ...
        if (cd.op() == Junctor.FALSE || ab.op() == Junctor.FALSE) {
            putInTermCache(provedByArithSndCache, provedByArithSndLock, key, problem);
            return problem;
        }
        ...
        if (res.op() == Junctor.TRUE) {
            putInTermCache(provedByArithSndCache, provedByArithSndLock, key, trueT);
            return trueT;
        }
        ...
        if (res.op() == Junctor.TRUE) {
            putInTermCache(provedByArithSndCache, provedByArithSndLock, key, falseT);
            return falseT;
        }
        ...
        if (op == geq) {
            if (opNot)
                pro = tb.geq(pro.sub(1),
                        tb.func(ig.getAdd(), pro.sub(0), ig.one()));
        } else {
            if (op == leq) {
                if (opNot)
                    pro = tb.geq(pro.sub(0),
                            tb.func(ig.getAdd(), pro.sub(1), ig.one()));
                else pro = tb.geq(pro.sub(1), pro.sub(0));
            } else pro = falseT;
        }
        
        putInTermCache(formattedTermCache, formattedTermCacheLock, problem, pro);
