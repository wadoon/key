...
///////////////////////////////
	...
	private RuleSetDispatchFeature setupCostComputationF() {
		...
		bindRuleSet(d, "concrete", add(longConst(-11000), ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 10.0)));
		...
		bindRuleSet(d, "gamma",
				add(not(isInstantiated("t")), ifZero(allowQuantifierSplitting(), longConst(0), longConst(50))));
		bindRuleSet(d, "gamma_destructive", inftyConst());

		bindRuleSet(d, "triggered", add(not(isTriggerVariableInstantiated()), longConst(500)));

		bindRuleSet(d, "comprehension_split", add(applyTF(FocusFormulaProjection.INSTANCE, ff.notContainsExecutable),
		...
		bindRuleSet(d, "split_if",
				add(sum(superFor, SuperTermGenerator.upwards(any(), getServices()), applyTF(superFor, not(ff.program))),
						longConst(50)));
		...
		bindRuleSet(d, "simplify_prog", ifZero(ThrownExceptionFeature.create(exceptionsWithPenalty, getServices()),
				longConst(500), ifZero(isBelow(add(ff.forF, not(ff.atom))), longConst(200), longConst(-100))));
		...
		case StrategyProperties.METHOD_CONTRACT:
			 ...
			 * expansion is disabled. The original cost was 200 and is now increased to 2000
			 * in order to repress method expansion stronger when method treatment by
			 * contracts is chosen.
			 */
		...
		case StrategyProperties.MPS_MERGE:
			/*
			 * For this case, we use a special feature, since deleting merge points should
			 * only be done after a merge rule application.
			 */
		...
		final String queryAxProp = strategyProperties.getProperty(StrategyProperties.QUERYAXIOM_OPTIONS_KEY);
		switch (queryAxProp) {
		...
		if (programsToRight) {
			bindRuleSet(d, "boxDiamondConv",
					SumFeature.createSum(new FindPrefixRestrictionFeature(
							...
							FindPrefixRestrictionFeature.PrefixChecker.ANTEC_POLARITY), longConst(-1000)));
		} else {
	...
	private void setupSelectSimplification(final RuleSetDispatchFeature d) {
		bindRuleSet(d, "pull_out_select",
				...
				// function symbol)
				add(applyTF("h", not(or(PrimitiveHeapTermFeature.create(heapLDT), AnonHeapTermFeature.INSTANCE))),
						ifZero(applyTF(FocusFormulaProjection.INSTANCE, ff.update), longConst(-4200), longConst(-1900)),
		...
		bindRuleSet(d, "simplify_select",
				...
				add(applyTF("sk", IsSelectSkolemConstantTermFeature.INSTANCE),
						applyTF(sub(FocusProjection.INSTANCE, 0), not(SimplifiedSelectTermFeature.create(heapLDT))),
						longConst(-5600)));
		...
		bindRuleSet(d, "hide_auxiliary_eq",
				...
				add(applyTF("auxiliarySK", IsSelectSkolemConstantTermFeature.INSTANCE),
						applyTF("result",
								rec(any(), add(SimplifiedSelectTermFeature.create(heapLDT), not(ff.ifThenElse)))),
						not(ContainsTermFeature.create(instOf("result"), instOf("auxiliarySK"))), longConst(-5400)));
		bindRuleSet(d, "hide_auxiliary_eq_const",
				// hide auxiliary equation after the skolem constatns have
				// been replaced by it's computed value
				add(applyTF("auxiliarySK", IsSelectSkolemConstantTermFeature.INSTANCE), longConst(-500)));
	}
	...
	private void setUpStringNormalisation(RuleSetDispatchFeature d) {
		...
		final TermFeature keepChar = or(op(seqLDT.getSeqSingleton()), or(op(charListLDT.getClIndexOfChar()),
				or(op(charListLDT.getClReplace()), op(charListLDT.getClLastIndexOfChar()))));

		bindRuleSet(d, "charLiteral_to_intLiteral", ifZero(isBelow(keepChar), inftyConst(), longConst(-100)));
		...
		final TermFeature seqLiteral = rec(anyLiteral,
				or(op(seqLDT.getSeqConcat()), or(op(seqLDT.getSeqSingleton()), or(anyLiteral, inftyTermConst()))));

		Feature belowModOpPenality = ifZero(isBelow(ff.modalOperator), longConst(500));

		bindRuleSet(d, "defOpsSeqEquality", add(NonDuplicateAppModPositionFeature.INSTANCE,
				ifZero(add(applyTF("left", seqLiteral), applyTF("right", seqLiteral)), longConst(1000), inftyConst()),
				belowModOpPenality));

		bindRuleSet(d, "defOpsConcat", add(NonDuplicateAppModPositionFeature.INSTANCE,
				ifZero(or(applyTF("leftStr", not(seqLiteral)), applyTF("rightStr", not(seqLiteral))), longConst(1000)
		...
		bindRuleSet(d, "stringsReduceSubstring", add(NonDuplicateAppModPositionFeature.INSTANCE, longConst(100)));
		...
		bindRuleSet(d, "stringsConcatNotBothLiterals",
				ifZero(MatchedIfFeature.INSTANCE,
						ifZero(add(applyTF(instOf("leftStr"), seqLiteral), applyTF(instOf("rightStr"), seqLiteral)),
								inftyConst()),
						inftyConst()));
		...
		bindRuleSet(d, "stringsContainsDefInline",
				SumFeature.createSum(new Feature[] { EqNonDuplicateAppFeature.INSTANCE, longConst(1000) }));
	}

	private void setupReplaceKnown(RuleSetDispatchFeature d) {
		final Feature commonF = add(ifZero(MatchedIfFeature.INSTANCE, DiffFindAndIfFeature.INSTANCE), longConst(-5000),
		...
		bindRuleSet(d, "replace_known_right", add(commonF, ifZero(DirectlyBelowSymbolFeature.create(Junctor.IMP, 1),
				longConst(100), ifZero(DirectlyBelowSymbolFeature.create(Equality.EQV), longConst(100)))));
	}

	private void setupUserTaclets(RuleSetDispatchFeature d) {
		for (int i = 1; i <= StrategyProperties.USER_TACLETS_NUM; ++i) {
			final String userTacletsProbs = strategyProperties.getProperty(StrategyProperties.userTacletsOptionsKey(i));
	...
	private void setupSystemInvariantSimp(RuleSetDispatchFeature d) {
		bindRuleSet(d, "system_invariant", ifZero(MatchedIfFeature.INSTANCE,
				add(applyTF("negLit", tf.negLiteral), applyTFNonStrict("nonNegLit", tf.nonNegLiteral))));
	}
	...
	private boolean autoInductionEnabled() { // chrisg
		// Negated!
		return !StrategyProperties.AUTO_INDUCTION_OFF
				.equals(strategyProperties.getProperty(StrategyProperties.AUTO_INDUCTION_OPTIONS_KEY));
	}

	private boolean autoInductionLemmaEnabled() { // chrisg
		final String prop = strategyProperties.getProperty(StrategyProperties.AUTO_INDUCTION_OPTIONS_KEY);
	...
	private Feature allowSplitting(ProjectionToTerm focus) {
		...
		if (StrategyProperties.SPLITTING_DELAYED
			...
			return or(applyTF(focus, ContainsExecutableCodeTermFeature.PROGRAMS), sequentContainsNoPrograms());
		}
		// else: SPLITTING_OFF
	...
	private void setupSplitting(RuleSetDispatchFeature d) {
		...
		bindRuleSet(d, "beta",
				SumFeature.createSum(noCutsAllowed, ifZero(PurePosDPathFeature.INSTANCE, longConst(-200)),
		...
		bindRuleSet(d, "split_cond", add(// do not split over formulas containing auxiliary variables
				applyTF(FocusProjection.INSTANCE, rec(any(), not(IsSelectSkolemConstantTermFeature.INSTANCE))),
				// prefer splits when condition has quantifiers (less
				// likely to be simplified away)
				applyTF(splitCondition, rec(ff.quantifiedFor, ifZero(ff.quantifiedFor, longTermConst(-10)))),
				...
				ifZero(applyTF(FocusProjection.INSTANCE, ContainsExecutableCodeTermFeature.PROGRAMS), longConst(-100),
						longConst(25))));
		ProjectionToTerm cutFormula = instOf("cutFormula");
		Feature countOccurrencesInSeq = ScaleFeature.createAffine(countOccurrences(cutFormula), -10, 10);
	...
	private void setupSplittingApproval(RuleSetDispatchFeature d) {
		...
		final Feature noBetterCut = sum(subFor, AllowedCutPositionsGenerator.INSTANCE, compareCutAllowed);
	...
	private void setupApplyEq(RuleSetDispatchFeature d, IntegerLDT numbers) {
		...
		bindRuleSet(d, "apply_equations", SumFeature.createSum(new Feature[] {
				...
				ifZero(MatchedIfFeature.INSTANCE, add(CheckApplyEqFeature.INSTANCE,
						let(equation, AssumptionProjection.create(0), add(not(applyTF(equation, ff.update)),
								// there might be updates in
								// front of the assumption
								// formula; in this case we wait
								// until the updates have
								// been applied
								let(left, sub(equation, 0), let(right, sub(equation, 1), ifZero(applyTF(left, tf.intF),
	...
	private void setupFormulaNormalisation(RuleSetDispatchFeature d, IntegerLDT numbers, LocSetLDT locSetLDT) {

		bindRuleSet(d, "negationNormalForm", add(not(NotBelowBinderFeature.INSTANCE), longConst(-500),
				ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 10.0)));

		bindRuleSet(d, "moveQuantToLeft", add(quantifiersMightSplit() ? longConst(0)
				: applyTF(FocusFormulaProjection.INSTANCE, ff.quantifiedPureLitConjDisj), longConst(-550)));

		bindRuleSet(d, "conjNormalForm",
				ifZero(add(or(FocusInAntecFeature.INSTANCE, NotBelowQuantifierFeature.INSTANCE),
						NotInScopeOfModalityFeature.INSTANCE),
						add(longConst(-150), ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 20)), inftyConst()));
		...
		bindRuleSet(d, "distrQuantifier", add(or(
				...
				SumFeature.createSum(OnlyInScopeOfQuantifiersFeature.INSTANCE,
						...
						ifZero(FocusInAntecFeature.INSTANCE, applyTF(FocusProjection.create(0), sub(ff.andF)),
								applyTF(FocusProjection.create(0), sub(ff.orF))))),
				longConst(-300)));

		bindRuleSet(d, "swapQuantifiers", add(applyTF(FocusProjection.create(0), add(ff.quantifiedClauseSet,
				EliminableQuantifierTF.INSTANCE, sub(not(EliminableQuantifierTF.INSTANCE)))), longConst(-300)));

		// category "conjunctive normal form"

		bindRuleSet(d, "cnf_orComm", SumFeature.createSum(new Feature[] { applyTF("commRight", ff.clause),
				applyTFNonStrict("commResidue", ff.clauseSet),
				or(applyTF("commLeft", ff.andF),
						add(applyTF("commLeft", ff.literal), literalsSmallerThan("commRight", "commLeft", numbers))),
				longConst(-100) }));

		bindRuleSet(d, "cnf_orAssoc", SumFeature.createSum(new Feature[] { applyTF("assoc0", ff.clause),
				applyTF("assoc1", ff.clause), applyTF("assoc2", ff.literal), longConst(-80) }));

		bindRuleSet(d, "cnf_andComm",
				SumFeature.createSum(new Feature[] { applyTF("commLeft", ff.clause), applyTF("commRight", ff.clauseSet),
						applyTFNonStrict("commResidue", ff.clauseSet),
						...
						ifZero(add(applyTF("commLeft", not(ff.literal)),
								applyTF("commRight", rec(ff.andF, not(ff.literal)))), longConst(100), longConst(-60)),
						clausesSmallerThan("commRight", "commLeft", numbers) }));

		bindRuleSet(d, "cnf_andAssoc", SumFeature.createSum(new Feature[] { applyTF("assoc0", ff.clauseSet),
		...
		bindRuleSet(d, "cnf_dist",
				SumFeature.createSum(new Feature[] { applyTF("distRight0", ff.clauseSet),
						...
						ifZero(applyTF("distLeft", ff.clause), longConst(-15), applyTF("distLeft", ff.clauseSet)),
						longConst(-35) }));

		final TermBuffer superFor = new TermBuffer();
		final Feature onlyBelowQuanAndOr = sum(superFor, SuperTermGenerator.upwards(any(), getServices()),
		...
		bindRuleSet(d, "cnf_expandIfThenElse",
				add(not(NotBelowQuantifierFeature.INSTANCE), onlyBelowQuanAndOr, belowUnskolemisableQuantifier));

		final Feature pullOutQuantifierAllowed = add(not(NotBelowQuantifierFeature.INSTANCE), onlyBelowQuanAndOr,
				applyTF(FocusProjection.create(0), sub(ff.quantifiedClauseSet, ff.quantifiedClauseSet)));

		bindRuleSet(d, "pullOutQuantifierUnifying", -20);

		bindRuleSet(d, "pullOutQuantifierAll",
				add(pullOutQuantifierAllowed, ifZero(FocusInAntecFeature.INSTANCE, longConst(-20), longConst(-40))));

		bindRuleSet(d, "pullOutQuantifierEx",
				add(pullOutQuantifierAllowed, ifZero(FocusInAntecFeature.INSTANCE, longConst(-40), longConst(-20))));
	}
	...
	private void setupQuantifierInstantiation(RuleSetDispatchFeature d) {
		if (quantifierInstantiatedEnabled()) {
			...
			final Feature branchPrediction = InstantiationCostScalerFeature.create(InstantiationCost.create(varInst),
					allowQuantifierSplitting());

			bindRuleSet(d, "gamma", SumFeature.createSum(new Feature[] { FocusInAntecFeature.INSTANCE,
					applyTF(FocusProjection.create(0),
							add(ff.quantifiedClauseSet,
									instQuantifiersWithQueries() ? longTermConst(0) : ff.notContainsExecutable)),
	...
	private void setupQuantifierInstantiationApproval(RuleSetDispatchFeature d) {
		if (quantifierInstantiatedEnabled()) {
			...
			bindRuleSet(d, "gamma", add(isInstantiated("t"),
					...
					InstantiationCostScalerFeature.create(InstantiationCost.create(instOf("t")), longConst(0))));

			final TermBuffer splitInst = new TermBuffer();
			bindRuleSet(d, "triggered", add(isTriggerVariableInstantiated(), not(sum(splitInst,
					TriggeredInstantiations.create(false), not(eq(instOfTriggerVariable(), splitInst))))));
		} else {
			bindRuleSet(d, "gamma", inftyConst());
			bindRuleSet(d, "triggered", inftyConst());
		}
	...
	private void setupPolySimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
		...
		bindRuleSet(d, "polySimp_mulAssoc", SumFeature.createSum(new Feature[] { applyTF("mulAssocMono0", tf.monomial),
				applyTF("mulAssocMono1", tf.monomial), applyTF("mulAssocAtom", tf.atom), longConst(-80) }));

		bindRuleSet(d, "polySimp_addOrder",
				SumFeature.createSum(
						new Feature[] { applyTF("commLeft", tf.monomial), applyTF("commRight", tf.polynomial),
		...
		bindRuleSet(d, "polySimp_dist",
				SumFeature.createSum(
						new Feature[] { applyTF("distSummand0", tf.polynomial), applyTF("distSummand1", tf.polynomial),
		...
		bindRuleSet(d, "polySimp_normalise",
				add(applyTF("invertRight", tf.zeroLiteral), applyTF("invertLeft", tf.negMonomial), longConst(-30)));
		...
		final TermFeature atLeastTwoLCEquation = opSub(Equality.EQUALS, opSub(tf.mul, tf.atom, tf.atLeastTwoLiteral),
				tf.intF);
	...
	private void setupPolySimpInstantiationWithoutRetry(RuleSetDispatchFeature d) {
		...
		bindRuleSet(d, "polySimp_newSym", ifZero(not(isInstantiated("newSymDef")), SumFeature.createSum(
				applyTF("newSymLeft", tf.atom), applyTF("newSymLeftCoeff", tf.atLeastTwoLiteral),
				applyTF("newSymRight", tf.polynomial),
				instantiate("newSymDef", MonomialColumnOp.create(instOf("newSymLeftCoeff"), instOf("newSymRight"))))));
		...
		bindRuleSet(d, "polySimp_applyEqPseudo", add(applyTF("aePseudoTargetLeft", tf.monomial),
				...
				ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(DiffFindAndIfFeature.INSTANCE,
						applyTF("aePseudoLeft", add(tf.nonCoeffMonomial, not(tf.atom))),
						applyTF("aePseudoLeftCoeff", tf.atLeastTwoLiteral), applyTF("aePseudoRight", tf.polynomial),
						MonomialsSmallerThanFeature.create(instOf("aePseudoRight"), instOf("aePseudoLeft"), numbers),
	...
	private void setupNewSymApproval(RuleSetDispatchFeature d, IntegerLDT numbers) {
		...
		bindRuleSet(d, "polySimp_newSym", add(isInstantiated("newSymDef"),
				sum(antecFor, SequentFormulasGenerator.antecedent(), not(add(columnOpEq, biggerLeftSide)))));
	}
	...
	private void setupDependencyPredicateStrategy(RuleSetDispatchFeature d) {
		...
		final Operator noWaW = funcNames.lookup("noWaW");
...
//	
	...
	private void setupPullOutGcd(RuleSetDispatchFeature d, String ruleSet, boolean roundingUp) {
		...
		bindRuleSet(d, ruleSet,
				add(applyTF("elimGcdLeft", tf.nonNegMonomial), applyTF("elimGcdRight", tf.polynomial), let(gcd,
						CoeffGcdProjection.create(instOf("elimGcdLeft"), instOf("elimGcdRight")),
						add(applyTF(gcd, tf.atLeastTwoLiteral), instantiate("elimGcd", gcd), instantiateDivs))));
	}
	...
	private void setupInEqSimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
		...
		bindRuleSet(d, "inEqSimp_commute", SumFeature.createSum(applyTF("commRight", tf.monomial),
				applyTF("commLeft", tf.polynomial), monSmallerThan("commLeft", "commRight", numbers), longConst(-40)));
		...
		bindRuleSet(d, "inEqSimp_exactShadow", SumFeature.createSum(new Feature[] {
				applyTF("esLeft", tf.nonCoeffMonomial), applyTFNonStrict("esCoeff2", tf.nonNegLiteral),
				applyTF("esRight2", tf.polynomial),
				ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(applyTFNonStrict("esCoeff1", tf.nonNegLiteral),
						applyTF("esRight1", tf.polynomial), not(PolynomialValuesCmpFeature.leq(instOf("esRight2"),
								instOf("esRight1"), instOfNonStrict("esCoeff1"), instOfNonStrict("esCoeff2"))))) }));

		// category "propagation"

		bindRuleSet(d, "inEqSimp_contradInEqs", add(applyTF("contradLeft", tf.monomial),
				ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(DiffFindAndIfFeature.INSTANCE,
						applyTF("contradRightSmaller", tf.polynomial), applyTF("contradRightBigger", tf.polynomial),
						applyTFNonStrict("contradCoeffSmaller", tf.posLiteral),
						applyTFNonStrict("contradCoeffBigger", tf.posLiteral),
						PolynomialValuesCmpFeature.lt(instOf("contradRightSmaller"), instOf("contradRightBigger"),
								instOfNonStrict("contradCoeffBigger"), instOfNonStrict("contradCoeffSmaller"))))));
		...
		bindRuleSet(d, "inEqSimp_subsumption", add(applyTF("subsumLeft", tf.monomial),
				ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(DiffFindAndIfFeature.INSTANCE,
						applyTF("subsumRightSmaller", tf.polynomial), applyTF("subsumRightBigger", tf.polynomial),
						applyTFNonStrict("subsumCoeffSmaller", tf.posLiteral),
						applyTFNonStrict("subsumCoeffBigger", tf.posLiteral),
						PolynomialValuesCmpFeature.leq(instOf("subsumRightSmaller"), instOf("subsumRightBigger"),
								instOfNonStrict("subsumCoeffBigger"), instOfNonStrict("subsumCoeffSmaller"))))));
		...
		bindRuleSet(d, "inEqSimp_and_subsumptionEq",
				SumFeature.createSum(new Feature[] { applyTF("subsumLeft", tf.monomial),
						applyTF("subsumRightSmaller", tf.polynomial), applyTF("subsumRightBigger", tf.polynomial),
						PolynomialValuesCmpFeature.lt(instOf("subsumRightSmaller"), instOf("subsumRightBigger")) }));
		...
		bindRuleSet(d, "inEqSimp_or_tautInEqs",
				SumFeature.createSum(new Feature[] { applyTF("tautLeft", tf.monomial),
						applyTF("tautRightSmaller", tf.polynomial), applyTF("tautRightBigger", tf.polynomial),
		...
		bindRuleSet(d, "inEqSimp_or_weaken",
				SumFeature.createSum(new Feature[] { applyTF("weakenLeft", tf.monomial),
						applyTF("weakenRightSmaller", tf.polynomial), applyTF("weakenRightBigger", tf.polynomial),
						PolynomialValuesCmpFeature.eq(opTerm(numbers.getAdd(), one, instOf("weakenRightSmaller")),
		...
		bindRuleSet(d, "inEqSimp_or_antiSymm",
				SumFeature.createSum(new Feature[] { applyTF("antiSymmLeft", tf.monomial),
						applyTF("antiSymmRightSmaller", tf.polynomial), applyTF("antiSymmRightBigger", tf.polynomial),
						PolynomialValuesCmpFeature.eq(opTerm(numbers.getAdd(), two, instOf("antiSymmRightSmaller")),
	...
	private void setupMultiplyInequations(RuleSetDispatchFeature d, Feature notAllowedF) {
		...
		final Feature totallyBounded = not(sum(intRel, SequentFormulasGenerator.sequent(), not(add(
				applyTF(intRel, tf.intRelation),
				InEquationMultFeature.totallyBounded(instOf("multLeft"), instOf("multFacLeft"), sub(intRel, 0))))));

		final Feature exactlyBounded = not(sum(intRel, SequentFormulasGenerator.sequent(), not(add(
				applyTF(intRel, tf.intRelation),
				InEquationMultFeature.exactlyBounded(instOf("multLeft"), instOf("multFacLeft"), sub(intRel, 0))))));
		...
		bindRuleSet(d, "inEqSimp_nonLin_multiply", add(applyTF("multLeft", tf.nonNegMonomial),
				...
				ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(applyTF("multFacLeft", tf.nonNegMonomial),
						 ...
						 */
						not(TermSmallerThanFeature.create(FocusProjection.create(0), AssumptionProjection.create(0))),
						ifZero(exactlyBounded, longConst(0), ifZero(totallyBounded, longConst(100), notAllowedF))
				/*
				 * ifZero ( partiallyBounded, longConst ( 400 ), notAllowedF ) ) ),
				 */
				/*
				 * applyTF ( "multLeft", rec ( tf.mulF, longTermConst ( 20 ) ) ), applyTF (
				 * "multFacLeft", rec ( tf.mulF, longTermConst ( 20 ) ) ), applyTF (
				 * "multRight", rec ( tf.addF, longTermConst ( 4 ) ) ), applyTF (
				 * "multFacRight", rec ( tf.addF, longTermConst ( 4 ) ) ),
				 */
				), notAllowedF)));
	}
	...
	private void setupInEqSimpInstantiationWithoutRetry(RuleSetDispatchFeature d) {
		...
		bindRuleSet(d, "inEqSimp_nonLin_divide", SumFeature.createSum(new Feature[] {
				applyTF("divProd", tf.nonCoeffMonomial), applyTFNonStrict("divProdBoundNonPos", tf.nonPosLiteral),
				applyTFNonStrict("divProdBoundNonNeg", tf.nonNegLiteral),
				ifZero(MatchedIfFeature.INSTANCE, let(divisor, instOf("divX"),
						let(dividend, instOf("divProd"), SumFeature.createSum(applyTF(divisor, tf.nonCoeffMonomial),
								...
								instantiate("divY", ReduceMonomialsProjection.create(dividend, divisor)))))) }));
	...
	private void setupNonLinTermIsPosNeg(RuleSetDispatchFeature d, String ruleSet, boolean pos) {
		...
		bindRuleSet(d, ruleSet,
				SumFeature.createSum(new Feature[] { applyTF("divProd", tf.nonCoeffMonomial),
						...
						ifZero(MatchedIfFeature.INSTANCE, let(divisor, instOf("divX"), let(dividend, instOf("divProd"),
								SumFeature.createSum(applyTF(divisor, tf.nonCoeffMonomial),
										...
										ReducibleMonomialsFeature.createReducible(dividend, divisor), let(quotient,
	...
	private void setupSquaresAreNonNegative(RuleSetDispatchFeature d) {
		...
		final Feature productContainsSquare = applyTF(sub(product, 0),
	...
	private void setupInEqCaseDistinctions(RuleSetDispatchFeature d) {
		...
		final Feature strengthening = forEach(intRel, SequentFormulasGenerator.antecedent(),
				SumFeature.createSum(applyTF(intRel, add(or(tf.geqF, tf.leqF), sub(tf.atom, tf.literal))),
		...
		final Feature rootInferences = forEach(intRel, SequentFormulasGenerator.antecedent(),
				add(isRootInferenceProducer(intRel), forEach(rootInf, RootsGenerator.create(intRel, getServices()),
						add(instantiate("cutFormula", rootInf), ifZero(applyTF(rootInf, op(Junctor.OR)), longConst(50)),
	...
	private Feature allowPosNegCaseDistinction(TermBuffer atom) {
		...
		return add(not(succIntEquationExists()),
				sum(antecFor, SequentFormulasGenerator.antecedent(),
						not(applyTF(antecFor, or(opSub(tf.eq, eqAtom, any()), opSub(tf.leq, eqAtom, tf.negLiteral),
	...
	private Feature allowInEqStrengthening(TermBuffer atom, TermBuffer literal) {
		...
		return add(not(succIntEquationExists()), not(sum(antecFor, SequentFormulasGenerator.antecedent(),
	...
	private Feature succIntEquationExists() {
		...
		return not(sum(succFor, SequentFormulasGenerator.succedent(), not(applyTF(succFor, tf.intEquation))));
	}
	...
	private void setupInEqCaseDistinctionsApproval(RuleSetDispatchFeature d) {
		...
		bindRuleSet(d, "cut", add(isInstantiated("cutFormula"), or(
				...
				ifZero(applyTF("cutFormula", opSub(tf.eq, tf.atom, tf.literal)), let(atom, sub(instOf("cutFormula"), 0),
						let(literal, sub(instOf("cutFormula"), 1), allowInEqStrengthening(atom, literal)))))));
	}
		...
		if (arithDefOps()) {
			...
			bindRuleSet(d, "defOps_div",
					SumFeature.createSum(new Feature[] { NonDuplicateAppModPositionFeature.INSTANCE,
							...
							applyTF("divNum", tf.notContainsDivMod), applyTF("divDenom", tf.notContainsDivMod),
			...
			bindRuleSet(d, "defOps_jdiv_inline",
					add(applyTF("divNum", tf.literal), applyTF("divDenom", tf.polynomial), longConst(-5000)));
			...
			bindRuleSet(d, "defOps_jdiv_inline",
					add(applyTF("divNum", tf.literal), applyTF("divDenom", tf.literal), longConst(-4000)));
	...
	private void setupDefOpsExpandMod(RuleSetDispatchFeature d) {
		...
		final Feature subsumedModulus = add(applyTF(superTerm, sub(opSub(tf.mod, any(), tf.literal), tf.zeroLiteral)),
		...
		final Feature exSubsumedModulus = add(applyTF("divDenom", tf.literal),
				not(sum(superTerm, SuperTermGenerator.upwardsWithIndex(sub(or(tf.addF, tf.mulF), any()), getServices()),
						not(subsumedModulus))));

		bindRuleSet(d, "defOps_mod",
				ifZero(add(applyTF("divNum", tf.literal), applyTF("divDenom", tf.literal)), longConst(-4000),
						SumFeature.createSum(applyTF("divNum", tf.polynomial), applyTF("divDenom", tf.polynomial),
	...
	private Feature isBelow(TermFeature t) {
		final TermBuffer superTerm = new TermBuffer();
		return not(sum(superTerm, SuperTermGenerator.upwards(any(), getServices()), not(applyTF(superTerm, t))));
	}
	...
	private void setupDivModDivision(RuleSetDispatchFeature d) {
		...
		final Feature checkNumTerm = ifZero(
				add(not(applyTF(numTerm, tf.addF)), ReducibleMonomialsFeature.createReducible(numTerm, denomLC)),
				add(instantiate("polyDivCoeff", ReduceMonomialsProjection.create(numTerm, denomLC)), inftyConst()));

		final Feature isReduciblePoly = sum(numTerm, SubtermGenerator.rightTraverse(instOf("divNum"), tf.addF),
				checkNumTerm);
		...
		final Feature isReduciblePolyE = sum(numTerm, SubtermGenerator.rightTraverse(instOf("divNum"), tf.addF),
				ifZero(applyTF(numTerm, tf.addF), longConst(0),
						sum(divCoeff, MultiplesModEquationsGenerator.create(numTerm, denomLC), checkCoeffE)));
		...
		bindRuleSet(d, "defOps_divModPullOut", SumFeature.createSum(new Feature[] {
				...
				applyTF("divNum", tf.polynomial), applyTF("divDenom", tf.polynomial),
				ifZero(applyTF("divDenom", tf.addF), let(denomLC, sub(instOf("divDenom"), 1), not(isReduciblePoly)),
	...
	private RuleSetDispatchFeature setupApprovalDispatcher() {
		...
		bindRuleSet(d, "apply_select_eq", add(isInstantiated("s"), isInstantiated("t1"), or(
				applyTF("s", rec(any(), SimplifiedSelectTermFeature.create(heapLDT))),
				add(NoSelfApplicationFeature.INSTANCE, applyTF("t1", IsSelectSkolemConstantTermFeature.INSTANCE)))));
		bindRuleSet(d, "apply_auxiliary_eq",
				add(NoSelfApplicationFeature.INSTANCE, isInstantiated("s"),
						applyTF("s", rec(any(), add(SimplifiedSelectTermFeature.create(heapLDT), not(ff.ifThenElse)))),
	...
	private void setupClassAxiomApproval(final RuleSetDispatchFeature d) {
		...
		if (classAxiomApplicationEnabled()) {
			...
			/*
			 * if 'sv_heap' is present and instantiated, allow application only if the heap
			 * used to instantiate 'sv_heap' still occurs in the sequent. Otherwise this was
			 * a rather short lived heap term which has been rewritten since. Hence, we
			 * discard it to avoid too many most likely useless applications.
			 */
	...
	private void setClassAxiomInstantiation(final RuleSetDispatchFeature d) {
		...
		final Feature heapInstantiator = ifZero(needsInstantiation,
				forEach(heapVar, HeapGenerator.INSTANCE_EXCLUDE_UPDATES, add(instantiate("sv_heap", heapVar),
	...
	/**
	 * For taclets that need instantiation, but where the instantiation is
	 * deterministic and does not have to be repeated at a later point, we setup the
	 * same feature terms both in the cost computation method and in the
	 * instantiation method. The definitions in
	 * <code>setupInstantiationWithoutRetry</code> should give cost infinity to
	 * those incomplete rule applications that will never be instantiated (so that
	 * these applications can be removed from the queue and do not have to be
	 * considered again).
	 */
	 ...
	 *         indicates that the rule shall not be applied at all (it is discarded
	 *         by the strategy).
	 */
	...
	/**
	 * Re-Evaluate a <code>RuleApp</code>. This method is called immediately before
	 * a rule is really applied
	 * 
