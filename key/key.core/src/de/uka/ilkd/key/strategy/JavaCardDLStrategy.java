...
public class JavaCardDLStrategy extends AbstractFeatureStrategy {
    ...
    protected JavaCardDLStrategy(Proof p_proof,
        ...
        this.strategyProperties = (StrategyProperties) strategyProperties
                .clone();
    ...
    protected Feature setupGlobalF(Feature dispatcher) {//
        ...
        final String methProp = strategyProperties
        ...
        final String depProp = strategyProperties
        ...
        final String loopProp = strategyProperties
        ...
        final String blockProperty = strategyProperties
        ...
        final Feature oneStepSimplificationF = oneStepSimplificationFeature(
                longConst(-11000));
    ...
    private RuleSetDispatchFeature setupCostComputationF() {
        final IntegerLDT numbers = getServices().getTypeConverter()
                .getIntegerLDT();
        final LocSetLDT locSetLDT = getServices().getTypeConverter()
                .getLocSetLDT();
        ...
        bindRuleSet(d, "concrete", add(longConst(-11000),
        ...
        bindRuleSet(d, "update_elim", add(longConst(-8000),
                ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 10.0)));
        bindRuleSet(d, "update_apply_on_update", add(longConst(-7000),
                ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 10.0)));
        ...
        bindRuleSet(d, "gamma",
                add(not(isInstantiated("t")), ifZero(allowQuantifierSplitting(),
        ...
        bindRuleSet(d, "confluence_restricted", ifZero(
                MatchedIfFeature.INSTANCE, DiffFindAndIfFeature.INSTANCE));

        setupApplyEq(d, numbers);

        bindRuleSet(d, "insert_eq_nonrigid", applyTF(FocusProjection.create(0),
        ...
        bindRuleSet(d, "comprehensions", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(-50)));

        bindRuleSet(d, "comprehensions_high_costs", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(10000)));

        bindRuleSet(d, "comprehensions_low_costs", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(-5000)));
        ...
        default:
            throw new RuntimeException(
                    "Unexpected strategy property " + methProp);
        }
        ...
        default:
            throw new RuntimeException(
                    "Unexpected strategy property " + queryAxProp);
        }
        ...
        bindRuleSet(d, "loop_expand",
                useLoopExpand ? longConst(0) : inftyConst());
        ...
        // partial inv axiom
        bindRuleSet(d, "partialInvAxiom", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(10000)));

        // inReachableState
        bindRuleSet(d, "inReachableStateImplication", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(100)));

        // limit observer (must have better priority than "classAxiom")
        bindRuleSet(d, "limitObserver", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(-200)));
        ...
        if (strategyProperties.contains(StrategyProperties.AUTO_INDUCTION_ON)
            ...
            bindRuleSet(d, "induction_var",
                    ifZero(applyTF(instOf("uSub"),
                            IsInductionVariable.INSTANCE), longConst(0),
                            inftyConst()));
        }
    ...
    private void setupSelectSimplification(final RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "simplify_select",
                ...
                add(applyTF("sk", IsSelectSkolemConstantTermFeature.INSTANCE),
                        applyTF(sub(FocusProjection.INSTANCE, 0), not(
                                SimplifiedSelectTermFeature.create(heapLDT))),
                        longConst(-5600)));
    ...
    private void setUpStringNormalisation(RuleSetDispatchFeature d) {
        ...
        final CharListLDT charListLDT = getServices().getTypeConverter()
                .getCharListLDT();
        final BooleanLDT booleanLDT = getServices().getTypeConverter()
                .getBooleanLDT();

        final TermFeature keepChar = or(op(seqLDT.getSeqSingleton()),
        ...
        final TermFeature anyLiteral = or(tf.charLiteral, or(tf.literal,
                op(booleanLDT.getFalseConst()), op(booleanLDT.getTrueConst())));

        final TermFeature seqLiteral = rec(anyLiteral,
                or(op(seqLDT.getSeqConcat()), or(op(seqLDT.getSeqSingleton()),
        ...
        bindRuleSet(d, "defOpsSeqEquality",
                add(NonDuplicateAppModPositionFeature.INSTANCE,
                        ifZero(add(applyTF("left", seqLiteral),
                                applyTF("right", seqLiteral)), longConst(1000),
                                inftyConst()),
                        belowModOpPenality));
        ...
        bindRuleSet(d, "stringsReduceSubstring", add(
                NonDuplicateAppModPositionFeature.INSTANCE, longConst(100)));
    ...
    private void setupReplaceKnown(RuleSetDispatchFeature d) {
        final Feature commonF = add(
                ifZero(MatchedIfFeature.INSTANCE,
                        DiffFindAndIfFeature.INSTANCE),
                longConst(-5000),
                add(DiffFindAndReplacewithFeature.INSTANCE, ScaleFeature
                        .createScaled(CountMaxDPathFeature.INSTANCE, 10.0)));

        bindRuleSet(d, "replace_known_left", commonF);
        bindRuleSet(d, "replace_known_right", add(commonF,
    ...
    private void setupUserTaclets(RuleSetDispatchFeature d) {
        for (int i = 1; i <= StrategyProperties.USER_TACLETS_NUM; ++i) {
            final String userTacletsProbs = strategyProperties.getProperty(
    ...
    private void setupSystemInvariantSimp(RuleSetDispatchFeature d) {
        bindRuleSet(d, "system_invariant",
                ifZero(MatchedIfFeature.INSTANCE, add(
                        applyTF("negLit", tf.negLiteral), applyTFNonStrict(
                                "nonNegLit", tf.nonNegLiteral))));
    }
    ...
    private Feature allowQuantifierSplitting() {
        if (StrategyProperties.QUANTIFIERS_INSTANTIATE.equals(strategyProperties
    ...
    private boolean classAxiomDelayedApplication() {
        String classAxiomSetting = (String) strategyProperties
    ...
    private boolean autoInductionLemmaEnabled() { // chrisg
        final String prop = strategyProperties
                .getProperty(StrategyProperties.AUTO_INDUCTION_OPTIONS_KEY);
    ...
    private void setupSplitting(RuleSetDispatchFeature d) {
        ...
        final Feature noCutsAllowed = sum(subFor,
        ...
        final ProjectionToTerm splitCondition = sub(FocusProjection.INSTANCE,
                0);
        bindRuleSet(d, "split_cond", add( // do not split over formulas
                ...
                ScaleFeature.createAffine(countOccurrences(splitCondition), -10,
                        10),
        ...
        Feature countOccurrencesInSeq = ScaleFeature
                .createAffine(countOccurrences(cutFormula), -10, 10);
        ...
        bindRuleSet(d, "cut_direct", SumFeature.createSum(new Feature[] {
                ...
                ifZero(NotBelowQuantifierFeature.INSTANCE, add(
    ...
    private void setupSplittingApproval(RuleSetDispatchFeature d) {
        ...
        final Feature noBetterCut = sum(subFor,
                AllowedCutPositionsGenerator.INSTANCE, compareCutAllowed);

        bindRuleSet(d, "cut_direct", add(
    ...
    private void setupApplyEq(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "apply_equations",
                SumFeature.createSum(new Feature[] {
                        ifZero(applyTF(FocusProjection.create(0), tf.intF), add(
                                applyTF(FocusProjection.create(0), tf.monomial),
                                ScaleFeature
                                        .createScaled(FindRightishFeature
                                                .create(numbers), 5.0))),
                        ifZero(MatchedIfFeature.INSTANCE, add(
                                CheckApplyEqFeature.INSTANCE, let(equation,
                                        ...
                                        add(not(applyTF(equation, ff.update)),
                                                ...
                                                // been applied
                                                let(left, sub(equation, 0), let(
                                                        right, sub(equation, 1),
                                                        ifZero(applyTF(left,
                                                                tf.intF),
                                                                add(
                                                                        applyTF(left,
    ...
    private void setupFormulaNormalisation(RuleSetDispatchFeature d,
        ...
        bindRuleSet(d, "negationNormalForm", add(
                not(NotBelowBinderFeature.INSTANCE), longConst(-500),
                ScaleFeature.createScaled(FindDepthFeature.INSTANCE, 10.0)));

        bindRuleSet(d,
                "moveQuantToLeft", add(
                        quantifiersMightSplit() ? longConst(0)
                                : applyTF(FocusFormulaProjection.INSTANCE,
        ...
        bindRuleSet(d, "apply_equations_andOr", add(
                let(left, instOf("applyEqLeft"), let(right,
                        instOf("applyEqRight"), ifZero(applyTF(left, tf.intF),
                        ...
                        or(applyTF(FocusProjection.create(0),
                                ...
                                SumFeature.createSum(
                                        ...
                                        ifZero(FocusInAntecFeature.INSTANCE,
                                                applyTF(FocusProjection.create(
                                                        0), sub(ff.andF)),
                                                applyTF(FocusProjection.create(
                                                        0), sub(ff.orF))))),
                        longConst(-300)));

        bindRuleSet(d, "swapQuantifiers", add(applyTF(FocusProjection.create(0),
        ...
        bindRuleSet(d, "cnf_orComm",
                SumFeature.createSum(
                        new Feature[] { applyTF("commRight", ff.clause),
                                applyTFNonStrict("commResidue", ff.clauseSet),
                                or(applyTF("commLeft", ff.andF),
                                        add(applyTF("commLeft", ff.literal),
                                                literalsSmallerThan("commRight",
                                                        "commLeft", numbers))),
                                longConst(-100) }));

        bindRuleSet(d, "cnf_orAssoc", SumFeature.createSum(new Feature[] {
                applyTF("assoc0", ff.clause), applyTF("assoc1", ff.clause),
        ...
        final TermBuffer superFor = new TermBuffer();
        final Feature onlyBelowQuanAndOr = sum(superFor,
                SuperTermGenerator.upwards(any(), getServices()),
                applyTF(superFor, or(ff.quantifiedFor, ff.andF, ff.orF)));

        final Feature belowUnskolemisableQuantifier = ifZero(
        ...
        bindRuleSet(d, "cnf_expandIfThenElse",
                add(not(NotBelowQuantifierFeature.INSTANCE), onlyBelowQuanAndOr,
                        belowUnskolemisableQuantifier));

        final Feature pullOutQuantifierAllowed = add(
                not(NotBelowQuantifierFeature.INSTANCE), onlyBelowQuanAndOr,
    ...
    private Feature clausesSmallerThan(String smaller, String bigger,
            IntegerLDT numbers) {
        return ClausesSmallerThanFeature.create(instOf(smaller), instOf(bigger),
                numbers);
    }
    ...
    private void setupQuantifierInstantiationApproval(
        ...
        if (quantifierInstantiatedEnabled()) {
            ...
            final TermBuffer splitInst = new TermBuffer();
            bindRuleSet(d, "triggered",
                    add(isTriggerVariableInstantiated(), not(sum(splitInst,
                            TriggeredInstantiations.create(false), not(
                                    eq(instOfTriggerVariable(), splitInst))))));
        }
        else {
            bindRuleSet(d, "gamma", inftyConst());
            bindRuleSet(d, "triggered", inftyConst());
    ...
    private void setupPolySimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "polySimp_homo", add(
                applyTF("homoRight", add(not(tf.zeroLiteral), tf.polynomial)),
        ...
        bindRuleSet(d, "polySimp_balance",
                SumFeature.createSum(new Feature[] { applyTF("sepResidue",
                        tf.polynomial), ifZero(
                                isInstantiated("sepPosMono"),
        ...
        final TermFeature atLeastTwoLCEquation = opSub(Equality.EQUALS,
        ...
        final Feature eq_monomial_feature = add(
                not(DirectlyBelowSymbolFeature.create(tf.mul)),
                ifZero(MatchedIfFeature.INSTANCE,
        ...
        bindRuleSet(d, "defOps_expandModulo",
                add(longConst(-600), ifZero(isBelow(ff.ifThenElse),
                        inftyConst(), IsInRangeProvable.INSTANCE)));
    ...
    // never be instantiated (so that these applications can be removed from
    // the queue and do not have to be considered again).
    private void setupPolySimpInstantiationWithoutRetry(
            RuleSetDispatchFeature d) {
        final IntegerLDT numbers = getServices().getTypeConverter()
                .getIntegerLDT();
        ...
        bindRuleSet(d, "polySimp_applyEqPseudo", add(
    ...
    private void setupNewSymApproval(RuleSetDispatchFeature d,
        ...
        final Feature columnOpEq = applyTF(antecFor, opSub(tf.eq,
                opSub(tf.mul, tf.atom, tf.atLeastTwoLiteral), tf.polynomial));
        final Feature biggerLeftSide = MonomialsSmallerThanFeature.create(
                instOf("newSymLeft"),
                subAt(antecFor, PosInTerm.getTopLevel().down(0).down(0)),
                numbers);
        bindRuleSet(d, "polySimp_newSym",
        ...
        if (roundingUp)
            instantiateDivs = add(
                    ...
                    instantiate("elimGcdRightDiv", DividePolynomialsProjection
                            .createRoundingUp(gcd, instOf("elimGcdRight"))));
        else
            instantiateDivs = add(
                    instantiate("elimGcdLeftDiv",
    ...
    private void setupInEqSimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "inEqSimp_homo", add(
                applyTF("homoRight", add(not(tf.zeroLiteral), tf.polynomial)),
                or(applyTF("homoLeft", or(tf.addF, tf.negMonomial)), not(
                        monSmallerThan("homoRight", "homoLeft", numbers)))));

        // category "direct inequations"

        // this is copied from "polySimp_balance"
        bindRuleSet(d,
                "inEqSimp_balance", add(
                        applyTF("sepResidue", tf.polynomial), ifZero(
                                isInstantiated("sepPosMono"), add(
                                        applyTF("sepPosMono",
                                                tf.nonNegMonomial),
                                        monSmallerThan("sepResidue",
                                                "sepPosMono", numbers))),
        ...
        bindRuleSet(d, "inEqSimp_contradInEqs", add(
        ...
        bindRuleSet(d, "inEqSimp_subsumption", add(
        ...
        bindRuleSet(d, "inEqSimp_and_contradInEqs",
                SumFeature.createSum(
                        new Feature[] { applyTF("contradLeft", tf.monomial),
                                applyTF("contradRightSmaller", tf.polynomial),
                                applyTF("contradRightBigger", tf.polynomial),
                                PolynomialValuesCmpFeature.lt(
                                        instOf("contradRightSmaller"),
        ...
        bindRuleSet(d, "inEqSimp_andOr_subsumption",
                SumFeature.createSum(
                        new Feature[] { applyTF("subsumLeft", tf.monomial),
                                applyTF("subsumRightSmaller", tf.polynomial),
                                applyTF("subsumRightBigger", tf.polynomial),
                                PolynomialValuesCmpFeature.leq(
                                        instOf("subsumRightSmaller"),
        ...
        bindRuleSet(d, "inEqSimp_or_tautInEqs",
                SumFeature.createSum(
                        new Feature[] { applyTF("tautLeft", tf.monomial),
        ...
        bindRuleSet(d, "inEqSimp_or_weaken",
                SumFeature.createSum(
                        new Feature[] { applyTF("weakenLeft", tf.monomial),
        ...
        bindRuleSet(d, "inEqSimp_or_antiSymm",
                SumFeature.createSum(
                        new Feature[] { applyTF("antiSymmLeft", tf.monomial),
    ...
    private void setupMultiplyInequations(RuleSetDispatchFeature d,
        ...
        final Feature totallyBounded = not(sum(intRel,
        ...
        final Feature exactlyBounded = not(sum(intRel,
                SequentFormulasGenerator.sequent(),
                not(add(applyTF(intRel, tf.intRelation),
        ...
        bindRuleSet(d, "inEqSimp_nonLin_multiply", add(
                ...
                ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(
                        ...
                        /*
                         * ifZero ( applyTF ( "multRight", tf.literal ),
                         * longConst ( -100 ), applyTF ( "multRight",
                         * tf.polynomial ) ), ifZero ( applyTF ( "multFacRight",
                         * tf.literal ), longConst ( -100 ), applyTF (
                         * "multFacRight", tf.polynomial ) ),
                         */
                        not(TermSmallerThanFeature.create(
                                FocusProjection.create(0),
                                AssumptionProjection.create(0))),
                        ifZero(exactlyBounded, longConst(0), ifZero(
                                totallyBounded, longConst(100), notAllowedF))
                /*
                 ...
                 * ) ) ), applyTF ( "multRight", rec ( tf.addF, longTermConst (
                 * 4 ) ) ), applyTF ( "multFacRight", rec ( tf.addF,
                 * longTermConst ( 4 ) ) ),
                 */
                ), notAllowedF)));
    }
    ...
    // never be instantiated (so that these applications can be removed from
    // the queue and do not have to be considered again).
    private void setupInEqSimpInstantiationWithoutRetry(
            RuleSetDispatchFeature d) {
        // category "direct inequations"
        ...
        bindRuleSet(d, "inEqSimp_nonLin_divide",
                SumFeature.createSum(new Feature[] {
                        ...
                        applyTFNonStrict("divProdBoundNonNeg",
                                tf.nonNegLiteral),
                        ifZero(MatchedIfFeature.INSTANCE, let(divisor,
                                instOf("divX"), let(dividend, instOf("divProd"),
                                        SumFeature.createSum(
                                                applyTF(divisor,
                                                        tf.nonCoeffMonomial),
                                                not(eq(dividend, divisor)),
                                                applyTFNonStrict("divXBoundPos",
                                                        tf.posLiteral),
                                                applyTFNonStrict("divXBoundNeg",
                                                        tf.negLiteral),
    ...
    private void setupNonLinTermIsPosNeg(RuleSetDispatchFeature d,
        ...
        bindRuleSet(d, ruleSet, SumFeature.createSum(new Feature[] {
                ...
                ifZero(MatchedIfFeature.INSTANCE, let(divisor, instOf("divX"),
                        let(dividend, instOf("divProd"), SumFeature.createSum(
    ...
    private void setupSquaresAreNonNegative(RuleSetDispatchFeature d) {
        ...
        final Feature productIsProduct = applyTF(product,
                opSub(tf.mul, any(), not(tf.mulF)));

        bindRuleSet(d, "inEqSimp_nonNegSquares", forEach(intRel,
                SequentFormulasGenerator.sequent(),
                ifZero(applyTF(intRel, tf.intRelation), forEach(product,
                        SubtermGenerator.leftTraverse(sub(intRel, 0),
                                tf.mulF),
                        ifZero(productIsProduct,
                                let(factor, sub(product, 1), ifZero(
                                        productContainsSquare, instantiate(
                                                "squareFac", factor))))))));
    }
    ...
    private void setupInEqCaseDistinctions(RuleSetDispatchFeature d) {
        ...
        zero.setContent(
                getServices().getTypeConverter().getIntegerLDT().zero());
        final TermBuffer rootInf = new TermBuffer();

        final Feature posNegSplitting = forEach(intRel,
                SequentFormulasGenerator.antecedent(),
                add(applyTF(intRel, tf.intRelation), forEach(atom,
                        SubtermGenerator.leftTraverse(sub(intRel, 0),
                                tf.mulF),
                        SumFeature.createSum(
        ...
        final Feature strengthening = forEach(intRel,
                SequentFormulasGenerator
                        .antecedent(),
                SumFeature.createSum(
                        applyTF(intRel,
                        ...
                        instantiate("cutFormula",
                                opTerm(tf.eq, sub(intRel, 0), sub(intRel, 1))),
        ...
        final Feature rootInferences = forEach(intRel,
                SequentFormulasGenerator.antecedent(),
                add(isRootInferenceProducer(intRel), forEach(rootInf,
                        RootsGenerator.create(intRel, getServices()), add(
                                instantiate("cutFormula", rootInf),
        ...
        bindRuleSet(d, "cut",
                oneOf(new Feature[] { strengthening, rootInferences }));
    }
    ...
    private Feature allowPosNegCaseDistinction(TermBuffer atom) {
        ...
        return add(not(succIntEquationExists()), sum(antecFor,
                SequentFormulasGenerator.antecedent(),
                not(applyTF(antecFor,
                        or(opSub(tf.eq, eqAtom, any()),
                                opSub(tf.leq, eqAtom, tf.negLiteral), opSub(
                                        tf.geq, eqAtom, tf.posLiteral))))));
    }

    private Feature allowInEqStrengthening(TermBuffer atom,
            TermBuffer literal) {
        final TermBuffer antecFor = new TermBuffer();
    ...
    private void setupInEqCaseDistinctionsApproval(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "cut", add(isInstantiated("cutFormula"), or(
                not(sum(intRel,
                        SequentFormulasGenerator.antecedent(),
                        ifZero(isRootInferenceProducer(intRel), sum(
                                rootInf,
    ...
    private void setupDefOpsExpandMod(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "defOps_mod", ifZero(
                add(applyTF("divNum", tf.literal),
                        applyTF("divDenom", tf.literal)),
                longConst(-4000),
                SumFeature.createSum(applyTF("divNum", tf.polynomial),
                        applyTF("divDenom", tf.polynomial),
                        ifZero(isBelow(ff.modalOperator), exSubsumedModulus, or(
    ...
    private Feature isBelow(TermFeature t) {
        final TermBuffer superTerm = new TermBuffer();
        return not(
                sum(superTerm, SuperTermGenerator.upwards(any(), getServices()),
    ...
    protected Feature setupApprovalF() {
        ...
        if (depProp.equals(StrategyProperties.DEP_ON)) {
            depSpecF = ConditionalFeature.createConditional(depFilter,
                    ifZero(new DependencyContractFeature(), longConst(250),
                            inftyConst()));
        }
        else {
            depSpecF = ConditionalFeature.createConditional(depFilter,
    ...
    private RuleSetDispatchFeature setupApprovalDispatcher() {
        ...
        bindRuleSet(d, "apply_select_eq",
                add(isInstantiated("s"), isInstantiated("t1"), or(
                        applyTF("s", rec(
                                any(), SimplifiedSelectTermFeature
                                        .create(heapLDT))),
                        add(NoSelfApplicationFeature.INSTANCE, applyTF("t1",
                                IsSelectSkolemConstantTermFeature.INSTANCE)))));

        bindRuleSet(d, "apply_auxiliary_eq", add(
                NoSelfApplicationFeature.INSTANCE, isInstantiated("s"),
                applyTF("s",
                        rec(any(),
                                add(SimplifiedSelectTermFeature.create(heapLDT),
                                        not(ff.ifThenElse)))),
    ...
    private void setupClassAxiomApproval(final RuleSetDispatchFeature d) {
        ...
        if (classAxiomApplicationEnabled()) {
            ...
            final Feature needsInstantiation = SVNeedsInstantiation
                    .create("sv_heap");
            /*
            ...
            final Feature approveInst = ifZero(isInstantiated("sv_heap"),
                     ...
                     * infinite if it does occur, the outer 'not' then ensures
                     * that the costs are infinite in the first and 0 in the
                     * latter case
                     */
            ...
            if (classAxiomDelayedApplication()) {
                bindRuleSet(d, "classAxiom",
                        add(sequentContainsNoPrograms(),
                                /*
                                 * can be applied if sv_heap is instantiated or
                                 * not present
                                 */
        ...
        final Feature heapInstantiator = ifZero(needsInstantiation,
                forEach(heapVar, HeapGenerator.INSTANCE_EXCLUDE_UPDATES,
                        add(instantiate("sv_heap", heapVar),
                                ...
                                // complicated ones
                                applyTF(heapVar,
                                        rec(not(tf.atom), longTermConst(10))))),
                longConst(0));
    ...
    public final boolean isApprovedApp(RuleApp app, PosInOccurrence pio,
            Goal goal) {
        return !(approvalF.computeCost(app, pio,
                goal) instanceof TopRuleAppCost);
    }
