...
public class MonomialsSmallerThanFeature extends AbstractMonomialSmallerThanFeature {
    ...
    protected boolean lessThan(Term t1, Term t2, PosInOccurrence focus, Goal currentGoal) {
        ...
        if ( t1Deg == 0 ) {
            ...
            final int v =
                introductionTime ( t2.op (), currentGoal )
                - introductionTime ( t1.op (), currentGoal );
            if ( v < 0 ) return true;
            if ( v > 0 ) return false;
        } else {
            ...
            final int v = compareLexNewSyms ( atoms1, atoms2, currentGoal );
            if ( v < 0 ) return true;
            if ( v > 0 ) return false;
        }
        
        return super.lessThan ( t1, t2, focus, currentGoal );
    }
