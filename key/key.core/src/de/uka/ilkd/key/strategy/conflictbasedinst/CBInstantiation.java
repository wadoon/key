...
public class CBInstantiation {
    ...
    private Condition operatorIsSupported=new Condition(){
        ...
        @Override public boolean decide(Term t){return(t.op()instanceof Equality)||(t.op()instanceof Function);}

    };

    private Condition termIsGround=new Condition(){

        @Override public boolean decide(Term t){return(t.boundVars().size()==0);}};
        ...
        private ImmutableSet<ImmutableSet<Term>> falsify(Term term, boolean b) {
            ImmutableSet<ImmutableSet<Term>> ret = DefaultImmutableSet
                    .<ImmutableSet<Term>> nil();
            if (isGround(term)) {
                if (satisfied(term) != b) {
            ...
            if (term.op() == Junctor.OR) {
                if (b) {
                    ImmutableSet<ImmutableSet<Term>> partial = falsify(term.sub(0),
                            b);
        ...
        private ImmutableSet<Term> match(Term term) {
        ...
        private ImmutableSet<Term> getMatches(Term term) {
            ImmutableSet<Term> ret = DefaultImmutableSet.<Term> nil();
            for (Term c : context) {
                ret = ret.union(matches(term, c));
            }
            return ret;
        }

        private ImmutableSet<Term> matches(Term term, Term c) {
            ImmutableSet<Term> ret = DefaultImmutableSet.<Term> nil();
            if (c.op() == term.sub(1).op()) {
                System.out.println(
                        "Matching: " + term.toString() + " and " + c.toString());
                ret = ret.add(tb.equals(term.sub(0), c));
                for (int i = 0; i < c.arity(); i++) {
                    ret = ret.add(tb.equals(term.sub(1).sub(i), c.sub(i)));
                }
            }
            for (Term sub : c.subs()) {
                ret = ret.union(matches(term, sub));
            }
            return ret;
        }
        ...
        public ImmutableSet<Term> getSubstitution() {
            ...
            ImmutableSet<ImmutableSet<Term>> falsifyca = falsify(
                    flat.getFlattenedBody(), true);
            System.out.println("Falsify: " + falsifyca);
            ImmutableSet<Term> matchca = DefaultImmutableSet.<Term> nil();
            for (Term term : flat.getConstraints()) {
                matchca = matchca.union(match(term));
            }
            System.out.println("Match: " + matchca.toString());
            HashSet<Term> A = new HashSet<Term>();
            for (Term term : falsifyca.iterator().next().union(matchca)) {
                A.add(term);
            }
            System.out.println("A: " + A);
            HashSet<Term> B = new HashSet<Term>();
            HashSet<HashSet<Term>> equivB = new HashSet<HashSet<Term>>();
            HashSet<Term> C = new HashSet<Term>();
            for (Term term : B) {
                if (closureContains(equivB, term)) {
                    C.add(term);
                }
                else {
                    addToClosure(equivB, term);
                    B.add(term);
                }
        ...
        private boolean closureContains(HashSet<HashSet<Term>> equivB,
                Term term) {
            return false;
        }
        ...
        private HashSet<Term> getGroundTerms(Term term) {
            ...
            if(isGround(term.sub(0))) ret.add(term.sub(0));
            if(isGround(term.sub(1))) ret.add(term.sub(1));
            return ret;
        }
}
