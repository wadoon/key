...
public class InfFlowContractAppFeature implements Feature {
    ...
    protected boolean sameApplication(RuleApp ruleCmp, TacletApp newApp,
        ...
        if (newPio != null) {
            ...
            final PosInOccurrence oldPio = ((PosTacletApp) cmp)
                    .posInOccurrence();
        ...
        if (newApp.ifFormulaInstantiations() == null
            ...
            final Iterator<IfFormulaInstantiation> it0 = newApp
                    .ifFormulaInstantiations().iterator();
            final Iterator<IfFormulaInstantiation> it1 = cmp
                    .ifFormulaInstantiations().iterator();

            while (it0.hasNext()) {
                // this test should be improved
                if (it0.next().getConstrainedFormula() != it1.next()
    ...
    private boolean equalInterestingInsts(SVInstantiations inst0,
        ...
        final ImmutableMap<SchemaVariable, InstantiationEntry<?>> interesting0 = inst0
                .interesting();
        final ImmutableMap<SchemaVariable, InstantiationEntry<?>> interesting1 = inst1
                .interesting();
    ...
    private boolean subset(
        ...
        for (final ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> entry0 : insts0) {
            if (entry0.key() instanceof SkolemTermSV
                    || entry0.key() instanceof SkolemUpdateSV
            ...
            if (instEntry1 == null || !entry0.value().getInstantiation()
     ...
     * sequent
     */
    protected boolean duplicateFindTaclet(TacletApp app, PosInOccurrence pos,
            Goal goal) {
        ...
        final SequentFormula assumesFor = app.ifFormulaInstantiations()
                .iterator().next().getConstrainedFormula();

        // assumtion has to occour before the find-term in the sequent in order
        // to avoid duplicated applications
        int focusPos = goal.node().sequent().formulaNumberInSequent(antec,
            focusFor);
        int assumesPos = goal.node().sequent().formulaNumberInSequent(antec,
            assumesFor);
    ...
    @Override
    public RuleAppCost computeCost(RuleApp ruleApp, PosInOccurrence pos,
            Goal goal) {
        assert pos != null : "Feature is only applicable to rules with find.";
    ...
    private boolean isInfFlowProof(Proof proof) {
        ProofOblInput po = proof.getServices().getSpecificationRepository()
