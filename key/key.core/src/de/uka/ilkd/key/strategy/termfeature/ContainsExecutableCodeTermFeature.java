...
public class ContainsExecutableCodeTermFeature extends BinaryTermFeature {
    ...
    protected boolean filter(Term t, Services services, MutableState mState) {
        return containsExec ( t, services, mState );
    }

    private boolean containsExec(Term t, Services services, MutableState mState) {
        ...
        for ( int i = 0; i != op.arity (); ++i ) {
            final boolean res = filter ( t.sub ( i ), services, mState );
            if ( res ) return true;
        }
