...
public class GetFrameCondition extends AbstractTermTransformer {
	...
	public Term transform(Term term, SVInstantiations svInst, Services services) {
		...
		MethodFrame mf = JavaTools.getInnermostMethodFrame(svInst.getContextInstantiation().contextProgram(), services);
		Statement activeStmt = (Statement) JavaTools.getActiveStatement(JavaBlock.createJavaBlock(mf.getBody()));
		...
		final List<LocationVariable> heapContext = HeapContext.getModHeaps(services, transaction);
		final ImmutableSet<ProgramVariable> localOuts = MiscTools.getLocalOuts(loop, services);

		final Map<LocationVariable, Map<Term, Term>> heapToBeforeLoop = //
				new LinkedHashMap<LocationVariable, Map<Term, Term>>();
		Term beforeLoopUpdate = createBeforeLoopUpdate(services, heapContext, localOuts, heapToBeforeLoop, term); 

		final TermBuilder tb = services.getTermBuilder();
		// Additional Heap Terms
		final Map<LocationVariable, Term> mods = new LinkedHashMap<LocationVariable, Term>();
		heapContext
				.forEach(heap -> mods.put(heap, spec.getModifies(heap, spec.getInternalSelfTerm(), atPres, services)));
	...
	/**
	 * Creates a conjunction of t1 and t2 if both are not null, and returns t2 only
	 * if t1 is null.
	 * 
	 * @param tb
	 ...
	 * elements in listOfT. fct may return null when applied to a T object; in this
	 * case, the result is ignored when constructing the conjunction.
	 * 
	 * @param services
	 ...
	 */
	protected static <T> Term mapAndConjunct(Services services, java.util.function.Function<T, Term> fct,
			final List<T> listOfT) {
		final TermBuilder tb = services.getTermBuilder();

		// @formatter:off
		return listOfT.stream().map(t -> fct.apply(t)).filter(term -> term != null).reduce(tb.tt(),
				(acc, term) -> tb.and(acc, term));
		// @formatter:on
	}
	 ...
	 */
	protected static Term prepareVariant(Term term, Term variant, TermServices services) {
		final TermBuilder tb = services.getTermBuilder();
		...
		final boolean dia = ((Modality) term.sub(0).op()).terminationSensitive();
	...
	/**
	 * Creates an update for the anonymization of all {@link ProgramVariable}s in
	 * localOuts.
	 * 
	 * @param localOuts
	...
	protected static Term createLocalAnonUpdate(ImmutableSet<ProgramVariable> localOuts, Services services) {
		...
		return localOuts.stream().map(pv -> {
			final Function anonFunc = new Function(new Name(tb.newName(pv.name().toString())), pv.sort(), true);
	 ...
	 */
	protected static Term createBeforeLoopUpdate(Services services, final List<LocationVariable> heapContext,
			final ImmutableSet<ProgramVariable> localOuts,
			final Map<LocationVariable, Map<Term, Term>> heapToBeforeLoop, Term term) {
		final TermBuilder tb = services.getTermBuilder();
		...
		for (ProgramVariable pv : localOuts) {
			...
			final LocationVariable pvBeforeLoop = new LocationVariable(new ProgramElementName(pvBeforeLoopName),
					pv.getKeYJavaType());
			progVarNS.addSafely(pvBeforeLoop);
			beforeLoopUpdate = tb.parallel(beforeLoopUpdate, tb.elementary(pvBeforeLoop, tb.var(pv)));
	 ...
	 */
	protected static AnonUpdateData createAnonUpdate(LocationVariable heap, Term mod, LoopSpecification inv,
			Services services) {
		final TermBuilder tb = services.getTermBuilder();
	...
	/**
	 * A container containing data for the anonymizing update, that is the actual
	 * update and the anonymized heap.
	 */
