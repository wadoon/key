...
public class RewriteTaclet extends FindTaclet {
    ...
    /**
     * all taclet constituents must appear in the same state (and not below a
     * modality (for efficiency reasons))
     */
     ...
     * "wellformed(h) ==>" or in "==> wellformed(h) -> (inv(h) = inv(h2))" or in
     * "==> \if(b) \then(!wellformed(h)) \else(!wellformed(h2))" has antecedent
     * polarity. The formula "wellformed(h)" in "wellformed(h) <->
     * wellformed(h2) ==>" has NO antecedent polarity.
     */
     ...
     * wellformed(h)" or in "wellformed(h) -> (inv(h) = inv(h2)) ==>" or in
     * "\if(b) \then(!wellformed(h)) \else(!wellformed(h2)) ==>" has succedent
     * polarity. The formula "wellformed(h)" in "wellformed(h) <->
     ...
     * must match on a formula within the same state as <code>\find</code> rsp.
     * <code>\add</code>. For efficiency no modalities are allowed above the
     * <code>\find</code> position</li>
     * <li>{@link RewriteTaclet#IN_SEQUENT_STATE} the <code>\find</code> part is
     * only allowed to match on formulas which are evaluated in the same state
     * as the sequent</li>
     * </ul>
     */
     ...
     * @param applPart
     *            the TacletApplPart that contains the application part of an
     *            Taclet that is the if-sequent, the variable conditions
     * @param goalTemplates
     *            a list of goal descriptions.
     * @param ruleSets
     *            a list of rule sets for the Taclet
     * @param attrs
     *            the TacletAttributes; these are boolean values indicating a
     *            noninteractive or recursive use of the Taclet.
     * @param find
     *            the find term of the Taclet
     * @param prefixMap
     *            a ImmMap<SchemaVariable,TacletPrefix> that contains the prefix
     *            for each SchemaVariable in the Taclet
     * @param p_applicationRestriction
     *            an int defining state restrictions of the taclet (required for
     *            location check)
     * @param choices
    ...
    public RewriteTaclet(Name name, TacletApplPart applPart,
            ...
            ImmutableList<RuleSet> ruleSets, TacletAttributes attrs, Term find,
    ...
    /**
     * this method is used to determine if top level updates are allowed to be
     * ignored. This is the case if we have an Antec or SuccTaclet but not for a
     * RewriteTaclet
     *
     ...
     * checks if the assignment pairs of the update contain free logic variables
     * and gives a veto if positive
     *
     * @param t
     *            the Term to check
     ...
     */
    private int polarity(final Operator op, final PIOPathIterator it,
            int polarity) {
        // toggle polarity if find term is
        // subterm of
        if ((op == Junctor.NOT) || // not
                (op == Junctor.IMP && it.getChild() == 0)) { // left hand side
                                                             // of implication
            polarity = polarity * -1;
            ...
            // is subterm of
                ...
                (op == Junctor.OR) || // or
                (op == Junctor.IMP && it.getChild() != 0) || // right hand side
                                                             // of implication
                (op == IfThenElse.IF_THEN_ELSE && it.getChild() != 0)) { // then
                                                                         // or
                                                                         // else
                                                                         // part
                                                                         // of
    ...
    protected StringBuffer toStringFind(StringBuffer sb) {
        ...
        if ((getApplicationRestriction()
                & RewriteTaclet.IN_SEQUENT_STATE) != 0) {
            res.append("\\inSequentState");
        }
        if ((getApplicationRestriction()
                & RewriteTaclet.ANTECEDENT_POLARITY) != 0) {
            res.append("\\antecedentPolarity");
        }
        if ((getApplicationRestriction()
                & RewriteTaclet.SUCCEDENT_POLARITY) != 0) {
    ...
    public SequentFormula getRewriteResult(Goal goal,
        ...
        return getExecutor().getRewriteResult(goal, termLabelState, services,
            app);
    }
    ...
    public RewriteTaclet setName(String s) {
        ...
        return new RewriteTaclet(new Name(s), applPart, goalTemplates(),
            getRuleSets(), attrs, find, prefixMap, applicationRestriction,
            choices, getSurviveSymbExec(), tacletAnnotations);
    }
