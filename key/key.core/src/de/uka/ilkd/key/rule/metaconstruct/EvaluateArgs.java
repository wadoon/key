...
public class EvaluateArgs extends ProgramTransformer {
    ...
    public ProgramElement transform(ProgramElement pe, Services services,
        ...
        MethodOrConstructorReference mr = (MethodOrConstructorReference) (pe instanceof CopyAssignment
                ? ((CopyAssignment) pe).getChildAt(1) : pe);

        List<Statement> evalstat = new LinkedList<Statement>();

        ReferencePrefix newCalled = null;
        ...
        if (invocationTarget instanceof Expression
            ...
            newCalled = evaluate((Expression) invocationTarget, evalstat,
                    services, ec);
        } else if (invocationTarget instanceof ThisReference) {
        ...
        if (mr instanceof New) {
            resMR = KeYJavaASTFactory.newOperator(mr.getReferencePrefix(),
                ...
                || (mr instanceof MethodReference && ((MethodReference) mr)
                        .getReferencePrefix() instanceof SuperReference)) {
        ...
        } else if (mr instanceof MethodReference) {
