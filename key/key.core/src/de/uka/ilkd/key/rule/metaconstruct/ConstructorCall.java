...
public class ConstructorCall extends ProgramTransformer {
	...
	int j=0;
	if(services.getProgramServices().getJavaInfo().getAttribute(ImplicitFieldAdder.IMPLICIT_ENCLOSING_THIS, classType)!=null){
	    j=1;
	}
	...
	if ( j == 1 ) {
            Sort s = services.getProgramServices().getJavaInfo().getAttribute(ImplicitFieldAdder.IMPLICIT_ENCLOSING_THIS, classType).sort();
	    Expression enclosingThis = (Expression) (constructorReference.getReferencePrefix() instanceof Expression?
	            constructorReference.getReferencePrefix() :
                        services.getProgramServices().getTypeConverter().convertToProgramElement(
                                services.getProgramServices().getTypeConverter().findThisForSort(s, ec)));
	...
	final MethodBodyStatement mbs = KeYJavaASTFactory.methodBody(
		services.getProgramServices().getJavaInfo(), null, newObject, classType,
