...
public abstract class TacletApp implements RuleApp {
    ...
    protected static SVInstantiations resolveCollisionVarSV(Taclet taclet,
            SVInstantiations insts, Services services) {
     ...
     *
     * @param boundVars
     *            ImmutableArray<QuantifiableVariable> with the bound
     *            SchemaVariables
     * @param x
    ...
    private static boolean contains(
            ImmutableArray<QuantifiableVariable> boundVars, LogicVariable x,
    ...
    /*
     * checks if application conditions are satisfied and returns
     ...
     */
    /*
     * public ImmutableList<Goal> executeForceIf(Goal goal, Services services) {
     ...
     * of Taclets
     *
     * @return ImmutableSet<SchemaVariable> that need to be instantiated but are
     *         not
     */
    protected ImmutableSet<SchemaVariable> calculateNonInstantiatedSV() {
        if (missingVars == null) {
            ImmutableSet<SchemaVariable> localMissingVars = DefaultImmutableSet
                    .<SchemaVariable> nil();
     ...
     *
     * @return ImmutableSet<SchemaVariable> with SchemaVariables that have not
    ...
    /**
     * returns true if the given {@link SchemaVariable} must be explicitly
     * instantiated it does not check whether sv is already instantiated or not
     *
     * @param sv
     *            the SchemaVariable
     ...
     */
    private Collection<String> collectClashNames(SchemaVariable sv,
        ...
        for (Named progvar : services.getNamespaces().programVariables()
                .allElements()) {
    ...
    public abstract TacletApp addInstantiation(SchemaVariable sv,
            ProgramElement pe, boolean interesting, Services services);
    ...
    public TacletApp setIfFormulaInstantiations(
            ImmutableList<IfFormulaInstantiation> p_list, Services p_services) {
     ...
     */
    public ImmutableList<TacletApp> findIfFormulaInstantiations(Sequent p_seq,
            Services p_services) {
     ...
     * applicable) as otherwise the TacletApp may become invalid. (This happened
     * sometimes during interactive proofs).
     *
     * @param pos
    ...
    public int hashCode() {
        ...
        result = 37 * result
                + (ifInstantiations == null ? 0 : ifInstantiations.hashCode());
        return result;
    }

    @Override
    public String toString() {
        return "Application of Taclet " + taclet() + " with " + instantiations()
                + " and " + ifFormulaInstantiations();
    }
    ...
    protected abstract ImmutableSet<QuantifiableVariable> contextVars(
            SchemaVariable sv);
     ...
     */
    public Namespace<Function> extendedFunctionNameSpace(
        ...
        while (it.hasNext()) {
            ...
            if (sv instanceof SkolemTermSV || sv instanceof SkolemUpdateSV) {
                ...
                Operator op = inst.op();
                assert op instanceof Function : "At this point the skolem instantiation is expected to "
    ...
    public static boolean checkVarCondNotFreeIn(Taclet taclet,
        ...
        while (it.hasNext()) {
            SchemaVariable sv = it.next();
            if (sv instanceof TermSV || sv instanceof FormulaSV) {
                final Term instantiation = (Term) instantiations
                        .getInstantiation(sv);
                if (!instantiation.freeVars().isEmpty()
                        && !instantiation.freeVars()
                                .subset(boundAtOccurrenceSet(
                                        taclet.getPrefix(sv), instantiations,
                                        pos))) {
                    return false;
                }
            }
