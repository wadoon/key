 ...
 * 
 *  To create such a program use the static method {@link #createProgram(JavaDLTerm)} and provide as argument the pattern
 *  for which you want to get a matcher.
 *  
 *  The program is executed by invoking {@link TacletMatchProgram#match(JavaDLTerm, MatchConditions, Services)}. 
 */
public class TacletMatchProgram {
    
    /**
     * creates a matcher for the given pattern
     * @param pattern the {@link JavaDLTerm} specifying the pattern
     * @return the specialized matcher for the given pattern
     */
    public static TacletMatchProgram createProgram(JavaDLTerm pattern) {
     ...
     * @param pattern the JavaDLTerm used as pattern for which to create a matcher
     * @param program the list of {@link MatchInstruction} to which the instructions for matching 
     * {@code pattern} are added.
     */
    private static void createProgram(JavaDLTerm pattern, ArrayList<MatchInstruction> program) {
        ...
        if (pattern.op() instanceof Modality || pattern.op() instanceof ModalOperatorSV) {
            program.add(Instruction.matchProgram(pattern.modalContent().program()));
        }
     ...
     * @param p_toMatch the {@link JavaDLTerm} to match
     ...
     */
    public MatchConditions match(JavaDLTerm p_toMatch, 
