...
/**
 * Matching algorithm using a virtual machine based approach inspired by
 * Voronkonv et al. It matches exactly one taclet and does not produce code
 * trees.
 *
...
public class VMTacletMatcher implements TacletMatcher {
    ...
    private final HashMap<Term, TacletMatchProgram> assumesMatchPrograms =
            new HashMap<>();
    ...
    /**
     * flag indicating if preceding updates of the term to be matched should be
     * ignored this requires the taclet to ignore updates and that the find term
     * does not start with an {@link UpdateApplication} operator
     */
    private final boolean ignoreTopLevelUpdates;
    /**
     * the find expression of the taclet of {@code null} if it is a
     * {@link NoFindTaclet}
     */
    ...
    public VMTacletMatcher(Taclet taclet) {
        ...
        if (taclet instanceof FindTaclet) {
            ...
            ignoreTopLevelUpdates =
                    ((FindTaclet) taclet).ignoreTopLevelUpdates()
        ...
        for (SequentFormula sf : assumesSequent) {
            assumesMatchPrograms.put(sf.formula(),
                    TacletMatchProgram.createProgram(sf.formula()));
        }
    }
     ...
     * (non-Javadoc)
     *
     * @see de.uka.ilkd.key.rule.TacletMatcher#matchIf(ImmutableList,
     *      de.uka.ilkd.key.logic.Term, de.uka.ilkd.key.rule.MatchConditions,
     *      de.uka.ilkd.key.java.Services)
     */
    @Override
    public final IfMatchResult matchIf(
        ...
        while (!workingList.isEmpty()) {
            ...
            if (formula != null) {// update context not present or update
                                  // context match succeeded
                final MatchConditions newMC = checkConditions(
                        prg.match(formula, p_matchCond, p_services),
                        p_services);
    ...
    /**
     * the formula ensures that the update context described the update of the
     * given formula It it does not {@code null} is returned, otherwise the
     * formula without the update context.
     *
     * @param context
     *            the list of update label pairs describing the update context
     * @param formula
     *            the formula whose own update context must be equal (modulo
     *            renaming) to the given one
     * @return {@code null} if the update context does not match the one of the
     *         formula or the formula without the update context
     */
    private Term matchUpdateContext(ImmutableList<UpdateLabelPair> context,
        ...
        for (int i = 0, size = context.size(); i < size; i++) {
            if (formula.op() instanceof UpdateApplication) {
                ...
                /*
                 * NOTE (DS, 2018-12-17): This seems really fishy since (1) term
                 * labels should not affect the logic side and (2) it happened
                 * here that an update (not an application) was assumed to be an
                 * application, so things failed.
                 *
     ...
     */
    @Override
    public final MatchConditions matchIf(
        ...
        for (final IfFormulaInstantiation candidateInst : p_toMatch) {
            assert itIfSequent
                    .hasNext() : "p_toMatch and assumes sequent must have same number of elements";
            newMC = matchIf(
                    ImmutableSLList.<IfFormulaInstantiation> nil()
                            .prepend(candidateInst),
                    itIfSequent.next().formula(), p_matchCond, p_services)
                            .getMatchConditions();
     ...
     */
    @Override
    public final MatchConditions checkConditions(MatchConditions cond,
            Services services) {
        ...
        if (result != null) {
            ...
            while (result != null && svIterator.hasNext()) {
                ...
                final Object o =
                        result.getInstantiations().getInstantiation(sv);
                if (o instanceof SVSubstitute) {
                    result = checkVariableConditions(sv, (SVSubstitute) o,
                            result, services);
                }
            }
        }
    ...
    /**
     * returns true iff the given variable is bound either in the ifSequent or
     * in any part of the TacletGoalTemplates
     *
     * @param v
    ...
    private boolean varIsBound(SchemaVariable v) {
        return (v instanceof QuantifiableVariable)
                && boundVars.contains((QuantifiableVariable) v);
    }
    ...
    public final MatchConditions checkVariableConditions(SchemaVariable var,
        ...
        if (matchCond != null) {
            if (instantiationCandidate instanceof Term) {
                ...
                if (!(term.op() instanceof QuantifiableVariable)) {
                    if (varIsBound(var) || varDeclaredNotFree(var)) {
                        // match(x) is not a variable, but the corresponding
                        // template variable is bound
                        // or declared non free (so it has to be matched to a
                        // variable)
                        return null; // FAILED
                    }
                }
            ...
            for (final VariableCondition vc : varconditions) {
                matchCond = vc.check(var, instantiationCandidate, matchCond,
                        services);
                if (matchCond == null) {
                    return null; // FAILED
                }
    ...
    /**
     * ignores a possible update prefix This method assumes that the taclet
     * allows to ignore updates and the find expression does not start with an
     ...
     * @param services
     *            the Services
     * @return a pair of updated match conditions and the unwrapped term without
     *         the ignored updates (Which have been added to the update context
     *         in the match conditions)
     */
    private Pair<Term, MatchConditions> matchAndIgnoreUpdatePrefix(
            final Term term, MatchConditions matchCond,
        ...
        if (sourceOp instanceof UpdateApplication) {
            ...
            matchCond = matchCond.setInstantiations(matchCond
                    .getInstantiations().addUpdate(update, term.getLabels()));
    ...
    public final MatchConditions matchFind(Term term, MatchConditions matchCond,
        ...
        if (findMatchProgram != TacletMatchProgram.EMPTY_PROGRAM) {
            ...
            matchCond = checkConditions(
                    findMatchProgram.match(term, matchCond, services),
                    services);
        }
        else {
            matchCond = null;
