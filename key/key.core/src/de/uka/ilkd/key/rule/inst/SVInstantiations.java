...
/**
 * This class wraps a ImmMap<SchemaVariable,InstantiationEntry<?>> and is used
 * to store instantiations of schemavariables. The class is immutable, this
 * means changing its content will result in creating a new object.
 */
public class SVInstantiations {
    ...
    private SVInstantiations() {
        ...
        map = DefaultImmutableMap
                .<SchemaVariable, InstantiationEntry<?>> nilMap();
        interesting = DefaultImmutableMap
                .<SchemaVariable, InstantiationEntry<?>> nilMap();
    }
    ...
    public SVInstantiations add(ModalOperatorSV sv, Operator op,
            Services services) {
    ...
    public SVInstantiations add(SchemaVariable sv,
            ImmutableArray<TermLabel> labels, Services services) {
    ...
    public SVInstantiations add(PosInProgram prefix, PosInProgram postfix,
            ...
            Services services) {
        return add(CONTEXTSV, new ContextInstantiationEntry(prefix, postfix,
                activeStatementContext, pe), services);
    }
    ...
    private SVInstantiations checkSorts(SchemaVariable p_sv,
            InstantiationEntry<?> p_entry, boolean p_forceRebuild,
            Services services) {
    ...
    private SVInstantiations rebuildSorts(Services services) {
        genericSortInstantiations = GenericSortInstantiations
                .create(map.iterator(), getGenericSortConditions(), services);
        return this;
    }
    ...
    public SVInstantiations add(SchemaVariable sv, InstantiationEntry<?> entry,
        ...
        return new SVInstantiations(map.put(sv, entry), interesting(),
                ...
                getGenericSortConditions()).checkSorts(sv, entry, false,
                        services);
    }

    public SVInstantiations addInteresting(SchemaVariable sv,
            InstantiationEntry<?> entry, Services services) {
    ...
    public SVInstantiations addInteresting(SchemaVariable sv, Name name,
            Services services) {
        ...
        if (name.equals(oldValue)) {
            ...
            return addInteresting(sv, new NameInstantiationEntry(name),
                    services);
        }
    }
     ...
     *            the SchemaVariable to be instantiated
     * @param entry
     *            the InstantiationEntry<?> the SchemaVariable is instantiated
     *            with
     */
    ...
    public SVInstantiations makeInteresting(SchemaVariable sv,
        ...
        return new SVInstantiations(map, interesting().put(sv, entry),
                getUpdateContext(), getGenericSortConditions()).checkSorts(sv,
                        entry, true, services);
     ...
     */
    public SVInstantiations replace(SchemaVariable sv,
            ImmutableArray<ProgramElement> pes, Services services) {
    ...
    public SVInstantiations replace(PosInProgram prefix, PosInProgram postfix,
            ExecutionContext activeStatementContext, ProgramElement pe,
            Services services) {
        return replace(CONTEXTSV, new ContextInstantiationEntry(prefix, postfix,
                activeStatementContext, pe), services);
    }
    ...
    public SVInstantiations addUpdate(Term update,
        ...
        return new SVInstantiations(map, interesting(),
                updateContext.append(
                        new UpdateLabelPair(update, updateApplicationlabels)),
                getGenericSortInstantiations(), getGenericSortConditions());
    }
    ...
    public static class UpdateLabelPair {
        ...
        public UpdateLabelPair(Term update,
                ImmutableArray<TermLabel> updateApplicationlabels) {
            this.update = update;
    ...
    public SVInstantiations addUpdateList(
            ImmutableList<UpdateLabelPair> updates) {
    ...
    public SVInstantiations clearUpdateContext() {
        ...
        return new SVInstantiations(map, interesting(),
                ImmutableSLList.<UpdateLabelPair> nil(),
                getGenericSortInstantiations(), getGenericSortConditions());
    }
    ...
    /**
     * returns iterator of the mapped pair (SchemaVariables,
     * InstantiationEntry<?>)
     *
    ...
    public boolean isEmpty() {
        ...
        return this == EMPTY_SVINSTANTIATIONS || (map.isEmpty()
                && updateContext.isEmpty() && genericSortConditions.isEmpty()
    ...
    public boolean equals(Object obj) {
        ...
        while (it.hasNext()) {
            final ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> e = it
                    .next();
    ...
    public SVInstantiations union(SVInstantiations other, Services services) {
        ...
        final Iterator<ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>>> it = other.map
                .iterator();

        while (it.hasNext()) {
            final ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> entry = it
                    .next();
            result = result.put(entry.key(), entry.value());
        }

        ImmutableList<UpdateLabelPair> updates = ImmutableSLList
                .<UpdateLabelPair> nil();
     ...
     */
    public SVInstantiations add(GenericSortCondition p_c, Services services)
            throws SortException {
        return new SVInstantiations(map, interesting(), getUpdateContext(),
                getGenericSortInstantiations(),
                getGenericSortConditions().prepend(p_c)).checkCondition(p_c,
                        false, services);
    }
    ...
    public ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> lookupEntryForSV(
            Name name) {
        final Iterator<ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>>> it = map
                .iterator();
        while (it.hasNext()) {
            final ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> e = it
                    .next();
    ...
    public SchemaVariable lookupVar(Name name) {
        final ImmutableMapEntry<SchemaVariable, InstantiationEntry<?>> e = lookupEntryForSV(
                name);
