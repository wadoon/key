...
public final class ApplyUpdateOnRigidCondition implements VariableCondition {
    ...
    public ApplyUpdateOnRigidCondition(UpdateSV u, SchemaVariable x,
            SchemaVariable x2) {
        this.u = u;
        this.x = x;
        this.x2 = x2;
    ...
    private static Term applyUpdateOnRigid(Term update, Term target,
        ...
        for (int i = 0; i < updatedSubs.length; i++) {
            updatedSubs[i] = services.getTermBuilder().apply(update,
                    target.sub(i), null);
        }
        Term result = services.getTermFactory().createTerm(target.op(),
                updatedSubs, target.boundVars(), target.javaBlock());
        return result;
    }
