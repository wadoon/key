...
public class WhileLoopTransformation extends JavaASTVisitor {
    ...
    public WhileLoopTransformation(ProgramElement root,
            ...
            Services services) {
        super(root, services);
        breakOuterLabel = (outerLabel == null ? null
                : KeYJavaASTFactory.breakStatement(outerLabel));
        breakInnerLabel = (innerLabel == null ? null
                : KeYJavaASTFactory.breakStatement(innerLabel));
    ...
    public WhileLoopTransformation(ProgramElement root, SVInstantiations inst,
            Services services) {
        super(root, services);
        instantiations = (inst == null ? SVInstantiations.EMPTY_SVINSTANTIATIONS
                : inst);
        replaceBreakWithNoLabel = 0;
        runMode = CHECK;
    }
        ...
        DefaultAction def = new DefaultAction() {
            ProgramElement createNewElement(ExtList changeList) {
                ...
                ImmutableSet<BlockContract> bcs = services
                        .getSpecificationRepository().getBlockContracts(x);
                if (bcs != null) {
     ...
     * == x.getInitializers().size(), "Critical Error: not all initializers
     * found. "+ "performActionOnFor in WhileLoopTransformation."); // get guard
     * if (x.getGuard() != null) { guard = (Expression)element; }
     * 
    ...
    public void performActionOnFor(For x) {
        ...
        if (replaceBreakWithNoLabel == 0) {
            ...
            final Statement outerBlockStatements[] = new Statement[initSize
                    + 1];
            ...
            outerBlockStatements[initSize] = KeYJavaASTFactory
                    .ifThen(guard.getExpression(), innerBlockStatements);
     ...
     * <p>
     * Warning: The unwinding does not comply with the rule in the KeY book up
     * to 100% The difference is revealed by the following example:
     * <p>
    ...
    public void performActionOnWhile(While x) {
        ...
        if (replaceBreakWithNoLabel == 0) {
            ...
            ProgVarReplaceVisitor replacer = new ProgVarReplaceVisitor(body,
                    new LinkedHashMap<ProgramVariable, ProgramVariable>(), true,
                    services);
            replacer.start();
