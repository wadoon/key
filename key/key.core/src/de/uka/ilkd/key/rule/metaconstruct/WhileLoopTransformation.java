...
public class WhileLoopTransformation extends JavaASTVisitor {
    ...
    /**
     * indicates if an unlabled break has been found and an outer label is
     * needed
     */
    protected boolean needOuterLabel = false;
    /**
     * Indicates if an unlabled continue has been found or if a labelled
     * continue with a label outside of the loop currently transformed has been
     * found. Then an inner label is needed.
     */
    protected boolean needInnerLabel = false;
    /**
     * counts the number of labelled continues with an label outside the while
     * loop that is transformed
     */
    ...
    public WhileLoopTransformation(ProgramElement root,
        ...
        super(root, services);
        breakOuterLabel =
                (outerLabel == null ? null : KeYJavaASTFactory
                        .breakStatement(outerLabel));
        breakInnerLabel =
                (innerLabel == null ? null : KeYJavaASTFactory
    ...
    protected void doAction(ProgramElement node) {
        if (runMode == CHECK) {
            ...
            if (node instanceof Break || node instanceof Continue
                    || node instanceof SchemaVariable || node instanceof Return) {
                node.visit(this);
            }
        }
        else {
    ...
    public void performActionOnSchemaVariable(SchemaVariable sv) {
        ...
        /**
         * This was the part handling only ProgramElements ProgramElement pe =
         * (ProgramElement) instantiations.getInstantiation(sv); if (pe != null)
         * { walk(pe); } else { // we cannont decide whether there are unlabeled
         * breaks that is why // both labeled are needed needInnerLabel = true;
         * needOuterLabel = true; }
         */
    ...
    /**
     *
     * public void performActionOnFor(For x) { ExtList changeList =
     * stack.peek(); if (replaceBreakWithNoLabel==0){ //most outer for loop if
     ...
     * //remainding 'for' statement Expression[] unchangedUpdates = new
     * Expression[x.getUpdates().size()];
     * 
     * Expression guard = null; Statement body = null; ProgramElement element =
     * (ProgramElement) (changeList.isEmpty() ? null :
     * changeList.removeFirst()); // get loop initializers int foundInitializers
     * = 0; while (element instanceof LoopInitializer) { init[foundInitializers]
     * = (LoopInitializer) element; element = (ProgramElement)
     * (changeList.isEmpty() ? null : changeList.removeFirst());
     * foundInitializers++; } de.uka.ilkd.key.util.Debug.assertTrue (init.length
     * == x.getInitializers().size(),
     * "Critical Error: not all initializers found. "+
     * "performActionOnFor in WhileLoopTransformation."); // get guard if
     * (x.getGuard() != null) { guard = (Expression)element; }
     * 
     * // getUpdates int foundUpdates = 0; element = (ProgramElement)
     * (changeList.isEmpty() ? null : changeList.removeFirst()); while (element
     * instanceof Expression && foundUpdates<x.getUpdates().size()) {
     * updates[foundUpdates] = (Expression) element; element = (ProgramElement)
     * (changeList.isEmpty() ? null : changeList.removeFirst());
     * unchangedUpdates[foundUpdates] =
     * x.getUpdates().getExpression(foundUpdates); foundUpdates++; }
     * de.uka.ilkd.key.util.Debug.assertTrue (updates.length ==
     * x.getUpdates().size(), "Critical Error: not all updates found. "+
     * "performActionOnFor in WhileLoopTransformation.");
     * 
     * // getBody body = (Statement) element;
     * 
     * For remainder = new For(null, x.getGuard(), unchangedUpdates,
     * x.getBody()); if (breakInnerLabel!=null) body = (Statement) new
     * LabeledStatement(breakInnerLabel.getLabel(), body);
     * 
     * 
     * Statement innerBlockStatements[] = new Statement[updates.length+2];
     * innerBlockStatements[0] = body; for (int copyStatements=0;
     * copyStatements<updates.length;copyStatements++)
     * innerBlockStatements[copyStatements+1] = (ExpressionStatement)
     * updates[copyStatements]; innerBlockStatements[updates.length+1] =
     * remainder;
     * 
     * Statement outerBlockStatements[] = new Statement[init.length+1]; for (int
     * copyStatements=0; copyStatements<init.length;copyStatements++)
     * outerBlockStatements[copyStatements] = init[copyStatements];
     * outerBlockStatements[init.length] = new If(guard ,new Then(new
     * StatementBlock(new ArrayOf<Statement>(innerBlockStatements))));
     * //outerBlockStatements[init.length+1] = remainder;
     * 
     * if (breakOuterLabel!=null) addChild(new
     * LabeledStatement(breakOuterLabel.getLabel(), new StatementBlock( new
     * ArrayOf<Statement>(outerBlockStatements)))); else addChild(new
     * StatementBlock(new ArrayOf<Statement>(outerBlockStatements))); changed();
     ...
     * // getUpdates int foundUpdates = 0; element = (ProgramElement)
     * (changeList.isEmpty() ? null : changeList.removeFirst()); while (element
     * instanceof Expression) { updates[foundUpdates] = (Expression) element;
     * element = (ProgramElement) (changeList.isEmpty() ? null :
     * changeList.removeFirst()); foundUpdates++; }
     * de.uka.ilkd.key.util.Debug.assertTrue (updates.length ==
     * x.getUpdates().size(), "Critical Error: not all updates found. "+
     * "performActionOnFor in WhileLoopTransformation.");
     * 
     * // getBody body = (Statement) element; addChild(new For(init, guard,
     * updates, body)); changed(); } else { doDefaultAction(x); } } }
     * 
     */
     ...
     * 
     * If the enhanced for loop is the toplevel loop nothing happens - return
     * the loop itself, as enhanced for loops cannot be unwound, a log message
     * is issued.
     *
     * If it is a loop deeper in the AST a new object is created if needed or
     * the original loop returned.
     * 
     * @author mulbrich
     */
    public void performActionOnEnhancedFor(EnhancedFor x) {
        ...
        if (replaceBreakWithNoLabel == 0) {
            // the outermost loop
            Debug.log4jError(
                    "Enhanced for loops may not be toplevel in WhileLoopTransformation",
                    null);
            doDefaultAction(x);
        }
        else {
    ...
    /*
     * Performs the unwinding of the loop Warning: The unwinding does not comply
     * with the rule in the KeY book up to 100% The difference is revealed by
     * the following example: <code> Label1:while(c){b}</code> According to the
     * KeY book the transformation should be <code> if(c) l':{l'':{p#}
     * Label1:while(c){b}}</code> This implementation creates however. <code>
     * Label1:if(c) l':{l'':{p#} while(c){b}}</code> Check if this is ok when
     * labeled continue statements are involved.
     */
    public void performActionOnWhile(While x) {
        ...
        if (replaceBreakWithNoLabel == 0) {
            ...
            /*
             * rename all occ. variables in the body (same name but different
             * object)
             */
            ...
            if (outerLabelNeeded() && breakOuterLabel != null) {
                ...
                then =
                        KeYJavaASTFactory.thenBlock(KeYJavaASTFactory
                                .labeledStatement(breakOuterLabel.getLabel(),
            ...
            if (changeList.getFirst() == CHANGED) {
                ...
                Statement body =
                        (Statement) (changeList.isEmpty() ? null : changeList
                                .removeFirst());
                While newLoop =
                        KeYJavaASTFactory.whileLoop(guard, body,
                                x.getPositionInfo());
                services.getSpecificationRepository().copyLoopInvariant(x,
                        newLoop);
                addChild(newLoop);
