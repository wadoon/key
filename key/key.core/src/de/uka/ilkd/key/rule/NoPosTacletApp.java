 ...
 * has been inserted by an addrule part of another taclet. For this reason the
 * {@link de.uka.ilkd.key.proof.TacletIndex} manages no position taclet
 * application objects instead of the taclets itself.</li>
 * </ul>
 *
 */
public class NoPosTacletApp extends TacletApp {
     ...
     * instantiations and CHECKS variable conditions as well as it resolves
     * collisions The ifInstantiations parameter is not matched against the if
     * sequence, but only stored. For matching use the method
    ...
    public static NoPosTacletApp createNoPosTacletApp(Taclet taclet,
        ...
        return createNoPosTacletApp(taclet, matchCond.getInstantiations(), null,
            services);
    }

    /**
     * Create TacletApp with immutable "instantiations", i.e. this
     * instantiations must not be modified later (e.g. by "addInstantiation").
     * However, this information is currently only used to decide about
     * introduction of metavariables. Immutable instantiations are important for
     * the "addrules" part of taclets.
     */
    public static NoPosTacletApp createFixedNoPosTacletApp(Taclet taclet,
            SVInstantiations instantiations, Services services) {
        NoPosTacletApp res = createNoPosTacletApp(taclet, instantiations, null,
            services);
    ...
    protected static boolean checkVarCondNotFreeIn(Taclet taclet,
        ...
        while (it.hasNext()) {
            ...
            if (sv instanceof ModalOperatorSV || sv instanceof ProgramSV
                    || sv instanceof VariableSV || sv instanceof SkolemTermSV
            ...
            final ImmutableSet<QuantifiableVariable> boundVarSet = boundAtOccurrenceSet(
    ...
    public TacletApp addInstantiation(SchemaVariable sv, Term term,
        ...
        else return createNoPosTacletApp(taclet(),
    ...
    public boolean complete() {
        return (uninstantiatedVars().isEmpty()
                && taclet() instanceof NoFindTaclet && ifInstsComplete());
     ...
     * numerous calls to the expensive pos.subTerm() while matching during a
     * recursive descent in a term (where the current subterm is known anyway).
     */
    public NoPosTacletApp matchFind(PosInOccurrence pos, Services services,
            Term t) {
        if ((t == null) && (pos != null))
        ...
        if (taclet() instanceof FindTaclet) {
            res = ((FindTaclet) taclet()).getMatcher().matchFind(t, mc,
                services);
    ...
    private NoPosTacletApp evalCheckRes(MatchConditions res,
        ...
        if (updateContextFixed && !updateContextCompatible(res)) {
            /*
             * Debug.out("NoPosTacletApp: Incompatible context",
