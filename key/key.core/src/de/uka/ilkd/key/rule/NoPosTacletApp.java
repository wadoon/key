...
public class NoPosTacletApp extends TacletApp {
	...
	createNoPosTacletApp(Taclet           taclet, 
		...
		return createNoPosTacletApp ( taclet,
				instantiations,
				null,
				costComputationTime, matchingTime,
				services);
	}
	...
	createNoPosTacletApp(Taclet                       taclet, 
		...
		if (checkVarCondNotFreeIn(taclet, inst)) {
			NoPosTacletApp app = new NoPosTacletApp(taclet,
					inst,					
					ifInstantiations);
	...
	public NoPosTacletApp matchFind(PosInOccurrence pos,
		...
		NoPosTacletApp result = evalCheckRes(res, services);
		if (result != null) { 
			result.setMatchingTime(System.nanoTime() - start + getMatchingTime());
		}
		return result;
	}
