...
/**
 * Instantiates "wholes" left by abstract execution.
 *
 ...
 */
public class InstantiateAbstractExecutionHoleRule implements BuiltInRule {
    /**
     * The name of the lazy SE taclet corresponding to loop treatment.
     */
    public static final String LAZY_LOOP_TACLET_NAME = "lazyLoop";
    public static final BuiltInRule INSTANCE = new InstantiateAbstractExecutionHoleRule();

    private final static Name NAME = new Name("Instantiate Loop Holes");

    private InstantiateAbstractExecutionHoleRule() {
    }
    ...
    public IBuiltInRuleApp createApp(PosInOccurrence pos,
        ...
        return new InstantiateAbstractExecutionHoleRuleApp(null);
    }
     ...
     */
    public static AbstractExecutionHole[] retrieveLoopHoles(Proof proof) {
        final Iterable<Node> nodes = () -> proof.root().subtreeIterator();
        final List<Node> abstractExecutionRuleNodes = //
                StreamSupport.stream(nodes.spliterator(), false)
                        .filter(node -> node.getAppliedRuleApp() != null) //
                        .filter(node -> node.getAppliedRuleApp()
                                .rule() instanceof Taclet)
                        .filter(
                            node -> ((Taclet) node.getAppliedRuleApp().rule())
                                    .getRuleSets().stream()
                                    .anyMatch(rs -> rs.name().toString()
                                            .equals("abstractExecution")))
                        .collect(Collectors.toList());

        final AbstractExecutionHole[] result = //
                new AbstractExecutionHole[abstractExecutionRuleNodes.size()];
        for (int i = 0; i < result.length; i++) {
            final Node node = abstractExecutionRuleNodes.get(i);
            final PosTacletApp posTacletApp = //
                    (PosTacletApp) node.getAppliedRuleApp();
            final AbstractPlaceholderStatement loop = (AbstractPlaceholderStatement) JavaTools
                    .getActiveStatement(TermBuilder
                            .goBelowUpdates(
                                posTacletApp.posInOccurrence().subTerm())
                            .javaBlock());
            ...
            result[i] = new AbstractExecutionHole(i + 1, pcPlaceholderName,
