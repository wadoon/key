...
public class BlockContractRule implements BuiltInRule {
    ...
    private static Term buildAfterVar(Term varTerm, String suffix,
            Services services) {
        if (varTerm == null) {
            return null;
        }
    ...
    private static ImmutableSet<BlockContract> getApplicableContracts(
    ...
    private static ImmutableSet<BlockContract> filterAppliedContracts(
    ...
    private static Term buildInfFlowPreAssumption(ProofObligationVars instVars,
            ImmutableList<Term> localOuts, ImmutableList<Term> localOutsAtPre,
            Term baseHeap, final TermBuilder tb) {
    ...
    private static Term buildInfFlowPostAssumption(ProofObligationVars instVars,
            ImmutableList<Term> localOuts, ImmutableList<Term> localOutsAtPost,
            Term baseHeap, Term applPredTerm, final TermBuilder tb) {
    ...
    private InfFlowValidityData setUpInfFlowValidityGoal(final Goal infFlowGoal,
            final BlockContract contract,
            final Map<LocationVariable, Function> anonymisationHeaps,
            final Services services, final Variables variables,
        ...
        final Term selfAtPost = hasSelf
                ? buildAfterVar(selfAtPre, "BLOCK", services) : tb.NULL();
        ...
        final Term resultAtPost = hasRes
                ? buildAfterVar(resultAtPre, "BLOCK", services) : tb.NULL();
        final Term exceptionAtPre = hasExc ? tb.var(variables.exception)
                : tb.NULL();
        final Term exceptionAtPost = hasExc
                ? buildAfterVar(exceptionAtPre, "BLOCK", services) : tb.NULL();
        ...
        InfFlowPOSnippetFactory infFlowFactory = POSnippetFactory
                .getInfFlowFactory(contract, ifVars.c1, ifVars.c2,
    ...
    private static boolean occursNotAtTopLevelInSuccedent(
            final PosInOccurrence occurrence) {
    ...
    private ImmutableList<Goal> applyJoinBlockContract(final Goal goal,
        ...
        if (!jB.isEmpty() && jB.program() instanceof StatementBlock) {
            ...
            StatementBlock newBody = getBlockWithJPS(body, instantiation.block,
    ...
    private StatementBlock getBlockWithJPS(StatementBlock block1,
        ...
        SourceElement sE = block1.getFirstElementIncludingBlocks();
        
        if (!sE.equals(block1) && sE instanceof StatementBlock) {
            if (sE.equals(block2)) {
                return insertJoinPoint(services, app, block1);
            }
            else {
                StatementBlock sB = getBlockWithJPS(
                        (StatementBlock) sE, block2, services,
                        app);
    ...
    private StatementBlock insertJoinPoint(final Services services,
        ...
        ProgramElement[] newContent = new ProgramElement[size + 2];

        newContent[0] = KeYJavaASTFactory.declare(
         progVar, KeYJavaASTFactory.falseLiteral());
        newContent[1] = block.getChildAt(0);
        newContent[2] = new JoinPointStatement(
                (BlockContract) application.getContract(), progVar);

        block.getBody().arraycopy(1, newContent, 3, size - 1);

        return KeYJavaASTFactory.block(new ExtList(newContent));
    }
    ...
    private ImmutableList<Goal> apply(final Goal goal, final Services services,
        ...
        // !application.getContract().hasModifiesClause();
        ...
        final Term anonymisationUpdate = updatesBuilder
                .buildAnonymisationUpdate(anonymisationHeaps,
    ...
    private Map<LocationVariable, Function> createAndRegisterAnonymisationVariables(
        ...
        for (LocationVariable variable : variables) {
            if (contract.hasModifiesClause(variable)) {
                final String anonymisationName = tb
                        .newName(ANONYMISATION_PREFIX + variable.name());
    ...
    @Override
    public BlockContractBuiltInRuleApp createApp(
    ...
    private static final class UpdatesBuilder extends TermBuilder {
        ...
        public Term buildAnonymisationUpdate(
                final Map<LocationVariable, Function> anonymisationHeaps,
                /*
        ...
        /* final Map<LocationVariable, Function> anonymisationLocalVariables, */) {
            Term result = skip();
            final Collection<LocationVariable> localOutVariables = variables.remembranceLocalVariables
                    .keySet();
            for (LocationVariable variable : localOutVariables) {
                final String anonymisationName = newName(
                        ANONYMISATION_PREFIX + variable.name());
                final Function anonymisationFunction = new Function(
                        new Name(anonymisationName), variable.sort(), true);
                services.getNamespaces().functions()
                        .addSafely(anonymisationFunction);
            ...
            return result;
            /*
             * Term result = skip(); for (Map.Entry<LocationVariable, Function>
             * anonymisationLocalVariable :
             * anonymisationLocalVariables.entrySet()) { result =
             * parallel(result, elementary(anonymisationLocalVariable.getKey(),
             * func(anonymisationLocalVariable.getValue()))); } return result;
             */
        }
    ...
    private static final class ConditionsAndClausesBuilder extends TermBuilder {
        ...
        public Term buildPrecondition() {
            ...
            for (LocationVariable heap : heaps) {
                result = and(result,
                        contract.getPrecondition(heap, getBaseHeap(),
    ...
    private final static class GoalsConfigurator {
        ...
        public void setUpWdGoal(final Goal goal, final BlockContract contract,
            ...
            final Term anon = anonHeap != null
                    ? services.getTermBuilder().func(anonHeap) : null;
    ...
    public static final class ValidityProgramConstructor {
        ...
        private void declareExceptionNull() {
            statements.add(KeYJavaASTFactory.declare(variables.exception,
                    NullLiteral.NULL, variables.exception.getKeYJavaType()));
        }
        ...
        private Statement wrapInTryCatch(final Statement labeldBlock,
            ...
            return new Try(new StatementBlock(labeldBlock),
                    new Branch[] { katch });
        }
    }
