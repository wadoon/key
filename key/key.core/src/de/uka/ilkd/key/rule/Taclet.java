 ...
 * indicates, if a term has to occur at the top level and if so, on which side
 * of the sequent. The goal descriptions consists of lists of add and
 * replacewith constructs. They describe, how to construct a new goal out of the
 * old one by adding or replacing parts of the sequent. Each of these lists
 * describe a new goal, whereas if no such list exists, means that the goal is
 * closed.
 * <p>
 ...
 * this taclet (see {@link de.uka.ilkd.key.rule.TacletApp TacletApp}. When this
 * attached taclet application object is applied, the new goals are constructed
 * as described by the goal descriptions. For example <br>
 * </br>
 * <code>
 *    find (A | B ==>) replacewith ( A ==> ); replacewith(B ==>)
 * </code> <br>
 * </br>
 * creates two new goals, where the first has been built by replacing
 * <code> A | B </code> with <code>A</code> and the second one by replacing
 * <code>A | B</code> with <code>B</code>. For a complete description of the
 * syntax and semantics of taclets consult the KeY-Manual. The objects of this
 * class serve different purposes: First they represent the syntactical
 * structure of a taclet, but they also include the taclet interpreter isself.
 * The taclet interpreter knows two modes: the match and the execution mode. The
 * match mode tries to find a a mapping from schemavariables to a given term or
 * formula. In the execution mode, a given goal is manipulated in the manner as
 * described by the goal descriptions.
 * </p>
 * <p>
 * But an object of this class neither copies or split the goal, nor it iterates
 * through a sequent looking where it can be applied, these tasks have to be
 * done in advance. For example by one of the following classes
...
public abstract class Taclet implements Rule, Named {
    ...
    /**
     * Variables that have to be created each time the taclet is applied. Those
     * variables occur in the varcond part of a taclet description.
     */
     ...
     * variables of a given formula(SV) Used by skolemization rules.
     */
    @Deprecated
    ...
    /**
     * This map contains (a, b) if there is a substitution {b a} somewhere in
     * this taclet
     */
    ...
    // In the long run, we should think about keeping those somewhere else,
    // e.g., in the services
    // such that we gain more flexibility like combined matchers that do not
    // just match one taclet but
     ...
     * @param attrs
     *            attributes for the Taclet; these are boolean values indicating
     *            a noninteractive or recursive use of the Taclet.
     */
    protected Taclet(Name name, TacletApplPart applPart,
            ImmutableList<TacletGoalTemplate> goalTemplates,
            ImmutableList<RuleSet> ruleSets, TacletAttributes attrs,
    ...
    protected Taclet(Name name, TacletApplPart applPart,
        ...
        this(name, applPart, goalTemplates, ruleSets, attrs, prefixMap, choices,
            false, tacletAnnotations);
    }

    /**
     * creates and initializes the taclet matching and execution engines has to
     * be called at the end of initialization
     */
     ...
     *
     * @return set of variables that occur bound in taclet entities others than
     *         goal templates
     */
    ...
    /**
     * looks if a variable is declared as new and returns its sort to match with
     * or the schema variable it shares the match-sort with. Returns null if the
     * SV is not declared to as new.
     *
     * @param var
    ...
    /**
     * returns the display name of the taclet, or, if not specified -- the
     * canonical name
     */
    @Override
    ...
    /**
     * returns an iterator over the variable pairs that indicate that are new in
     * the Taclet.
     */
    ...
    /**
     * returns the computed prefix for the given schemavariable. The prefix of a
     * schemavariable is used to determine if an instantiation is correct, in
     * more detail: it mainly contains all variables that can appear free in an
     * instantiation of the schemvariable sv (rewrite taclets have some special
     * handling, see paper of M. Giese and comment of method isContextInPrefix).
     *
     * @param sv
     *            the Schemavariable
    ...
    /**
     * returns true iff a context flag is set in one of the entries in the
     * prefix map. Is cached after having been called once. __OPTIMIZE__ is
     * caching really necessary here?
     *
     * @return true iff a context flag is set in one of the entries in the
     *         prefix map.
     */
     ...
     * <code>o</code> and <code>this</code> contain no mutually exclusive taclet
     * options.
     */
    @Override
    ...
    /**
     * Find a schema variable that could be used to choose a name for an
     * instantiation (a new variable or constant) of "p"
     *
     * @return a schema variable that is substituted by "p" somewhere in this
     *         taclet (that is, these schema variables occur as arguments of a
    ...
    public SchemaVariable getNameCorrespondent(SchemaVariable p,
        ...
        if (svNameCorrespondences == null) {
            final SVNameCorrespondenceCollector c = new SVNameCorrespondenceCollector(
    ...
    /**
     * @return true iff <code>this</code> taclet may be applied for the given
     *         mode (interactive/non-interactive, activated rule sets)
     */
    public boolean admissible(boolean interactive,
            ImmutableList<RuleSet> p_ruleSets) {
        if (interactive)
            return admissibleInteractive(p_ruleSets);
        else return admissibleAutomatic(p_ruleSets);
    }

    protected boolean admissibleInteractive(
            ImmutableList<RuleSet> notAdmissibleRuleSets) {
        return true;
    }

    protected boolean admissibleAutomatic(
    ...
    public static class TacletLabelHint {
        ...
        /**
         * The optional {@link Sequent} of the add or replace part of the
         * taclet.
         */
         ...
         *            The currently performed operation.
         * @param sequent
         *            The optional {@link Sequent} of the add or replace part of
         *            the taclet.
         */
        public TacletLabelHint(TacletOperation tacletOperation,
                Sequent sequent) {
         ...
         *
         * @param labelHint
         *            The previous {@link TacletLabelHint} which is now
         *            specialised.
        ...
        public TacletLabelHint(TacletLabelHint labelHint,
            ...
            assert !TacletOperation.REPLACE_TERM
                    .equals(labelHint.getTacletOperation());
        ...
        /**
         * Returns the optional {@link Sequent} of the add or replace part of
         * the taclet.
         *
         * @return The optional {@link Sequent} of the add or replace part of
         *         the taclet.
         */
        ...
        /**
         * Returns the optional {@link SequentFormula} contained in
         * {@link #getSequent()}.
         *
         * @return The optional {@link SequentFormula} contained in
         *         {@link #getSequent()}.
         */
         ...
         * {@link Term}.
         *
         * @return The stack maintained during application of a taclet
         *         {@link Term}.
         */
        ...
        @Override
        public String toString() {
            return tacletOperation + ", sequent = " + sequent
                    + ", sequent formula = " + sequentFormula + ", term = "
                    + term;
        }
        ...
        public static enum TacletOperation {
             ...
             * Available information are {@link TacletLabelHint#getSequent()}
             * and {@link TacletLabelHint#getSequentFormula()}.
             */
            ADD_ANTECEDENT,
            ...
            /**
             * Replace clause of a {@link Taclet} provides a {@link Sequent} and
             * currently additional adds to the antecedent are performed.
             * Available information are {@link TacletLabelHint#getSequent()}
             * and {@link TacletLabelHint#getSequentFormula()}.
             */
            ...
            /**
             * Replace clause of a {@link Taclet} provides a {@link Sequent} and
             * currently the current {@link PosInOccurrence} on the succedent is
             * modified. Available information are
             * {@link TacletLabelHint#getSequent()} and
            ...
            /**
             * Replace clause of a {@link Taclet} provides a {@link Sequent} and
             * currently the current {@link PosInOccurrence} on the antecedent
             * is modified. Available information are
             * {@link TacletLabelHint#getSequent()} and
             * {@link TacletLabelHint#getSequentFormula()}.
             */
            ...
            /**
             * Replace clause of a {@link Taclet} provides a {@link Sequent} and
             * currently additional adds to the succedent are performed.
             * Available information are {@link TacletLabelHint#getSequent()}
             * and {@link TacletLabelHint#getSequentFormula()}.
             */
            ...
            /**
             * Replace clause of a {@link Taclet} provides a {@link Term} which
             * is currently used to modify the {@link PosInOccurrence}.
     ...
     * @return List of the goals created by the rule which have to be proved. If
     *         this is a close-goal-taclet ( this.closeGoal () ), the first goal
     *         of the return list is the goal that should be closed (with the
     *         constraint this taclet is applied under).
     */
    @Override
