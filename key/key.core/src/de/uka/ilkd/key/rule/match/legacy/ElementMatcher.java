...
public abstract class ElementMatcher<T extends Operator> {
    ...
    static <T extends Operator> ElementMatcher<? super T> getElementMatcherFor(
            T o) {
        if (o instanceof SchemaVariable) {
            ...
            } else if (o instanceof SkolemUpdateSV) {
                return (ElementMatcher<? super T>) skolemUpdateSVMatcher;
    ...
    private final static SkolemUpdateSVMatcher skolemUpdateSVMatcher = new SkolemUpdateSVMatcher();
    ...
    private static abstract class AbstractSVMatcher<S extends AbstractSV>
         ...
         */
        protected MatchConditions addInstantiation(AbstractSV op,
                ProgramElement pe, MatchConditions matchCond,
                Services services) {

            final SVInstantiations instantiations = matchCond
                    .getInstantiations();
            ...
            if (inMap == null) {
                try {
                    return matchCond.setInstantiations(
                        instantiations.add(op, pe, services));
                } catch (IllegalInstantiationException e) {
                    Debug.out(
                        "Exception thrown by class Taclet at setInstantiations");
                }
            } else {
         ...
         * successful the resulting conditions are returned, otherwise null.
         * Failure is possible e.g. if this schemavariable has been already
         * matched to a term <tt>t2</tt> which is not unifiable with the given
         * term.
         */
        protected final MatchConditions addInstantiation(AbstractSV op,
                Term term, MatchConditions matchCond, Services services) {
            ...
            final Term t = inst.getTermInstantiation(op,
                inst.getExecutionContext(), services);
            if (t != null) {
            ...
            try {
                return matchCond
                        .setInstantiations(inst.add(op, term, services));
            } catch (IllegalInstantiationException e) {
                Debug.out("FAILED. Exception thrown at sorted schema variable",
                    e);
            }
    ...
    private static class ElementaryUpdateMatcher
            extends ElementMatcher<ElementaryUpdate> {

        @Override
        public MatchConditions match(ElementaryUpdate op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            if (op == subst) {
                return mc;
            } else if (!(subst instanceof ElementaryUpdate)) {
                Debug.out(
                    "FAILED. Incompatible operators " + "(template, operator)",
                    op, subst);
                return null;
            }

            final ElementaryUpdate eu = (ElementaryUpdate) subst;
            final MatchConditions result = ElementMatcher
                    .getElementMatcherFor(op.lhs())
                    .match(op.lhs(), eu.lhs(), mc, services);
            if (result == null) {
                Debug.out("FAILED. Lhs mismatch " + "(template, operator)", op,
                    eu);
            }
            return result;
        }
    ...
    private static class FormulaSVMatcher extends AbstractSVMatcher<FormulaSV> {
        ...
        @Override
        public MatchConditions match(FormulaSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
    ...
    private static class IdentityOperatorMatcher
            extends ElementMatcher<Operator> {

        /**
         * implements the default operator matching rule which means that the
         * compared object have to be equal otherwise matching fails
         */
        @Override
        public MatchConditions match(Operator op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            if (subst == op) {
                return mc;
            }
            Debug.out("FAILED. Operators are different(template, candidate)",
                op, subst);
            return null;
        }
    ...
    private static class LogicVariableMatcher
            extends ElementMatcher<LogicVariable> {
        /**
         * a match between two logic variables is possible if they have been
         * assigned they are same or have been assigned to the same abstract
         * name and the sorts are equal.
         */
        @Override
    ...
    private static class ModalOperatorSVMatcher
            extends AbstractSVMatcher<ModalOperatorSV> {

        @Override
        public MatchConditions match(ModalOperatorSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            ...
            if (op.getModalities().contains(m)) {
                Operator o = (Operator) mc.getInstantiations()
                        .getInstantiation(op);
                if (o == null) {
                    return mc.setInstantiations(
                        mc.getInstantiations().add(op, m, services));
                } else if (o != m) {
                    Debug.out(
                        "FAILED. Already instantiated with a different operator.");
                    return null;
                } else {
                    return mc;
                }
    ...
    private static class ProgramSVMatcher extends AbstractSVMatcher<ProgramSV> {
        ...
        @Override
        public MatchConditions match(ProgramSV op, SVSubstitute substitute,
                MatchConditions mc, Services services) {

            final ProgramSVSort svSort = (ProgramSVSort) op.sort();

            if (substitute instanceof Term
                    && svSort.canStandFor((Term) substitute)) {
                return addInstantiation(op, (Term) substitute, mc, services);
                    ...
                    && svSort.canStandFor((ProgramElement) substitute,
                        mc.getInstantiations().getExecutionContext(),
                        services)) {
                return addInstantiation(op, (ProgramElement) substitute, mc,
                    services);
            }
            Debug.out("FAILED. Cannot match ProgramSV with given "
                    + "instantiation(template, orig)",
                op, substitute);
            return null;
        }
    ...
    private static class SkolemTermSVMatcher
            extends AbstractSVMatcher<SkolemTermSV> {

        @Override
        public MatchConditions match(SkolemTermSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
    ...
    private static class SkolemUpdateSVMatcher
            extends AbstractSVMatcher<SkolemUpdateSV> {

        @Override
        public MatchConditions match(SkolemUpdateSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            if (subst.equals(mc.getInstantiations().getInstantiation(op))) {
                return mc;
            } else {
    ...
    private static class SortDependingFunctionMatcher
         ...
         *
         * @param s1
         *            Sort tried to matched (maybe concrete or (contain)
         *            generic)
         * @param s2
         *            concrete Sort
         * @param mc
         ...
         *         otherwise
         */
        private static MatchConditions matchSorts(Sort s1, Sort s2,
                MatchConditions mc, Services services) {
            // This restriction has been dropped for free generic sorts to prove
            // taclets correct
            ...
            if (!(s1 instanceof GenericSort)) {
                ...
                final GenericSortCondition c = GenericSortCondition
                        .createIdentityCondition(gs, s2);
                if (c == null) {
                    Debug.out("FAILED. Generic sort condition");
                    return null;
                } else {
                    try {
                        mc = mc.setInstantiations(
                            mc.getInstantiations().add(c, services));
         ...
         * <code>op</code>, the necessary conditions are returned or null if not
         * unifiable (matchable). A sortdepending function is matched
         * successfully against another sortdepending function if the sorts can
         * be matched and they are of same kind.
         */
        @Override
        public MatchConditions match(SortDependingFunction op,
                SVSubstitute subst, MatchConditions mc, Services services) {
            ...
            if (!op.isSimilar(sdp)) {
                Debug.out("FAILED. Sort depending symbols not similar.", op,
                    subst);
                return null;
            }

            final MatchConditions result = matchSorts(op.getSortDependingOn(),
                sdp.getSortDependingOn(), mc, services);
            if (result == null) {
    ...
    private static class TermLabelSVMatcher
            extends AbstractSVMatcher<TermLabelSV> {
        ...
        public MatchConditions match(TermLabelSV op, SVSubstitute subst,
            ...
            /*
             * if (!t.hasLabels()) { statements about the non-existence return
             * null; of term labels should also be } possible.
             */
            final SVInstantiations svInsts = mc.getInstantiations();
            final TermLabelInstantiationEntry inst = (TermLabelInstantiationEntry) svInsts
                    .getInstantiation(op);
            if (inst != null) {
            ...
            return mc.setInstantiations(
                svInsts.add(op, t.getLabels(), services));
        }
    }

    private static class TermSVMatcher extends AbstractSVMatcher<TermSV> {
        @Override
        public MatchConditions match(TermSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            if (subst instanceof Term) {
                return addInstantiation(op, (Term) subst, mc, services);
            }
    ...
    private static class UpdateSVMatcher extends AbstractSVMatcher<UpdateSV> {
        ...
        @Override
        public MatchConditions match(UpdateSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            if (subst instanceof Term) {
                return addInstantiation(op, (Term) subst, mc, services);
            }
    ...
    private static class VariableSVMatcher
            extends AbstractSVMatcher<VariableSV> {

        @Override
        public MatchConditions match(VariableSV op, SVSubstitute subst,
                MatchConditions mc, Services services) {
            final Term substTerm;
            if (subst instanceof LogicVariable) {
                substTerm = services.getTermBuilder()
                        .var((LogicVariable) subst);
            ...
            if (foundMapping == null) {
                ...
                Debug.out(
                    "FAILED. Already instantiated with different variable.");
                return null;
            }
        }
    ...
    public abstract MatchConditions match(T op, SVSubstitute subst,
            MatchConditions mc, Services services);
