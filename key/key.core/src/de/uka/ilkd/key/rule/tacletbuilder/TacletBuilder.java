...
public abstract class TacletBuilder<T extends Taclet> {
    ...
    protected ImmutableSet<TacletAnnotation> tacletAnnotations = DefaultImmutableSet
            .<TacletAnnotation> nil();
     ...
     * <code>soc</code>
     */
    public void addGoal2ChoicesMapping(TacletGoalTemplate gt,
     ...
     * v is new and has the same type as peerSV
     */
    ...
    public void addVarsNew(NewVarcond nv) {
        if (!(nv.getSchemaVariable() instanceof ProgramSV)) {
            throw new TacletBuilderException(this,
                "Tried to add condition:" + nv + "to new vars-list. That can"
                        + "match more than program" + " variables.");
        }
     ...
     * flags imply that the flags are not set. No specified find part for
     * Taclets that require a find part causes an IllegalStateException.
     */
    public abstract T getTaclet();

    public T getTacletWithoutInactiveGoalTemplates(
            ImmutableSet<Choice> active) {
    ...
    public static class TacletBuilderException
        ...
        public TacletBuilderException(TacletBuilder<?> tb,
                String errorMessage) {
