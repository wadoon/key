...
public class PositionTable {
     ...
     * Sequent, etc.)
     *
     * @param rows the number of direct sub-elements in the term whose position
     ...
     * @param length the length of the whole string corresponding to this position
    ...
    public Range rangeForPath(ImmutableList<Integer> path, int length) {
        if (path.isEmpty()) {
            ...
            Range r = child[sub]
                    .rangeForPath(path.tail(), endPos[sub] - startPos[sub]);
            r.start += startPos[sub];
            r.end += startPos[sub];
            return r;
        }
    ...
    public PositionTable[] getChildren() {
        return child;
    }
    ...
    public String toString() {
        String result = "[";
        for (int i = 0; i < rows; i++) {
            result = result + "<" + startPos[i] + "," + endPos[i] + ","
                    + child[i] + ">";
            if (rows - 1 != i)
                result = result + ",";
        }
     ...
     * @param filter  the sequent print filter from that was used to print the
     *                sequent
     */
    ...
    protected PosInSequent getSequentPIS(ImmutableList<Integer> posList,
        ...
        PosInOccurrence currentPos = new PosInOccurrence(cfma,
     ...
     *
     * @param filterEntry the print filter entry that contains information about which
    ...
    private PosInSequent getTermPIS(SequentPrintFilterEntry filterEntry,
        ...
        if (posList.isEmpty()) {
            ...
            return child[subNo].getTermPIS(filterEntry, posList.tail(), subpio);
        }
    }

    private static SequentPrintFilterEntry getFilterEntry(int cfmaNo,
            SequentPrintFilter filter) {
        int i = cfmaNo;
        ImmutableList<SequentPrintFilterEntry> list = filter.getAntec()
                .append(filter.getSucc());
        while (i-- != 0)
            list = list.tail();
        return list.head();
    }
