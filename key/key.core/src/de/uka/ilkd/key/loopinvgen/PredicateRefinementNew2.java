...
public class PredicateRefinementNew2 {
	...
	public PredicateRefinementNew2(Services s, Sequent sequent, Set<Term> allPredList, Term i, int itr) {
		services = s;
...
//		System.out.println("in: " +  sideSeq);
		...
		if (provable && pred.op() == services.getTypeConverter().getDependenciesLDT().getNoWaR()) {//
			System.out.println("Proved: " + ProofSaver.printAnything(sideSeq, services));
		}
...
//		}
	...
	private Set<Term> weakeningDependencePredicates(Term unProven) {
		...
		Set<Term> tempResult = new HashSet<>();
		boolean b = false;
		tempResult.addAll(weakenByPredicateSymbol(unProven));
		tempResult.addAll(weakenByIndex(unProven));
//		tempResult.addAll(weakenBySequent(unProven));
		for (Term r : tempResult) {
			if (sequentImpliesPredicate(r)) {
				result.add(r);
			}
		}
		if (tempResult.isEmpty() && itrNumber < 2) {
			tempResult.addAll(weakenBySubSet(unProven));
			b =true;
		}
		if(b) {
			for (Term r : tempResult) {
