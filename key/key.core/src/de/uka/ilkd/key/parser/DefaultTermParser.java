...
public final class DefaultTermParser {
    ...
    /**
     * The method reads the input and parses a term with the specified
     * namespaces. The method ensures, that the term has the specified sort.
     *
     * @param sort
     *            The expected sort of the term.
     * @return The parsed term of the specified sort.
     * @throws ParserException
     *             The method throws a ParserException, if the input could not
     *             be parsed correctly or the term has an invalid sort.
     */
    public Term parse(Reader in, Sort sort, Services services,
            Namespace<QuantifiableVariable> var_ns, Namespace<Function> func_ns,
            Namespace<Sort> sort_ns, Namespace<IProgramVariable> progVar_ns,
            AbbrevMap scm) throws ParserException {
        return parse(in, sort, services,
            new NamespaceSet(var_ns, func_ns, sort_ns, new Namespace<RuleSet>(),
                new Namespace<Choice>(), progVar_ns, new Namespace<>()),
            scm);
    }
     ...
     *             be parsed correctly or the term has an invalid sort.
     */
    public Term parse(Reader in, Sort sort, Services services, NamespaceSet nss,
            AbbrevMap scm) throws ParserException {
        KeYParserF parser = null;
        try {
            parser = new KeYParserF(ParserMode.TERM, new KeYLexerF(in, ""),
                new Recoder2KeY(services, nss), services, nss, scm);

            final Term result = parser.termEOF();
            if (sort != null && !result.sort().extendsTrans(sort))
                throw new ParserException("Expected sort " + sort
                        + ", but parser returns sort " + result.sort() + ".",
                    null);
            return result;
    ...
    /**
     * The method reads the input and parses a sequent with the specified
     * namespaces.
     *
     ...
     *             be parsed correctly
     */
    public Sequent parseSeq(Reader in, Services services, NamespaceSet nss,
            AbbrevMap scm) throws ParserException {
        KeYParserF p = null;
        try {
            p = new KeYParserF(ParserMode.TERM, new KeYLexerF(in, ""),
                new Recoder2KeY(services, nss), services, nss, scm);
