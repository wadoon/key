...
/**
 * the class manages the available TacletApps. This index has to be used if one
 * wants to ask for the available taclet application at a specific position (or
 * the whole sequent if taclet is a nofind rule). This means all taclet
 * applications that have a position information are managed by this index. For
 * all others the class TacletIndex is used. This index uses also the
 * TacletIndex to calculate new TacletApps.
 */
...
public class TacletAppIndex {
    ...
    /**
     * The sequent with the formulas for which taclet indices are hold by this
     * object. Invariant: <code>seq != null</code> implies that the indices
     * <code>antecIndex</code>, <code>succIndex</code> are up to date for the
     * sequent <code>seq</code>
     */
    private Sequent seq;
    ...
    private TacletAppIndex(TacletIndex tacletIndex,
            ...
            SemisequentTacletAppIndex succIndex, Goal goal, Sequent seq,
    ...
    TacletAppIndex copyWithTacletIndex(TacletIndex p_tacletIndex) {
        return new TacletAppIndex(p_tacletIndex, antecIndex, succIndex,
                getGoal(), getSequent(), ruleFilter, indexCaches, cache);
    }
    ...
    /**
     * Forces all delayed computations to be performed, so that the cache is
     * fully up-to-date (NewRuleListener gets informed)
     */
    ...
    private void createAllFromGoal() {
        ...
        antecIndex = new SemisequentTacletAppIndex(getSequent(), true,
                getServices(), tacletIndex(), getNewRulePropagator(),
     ...
     *         sequent of the associated goal; this does not detect other
     *         modifications like an altered user constraint
     */
    ...
    /**
     * returns the set of rule applications at the given position of the given
     * sequent.
     *
     * @param pos
    ...
    public ImmutableList<TacletApp> getTacletAppAt(PosInOccurrence pos,
            TacletFilter filter, Services services) {
        ImmutableList<TacletApp> sal = getFindTacletWithPos(pos, filter,
                services);
     ...
     *
     * @param tacletInsts
     *            the list of NoPosTacletApps the TacletApps are to be created
     *            from
     * @param pos
     *            the PosInOccurrence to focus
     * @return list of all created TacletApps
     */
    static ImmutableList<TacletApp> createTacletApps(
            ImmutableList<NoPosTacletApp> tacletInsts, PosInOccurrence pos,
            Services services) {
     ...
     * constrainedFormula described by a PosInOccurrence. RewriteTaclets with
     * wrong prefix are filtered out.
     *
     * @param pos
     *            the PosInOccurrence to focus
     * @param services
    ...
    public ImmutableList<NoPosTacletApp> getRewriteTaclet(PosInOccurrence pos,
            TacletFilter filter, TermServices services) {

        final Iterator<NoPosTacletApp> it = getFindTaclet(pos, filter, services)
                .iterator();

        ImmutableList<NoPosTacletApp> result = ImmutableSLList
                .<NoPosTacletApp> nil();

        while (it.hasNext()) {
            final NoPosTacletApp tacletApp = it.next();
            final Taclet t = tacletApp.taclet();
            if (t instanceof RewriteTaclet && ((RewriteTaclet) t)
                    .checkPrefix(pos, MatchConditions.EMPTY_MATCHCONDITIONS
                            .setSeqFor(pos)) != null)
                result = result.prepend(tacletApp);
        }
    ...
    public ImmutableList<TacletApp> getTacletAppAtAndBelow(PosInOccurrence pos,
            TacletFilter filter, Services services) {
        final ImmutableList<TacletApp> findTaclets = getIndex(pos)
                .getTacletAppAtAndBelow(pos, filter, services);
    ...
    private void updateIndices(SequentChangeInfo sci) {
        ...
        succIndex = succIndex.sequentChanged(sci, getServices(), tacletIndex(),
    ...
    public void addedNoPosTacletApps(Iterable<NoPosTacletApp> tacletApps) {
        for (TacletApp tacletApp : tacletApps) {
            if (indexCaches.isRelevantTaclet(tacletApp.taclet())) {
                // we must flush the index cache, and we must no longer use a
                // cache
                // that we share with other instances of
                // <code>TacletAppIndex</code>
                // (that maybe live of different goals)
                createNewIndexCache();
                break;
            }
    ...
    /**
     * updates the internal caches after a Taclet with instantiation information
     * has been removed from the TacletIndex.
     *
     * @param tacletApp
    ...
    /**
     * Reports all cached rule apps. Calls ruleAdded on the given
     * NewRuleListener for every cached taclet app.
     */
