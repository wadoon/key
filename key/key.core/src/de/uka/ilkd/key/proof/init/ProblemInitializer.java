...
public final class ProblemInitializer {
    ...
    public InitConfig prepare(EnvInput envInput) throws ProofInputException {
        synchronized (SchemaJavaParser.class) {
            // The synchronized statement is required for thread save parsing
            // since all JavaCC parser are generated static. For our own parser
            // (ProofJavaParser.jj and SchemaJavaParser.jj) it is possible to
            // generate them non static which is done on branch
            // "hentschelJavaCCInstanceNotStatic". But recoder still uses
            // static methods and the synchronized statement can not be avoided
            // for this reason.
            InitConfig currentBaseConfig = baseConfig;
            // It is required to work with a copy to make this method thread
            // save required by the Eclipse plug-ins.
            ...
            if (currentBaseConfig == null || profile != currentBaseConfig.getProfile()) {
                ...
                if (tacletBase != null) {
                    KeYFile tacletBaseFile =
                            new KeYFile("taclet base", tacletBase,
                                        progMon, profile);
