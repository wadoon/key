...
public final class SimpleBlockContract implements BlockContract {
    ...
    public SimpleBlockContract(final StatementBlock block,
            ...
            final JoinProcedure joinProcedure, final String mergeParams,
    ...
    public Terms getVariablesAsTerms(Services services) {
        Term selfTerm = (this.variables.self != null
                ? services.getTermBuilder().var(this.variables.self) : null);
    ...
    public Term getPrecondition(final LocationVariable heap,
        ...
        final Map<ProgramVariable, ProgramVariable> replacementMap = createReplacementMap(
                new Variables(self, null, null, null, null, null,
                        remembranceHeaps, null, services),
                services);
    ...
    public Term getPrecondition(final LocationVariable heapVariable,
        ...
        final Map<Term, Term> replacementMap = createReplacementMap(heap,
                new Terms(self, null, null, null, null, null, remembranceHeaps,
                        null),
                services);
        final OpReplacer replacer = new OpReplacer(replacementMap,
                services.getTermFactory());
    ...
    public Term getPrecondition(final LocationVariable heap,
            final Services services) {
        return getPrecondition(heap, variables.self, variables.remembranceHeaps,
                services);
    }
    ...
    public Term getModifiesClause(final LocationVariable heapVariable,
        ...
        assert services != null;
        final Map<Term, Term> replacementMap = createReplacementMap(heap,
                new Terms(self, null, null, null, null, null, null, null),
                services);
        final OpReplacer replacer = new OpReplacer(replacementMap,
                services.getTermFactory());
    ...
    public String getPlainText(final Services services, Terms terms) {
        ...
        for (LocationVariable heap : heapLDT.getAllHeaps()) {
            Term modifiesClause = getModifiesClause(heap,
                    services.getTermBuilder().var(heap), terms.self, services);
        ...
        String pres = "";
        for (LocationVariable heap : heapLDT.getAllHeaps()) {
            Term precondition = getPrecondition(heap, baseHeapTerm, terms.self,
        ...
        String posts = "";
        for (LocationVariable heap : heapLDT.getAllHeaps()) {
            Term postcondition = getPostcondition(heap, baseHeapTerm, terms,
                    services);
    ...
    public BlockContract update(final StatementBlock newBlock,
            ...
            final JoinProcedure newJoinProcedure, final String newMergeParams,
        ...
        return new SimpleBlockContract(newBlock, labels, method, modality,
                ...
                newinfFlowSpecs, newJoinProcedure, newMergeParams, newVariables,
    ...
    public BlockContract setBlock(StatementBlock newBlock) {
        return update(newBlock, preconditions, postconditions, modifiesClauses,
                infFlowSpecs, joinProcedure, mergeParams, variables);
    }
    ...
    public BlockContract setTarget(KeYJavaType newKJT,
        ...
        return new SimpleBlockContract(block, labels, (IProgramMethod) newPM,
                modality, preconditions, postconditions, modifiesClauses,
                infFlowSpecs, joinProcedure, mergeParams, variables,
                transactionApplicable, hasMod);
    }
    ...
    public OriginalVariables getOrigVars() {
        ...
        for (LocationVariable h : variables.remembranceLocalVariables
                .keySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (joinProcedure == null) {
            ...
            return false;
        if (mergeParams == null) {
            if (other.mergeParams != null)
                return false;
        }
        else if (!mergeParams.equals(other.mergeParams))
            return false;
        if (labels == null) {
    ...
    public String toString() {
        return "SimpleBlockContract [block=" + block + ", labels=" + labels
                ...
                + joinProcedure + " with parameters= " + mergeParams +  ", variables=" + variables
                + ", transactionApplicable=" + transactionApplicable
                + ", hasMod=" + hasMod + "]";
    }

    private Map<ProgramVariable, ProgramVariable> createReplacementMap(
    ...
    public static final class Creator extends TermBuilder {
        ...
        private final String mergeParams;
        ...
        public Creator(final StatementBlock block, final List<Label> labels,
                final IProgramMethod method, final Behavior behavior,
        ...
        private Term buildPostcondition(final LocationVariable heap) {
            ...
            // TODO Why do we handle the two cases differently? Surely has
        ...
        private ImmutableSet<BlockContract> create(
                final Map<LocationVariable, Term> preconditions,
                final Map<LocationVariable, Term> postconditions,
                final Map<LocationVariable, Term> modifiesClauses,
                final ImmutableList<InfFlowSpec> infFlowSpecs,
            ...
            if (ifDivergesConditionCannotBeExpressedByAModality()) {
                result = result.add(new SimpleBlockContract(block, labels,
                        ...
                        joinProcedure, mergeParams, variables,
                        transactionApplicable, hasMod));
            }
            return result;
        }
    ...
    private static final class Combinator extends TermBuilder {
        ...
        public Combinator(final ImmutableSet<BlockContract> contracts,
                final Services services) {
            super(services.getTermFactory(), services);
        ...
        private BlockContract[] sort(
                final ImmutableSet<BlockContract> contracts) {
            // sort contracts alphabetically (for determinism)
            BlockContract[] result = contracts
                    .toArray(new BlockContract[contracts.size()]);
            Arrays.sort(result, new Comparator<BlockContract>() {
                public int compare(BlockContract firstContract,
                        BlockContract secondContract) {
        ...
        private BlockContract combine() {
            ...
            return new SimpleBlockContract(head.getBlock(), head.getLabels(),
                    ...
                    head.getJoinProcedure(), head.getMergeParams(),
        ...
        private Term addPreconditionFrom(final BlockContract contract,
        ...
        private void addPostconditionFrom(final Term precondition,
                final BlockContract contract, final LocationVariable heap) {
            ...
            if (unconditionalPostcondition != null) {
                final Term conditionalPostcondition = imp(preify(precondition),
