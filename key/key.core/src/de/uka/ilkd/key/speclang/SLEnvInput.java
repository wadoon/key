...
public final class SLEnvInput extends AbstractEnvInput {
    ...
    public SLEnvInput(String javaPath, List<File> classPath, File bootClassPath,
            Profile profile, List<File> includes) {
        super(getLanguage() + " specifications", javaPath, classPath,
        ...
        for (KeYJavaType kjt : allKJTs) {
            if (kjt.getJavaType() instanceof TypeDeclaration
                ...
                if (file.isFile()) {
                    ...
                    URL url = KeYResourceManager.getManager()
                            .getResourceFile(Recoder2KeY.class, filePath);
                    if (url != null) {
                ...
                if (rs != null) {
                    final KeYFile keyFile =
                            new KeYFile(path, rs, null, getProfile());
    ...
    /**
     * For all library classes C, look for file C.key in same directory; if
     * found, read specifications from this file.
     */
    private ImmutableSet<PositionedString> createDLLibrarySpecs()
        ...
        if (bootClassPath != null) {
            ...
            if (!initConfig.getProfile().getInternalClassDirectory()
                    .isEmpty()) {
                path += "/"
                        + initConfig.getProfile().getInternalClassDirectory();
            }
            warnings =
                    warnings.union(createDLLibrarySpecsHelper(allKJTs, path));
        }
    ...
    private void addLoopInvariants(SpecExtractor specExtractor,
            final SpecificationRepository specRepos, final KeYJavaType kjt,
            final IProgramMethod pm) throws ProofInputException {
        // loop invariants
        ...
        for (ProgramElement loop : collector.getNodes()) {
            LoopSpecification inv = specExtractor.extractLoopInvariant(pm,
                    (LoopStatement) loop);
            if (inv != null) {
    ...
    private void addBlockAndLoopContracts(SpecExtractor specExtractor,
        ...
        final JavaASTCollector blockCollector =
                ...
                // , AbstractPlaceholderStatement.class
                );
        blockCollector.start();
        for (ProgramElement block : blockCollector.getNodes()) {
            final ImmutableSet<BlockContract> blockContracts = specExtractor
                    .extractBlockContracts(pm, (StatementBlock) block);

            for (BlockContract specification : blockContracts) {
                specRepos.addBlockContract(specification, true);
            }

            final ImmutableSet<LoopContract> loopContracts = specExtractor
                    .extractLoopContracts(pm, (StatementBlock) block);
    ...
    private void addMergePointStatements(SpecExtractor specExtractor,
        ...
        for (ProgramElement mps : mpsCollector.getNodes()) {
            ...
            mergeContracts.forEach(mc -> specRepos.addMergeContract(mc));
        }
    }
    ...
    private void addLabeledBlockContracts(SpecExtractor specExtractor,
        ...
        for (ProgramElement labeled : labeledCollector.getNodes()) {
            final ImmutableSet<BlockContract> blockContracts = specExtractor
                    .extractBlockContracts(pm, (LabeledStatement) labeled);
            for (BlockContract specification : blockContracts) {
                specRepos.addBlockContract(specification, true);
            }
        }
    ...
    private ImmutableSet<PositionedString> createSpecs(
            SpecExtractor specExtractor) throws ProofInputException {
        ...
        final KeYJavaType[] kjts = sortKJTs(allKeYJavaTypes
                .toArray(new KeYJavaType[allKeYJavaTypes.size()]));
        ...
        for (KeYJavaType kjt : kjts) {
            ...
            // Later, we will only add static invariants to contracts per
            // default if
            // there is an explicit static invariant present.
            boolean staticInvPresent = false;
            for (SpecificationElement s : classSpecs) {
                if (s instanceof ClassInvariant
                        && ((ClassInvariant) s).isStatic()) {
            ...
            for (IProgramMethod pm : pms) {
                ...
                addMergePointStatements(specExtractor, specRepos, pm,
                        methodSpecs);
