...
public final class QueryAxiom extends ClassAxiom {
    ...
    public ImmutableSet<Taclet> getTaclets(
        ...
        for (int i = 0; i < paramSVs.length; i++) {
            ...
            final KeYJavaType kjt = new KeYJavaType(
                target.getParamType(i).getJavaType(),
                target.getParameterType(i).getSort());
            tacletBuilder.addVarsNew(paramProgSVs[i], kjt);
        }
        final KeYJavaType kjt = new KeYJavaType(
            target.getReturnType().getJavaType(),
            target.getReturnType().getSort());
        tacletBuilder.addVarsNew(resultProgSV, kjt);
