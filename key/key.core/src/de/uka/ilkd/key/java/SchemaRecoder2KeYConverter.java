...
public class SchemaRecoder2KeYConverter extends Recoder2KeYConverter {
    ...
    public ProgramTransformer convert(
        ...
        } else if ("#foreach".equals(mcName)) {
            final ProgramSV[] svs = mc.getSV();
            assert svs != null;
            assert svs.length > 0;
            assert svs.length % 2 == 0;
            final ProgramSV[] indexVariables = new ProgramSV[svs.length / 2];
            System.arraycopy(svs, 0, indexVariables, 0, indexVariables.length);
            final ProgramSV[] listVariables = new ProgramSV[svs.length / 2];
            System.arraycopy(svs, indexVariables.length, listVariables, 0,
    ...
    protected VariableSpecification convertVarSpecWithSVType(
        ...
        if (varspec == null) {
            ...
            varspec = new VariableSpecification((IProgramVariable) pv,
                recoderVarspec.getDimensions(), (Expression) init, typeSVType,
                null);
    ...
    public TypeReference convert(recoder.java.reference.TypeReference tr) {
        ...
        PackageReference packref = result != null
                ? (PackageReference) convert(result)
                : null;
