            ...
            try {
                if (currNodeInterm instanceof BranchNodeIntermediate) {
                    ...
                    if (GeneralSettings.slicing) {
                        ...
                        LOGGER.trace("slicing @ {} [{}] {} (apply = {}, line = {}, original app = {}, override = {})",
        ...
        if (currFormula != 0) { // otherwise we have no pos
            try {
                if (!GeneralSettings.slicing) {
                    ...
                    if (!done) {
                        LOGGER.error("failed to find new formula @ rule name {}, step idx {}, added by {}, required formula {}",
                                tacletName,
                                stepIndex,
                                GeneralSettings.stepIndexToDynamicRule.getOrDefault(stepIndex, -1),
                                oldFormula);
                    }
                }
