...
public class PrepareObjectBuilder extends JavaTransformer {
    ...
    private final HashMap<TypeDeclaration<?>, NodeList<Statement>> class2fields;
    ...
    public PrepareObjectBuilder(TransformationPipelineServices services) {
        super(services);
        class2fields = new LinkedHashMap<>();
    }
    ...
    public ProblemReporter analyze() {
        ...
        for (final TypeDeclaration<?> cd : cache.classDeclarations()) {
                ...
                for (VariableDeclarator variable : field.getVariables()) {
                    ...
                    if (!fieldId.getIdentifier().startsWith("<")) {
                        result.add(assign((attribute(new ThisExpr(), fieldId.getIdentifier())),
                                services.getDefaultValue(services.getCrossReferenceSourceInfo().getType(field))));
                    }
                }
            }
        }
    ...
    public MethodDeclaration createMethod(TypeDeclaration type) {
        ...
        md.setBody(createPrepareBody(new ThisExpr(), type));
        return md;
    }
