...
public class ScriptAwareAutoMacro extends StrategyProofMacro {
    ...
    private Map<Goal, ProofCommandStatement> detectedCommands = new IdentityHashMap<Goal, ProofCommandStatement>();
    ...
    public Map<Goal, ProofCommandStatement> getDetectedCommands() {
    ...
    private class ScriptAwareStrategy implements Strategy {
        ...
        public boolean isApprovedApp(RuleApp app, PosInOccurrence pio, Goal goal) {
            if (detectedCommands.containsKey(goal)) {
                // we had found a command earlier.
                return false;
            }
            if (app.rule() instanceof ProofCommandStatementRule) {
                detectedCommands.put(goal, ProofCommandStatementRule.getCommand(pio));
            }
