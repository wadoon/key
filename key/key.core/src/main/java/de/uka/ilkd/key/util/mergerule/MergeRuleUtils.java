...
public class MergeRuleUtils {
    ...
    public static ImmutableSet<LocationVariable> getUpdateLeftSideLocations(
        ...
        return getElementaryUpdates (u, tb).stream()
                    .map(elem -> ((ElementaryUpdate) elem.op()).lhs()).map(LocationVariable.class::cast)
    ...
    public static Term getUpdateRightSideFor(Term update,
        ...
        return elems.stream()
                .filter(elem -> ((ElementaryUpdate) elem.op()).lhs().equals(leftSide))
    ...
    private static boolean isProvable(Sequent toProve, Services services,
            boolean doSplit, int timeout) { ApplyStrategyInfo proofResult = tryToProve(toProve, services, doSplit,
     ...
     * @param globals
     *            The global variables for the given branch.
