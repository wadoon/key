...
public class ProofJavaProgramFactory extends JavaProgramFactory {
    ...
    public Expression createMethodReferenceOrEscapeFunction(
            ReferencePrefix referencePrefix,
            Identifier identifier, ASTList<Expression> args, ASTList<TypeArgumentDeclaration> typeArguments) {
        String name = identifier.getText();
        Expression result = null;
        switch (name) {
            case "\\singleton":
                result = new Singleton(args.get(0));
                break;
            case "\\set_union":
                result = new SetUnion(args.get(0), args.get(1));
                break;

            case "\\intersect":
                result = new Intersect(args.get(0), args.get(1));
                break;
            case "\\set_minus":
                result = new SetMinus(args.get(0), args.get(1));
                break;
            case "\\all_fields":
                result = new AllFields(args.get(0));
                break;
            case "\\all_objects":
                result = new AllObjects(args.get(0));
                break;
            case "\\seq_singleton":
                result = new SeqSingleton(args.get(0));
                break;
            case "\\seq_concat":
                result = new SeqConcat(args.get(0), args.get(1));
                break;
            case "\\seq_sub":
                result = new SeqSub(args.get(0), args.get(1), args.get(2));
                break;
            case "\\seq_reverse":
                result = new SeqReverse(args.get(0));
                break;
            case "\\seq_put":
                // desugaring
                final Expression one = createIntLiteral(1);
                final Expression first = new SeqSub(args.get(0), createIntLiteral(0), createMinus(args.get(1), one));
                final Expression second = new SeqSingleton(args.get(2));
                final Expression third = new SeqSub(args.get(0), createPlus(args.get(1), one), createMinus(new SeqLength(args.get(0)), one));
                result = new SeqConcat(first, new SeqConcat(second, third));
                break;
            case "\\indexOf":
                result = new SeqIndexOf(args.get(0), args.get(1));
                break;
            case "\\seq_length":
                result = new SeqLength(args.get(0));
                break;
            case "\\seq_get":
                result = new SeqGet(args.get(0), args.get(1));
                break;
            default:
                if (name.startsWith("\\")) {
                    result = EscapeExpression.getEscapeExpression(name, args);
                } else {
                    result = createMethodReference(referencePrefix, identifier, args, typeArguments);
                }
        }
        return result;
    }

    public Expression handleEscapedIdentifier(UncollatedReferenceQualifier name) {
        switch (name.getName()) {
            case "\\empty": return EmptySetLiteral.INSTANCE;
            case "\\seq_empty": return EmptySeqLiteral.INSTANCE;
            case "\\map_empty": return EmptyMapLiteral.INSTANCE;
            default:
                return name;
        }
    }
