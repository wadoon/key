...
public class SVInstantiations {
     ...
     *
     * @param map the ImmMap<SchemaVariable,InstantiationEntry<?>> with the
     *            instantiations
     */
    ...
    public SVInstantiations addList(SchemaVariable sv,
        ...
        return add(sv,
                new ListInstantiation(
                        sv,
                        ImmutableSLList.nil().prepend(list)),
                services);
    }
    ...
    public SVInstantiations addInterestingList(SchemaVariable sv,
        ...
        return addInteresting(
                sv,
                new ListInstantiation(sv, ImmutableSLList.nil().prepend(list)),
                services);
    }
    ...
    private SVInstantiations checkSorts(SchemaVariable sv,
                                        InstantiationEntry<?> entry,
                                        boolean forceRebuild,
                                        Services services) {
        Boolean b = getGenericSortInstantiations().checkSorts(sv, entry);
    ...
    private SVInstantiations checkCondition(GenericSortCondition c,
                                            boolean forceRebuild,
                                            Services services) {
        Boolean b = getGenericSortInstantiations().checkCondition(c);

        if (b == null)
            return rebuildSorts(services);
        else if (!b)
            throw new SortException("Conditions for sorts" + " cannot be satisfied");
        if (forceRebuild)
            return rebuildSorts(services);
        return this;
    }
     ...
     *
     * @param sv  the SchemaVariable to be instantiated
     * @param pes the ArrayOf<t> the SchemaVariable is instantiated
     *            with
     */
    public SVInstantiations replace(SchemaVariable sv,
     ...
     *
     * @param prefix                 the PosInProgram describing the position of the first
     *                               statement after the prefix
     * @param postfix                the PosInProgram describing the position of the statement just
     *                               before the postfix
     * @param activeStatementContext the ExecutionContext of the first active statement
     * @param pe                     the ProgramElement the context positions are related to
     */
    ...
    public Term getTermInstantiation(SchemaVariable sv,
        ...
        if (inst == null) {
            ...
            throw new SortException("Instantiation of SV " + "cannot be converted to logic");
        }
    }
    ...
    public SVInstantiations addUpdate(Term update,
        ...
        if (update.sort() != Sort.UPDATE) throw new IllegalArgumentException();
        ...
        public boolean equals(Object obj) {
            if (obj instanceof UpdateLabelPair) {
                return update.equals(((UpdateLabelPair) obj).getUpdate()) &&
                        updateApplicationlabels.equals(
                                ((UpdateLabelPair) obj).getUpdateApplicationlabels());
            } else {
                return false;
            }
        }
    ...
    public SVInstantiations clearUpdateContext() {
        ...
        return new SVInstantiations(map, interesting(),
                ImmutableSLList.nil(), getGenericSortInstantiations(),
                getGenericSortConditions());
    }
    ...
    public boolean isEmpty() {
        ...
        return this == EMPTY_SVINSTANTIATIONS ||
                (map.isEmpty() && updateContext.isEmpty()
    ...
    public String toString() {
        return "SV Instantiations: " + map;
    }

    /**
     * Add the given additional condition for the generic sort instantiations
     */
    public SVInstantiations add(GenericSortCondition c, Services services) throws SortException {
        return new SVInstantiations(map, interesting(), getUpdateContext(),
                getGenericSortInstantiations(), getGenericSortConditions()
                .prepend(c)).checkCondition(c, false, services);
    }
