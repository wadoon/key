...
public class TestMatchTaclet extends TestCase {
	...
	MatchConditions mc=(taclet.getMatcher().matchFind
			    (match,
			     MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services)); 
	assertNotNull(mc);
    }
	...
	assertTrue("An area conflict should happen because there is a free"+
		   ...
		   .findIfFormulaInstantiations ( seq, currGoal, services ).size() == 0);
	...
	(find_addrule_conflict,
	        find_addrule_conflict.getMatcher().matchFind(match.sub(0), 
	                MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services).getInstantiations(),
	...
	(if_find_clash,
            if_find_clash.getMatcher().matchFind(match.sub(0),  
               ...
               currGoal, services).getInstantiations(),
	...
	assertTrue("Match not found", 
		   if_find_clash.getMatcher().matchFind(match,  
				       MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services) != null);	           
    }
	...
	assertTrue("Match should not be found because of conflict with "+
		   ...
		   (not_free_conflict,
		    ...
		    (free_in, 
		     MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services), services) == null);
	...
	TacletApp tacletApp = index.getSuccedentTaclet(pio,
	                                               ...
	                                               currGoal, services).iterator().next();
	assertTrue("Match should be possible(modulo renaming)",
		   tacletApp.findIfFormulaInstantiations ( seq, currGoal, services ).size()>0);
    }
	...
	assertTrue("Instantiations should be found as updates can be ignored if "+
		   ...
		   all_left.getMatcher().matchFind(match, MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services)!=null);
		
	Term match2 = TacletForTests.parseTerm("\\<{int i;}\\>{i:=Z(2(#))} true");
	match2 = match2.sub(0);
	assertTrue("Instantiations should be found.",
		   assign_n.getMatcher().matchFind(match2, MatchConditions.EMPTY_MATCHCONDITIONS, currGoal, services)!=null);
    }
