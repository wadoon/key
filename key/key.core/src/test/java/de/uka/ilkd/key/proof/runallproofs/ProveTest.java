...
import static de.uka.ilkd.key.proof.runallproofs.proofcollection.ProofTest.TestProperty.*;
...
public class ProveTest {
    ...
    protected void assertProvability(String file) throws Exception {
        runKey(file, PROVABLE);
    }

    protected void assertUnProvability(String file) throws Exception {
        runKey(file, NOT_PROVABLE);
    }

    protected void assertLoadability(String file) throws Exception {
        runKey(file, LOADABLE);
    }

    private void runKey(String file, ProofTest.TestProperty testProperty) throws Exception {
        ...
        try {
            ...
            if (testProperty == LOADABLE) {
                success = true;
                debugOut("... success: loaded");
            } else {
                autoMode(env, loadedProof, script);
                success = (testProperty == PROVABLE) == loadedProof.closed();
     ...
     */
    private Pair<KeYEnvironment<DefaultUserInterfaceControl>, Pair<String, Location>> load(File keyFile) throws ProblemLoaderException {
