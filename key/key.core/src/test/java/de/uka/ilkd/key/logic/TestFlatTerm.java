...
public class TestFlatTerm {
    ...
    public  void myTest(){
        ...
        Term t_p_x = tf.createTerm(p,t_x);
        Term t_p_y = tf.createTerm(p,t_y);

        Term t_x_imp_y = tf.createTerm(Junctor.IMP,t_p_x,t_p_y);
        Term t_y_imp_x = tf.createTerm(Junctor.IMP,t_p_y,t_p_x);

        Term t_allx_x_imp_y = tb.all(x, t_x_imp_y);
        Term t_ally_y_imp_x = tb.all(y, t_y_imp_x);

        Term t_allx_x_imp_y_and_t_ally_y_imp_x = tf.createTerm(Junctor.AND,t_allx_x_imp_y,t_ally_y_imp_x);

        assertEquals(t_allx_x_imp_y_and_t_ally_y_imp_x.arity(),2);

        System.out.println(t_allx_x_imp_y_and_t_ally_y_imp_x.freeVars());
        /*
        System.out.println(t_allx_x_imp_y_and_t_ally_y_imp_x);
        System.out.println(t_allx_x_imp_y_and_t_ally_y_imp_x.boundVars());
        System.out.println(t_allx_x_imp_y_and_t_ally_y_imp_x.freeVars());*/
    }
