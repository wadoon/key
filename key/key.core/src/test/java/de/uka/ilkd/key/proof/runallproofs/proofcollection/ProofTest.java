...
/**
 * Data structure for integration test based on {@code *.key}-files.
 * These files are tested by {@link RunAllProofsTest}. It consists of a {@link #testProperty}
 ...
 * @author weigl
 */
public class ProofTest implements Serializable {
    ...
    static File getAbsoluteFile(File baseDirectory, String pathName) {
        ...
        if (!baseDirectory.isAbsolute()) {
            throw new RuntimeException(format("Expecting an absolute path but found: %s", baseDirectory));
        }

        if (!baseDirectory.isDirectory()) {
            throw new RuntimeException(
                    format("Given file system location is not a directory: %s", baseDirectory));
        }
    ...
    public ProofTest(TestProperty testProperty, String path, ProofCollectionSettings settings) {
        this.path = path;
     ...
     */
    protected void reload(boolean verbose, File proofFile, Proof loadedProof, boolean success) throws Exception {
    ...
    public enum TestProperty {
        PROVABLE, NOT_PROVABLE, LOADABLE
    }
}
