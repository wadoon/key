...
public class TestDeclParser extends TestCase {
    ...
    private KeYParseTreeVisitor parser() {
        return new KeYParseTreeVisitor(nss);
    }
    ...
    protected CCGenericSort checkGenericSort(Named p_n,
            ImmutableSet<Sort> p_ext,
            ImmutableSet<Sort> p_oneof) {
        assertTrue("Generic sort does not exist", p_n != null);
        assertTrue("Generic sort does not have type CCGenericSort, but " + p_n.getClass(),
                p_n instanceof CCGenericSort);

        CCGenericSort gs = (CCGenericSort) p_n;
...
//    }
    ...
    public void testAmbigiousDecls() {
        try {
            ...
            if (!(e.getCause() instanceof AmbiguousDeclException)) {
                e.printStackTrace();
                fail("Unexpected excpetion. Testcase failed." + e);
            }
        }
        catch (Exception re) {
            if (!(re instanceof AmbiguousDeclException)) {
                re.printStackTrace();
