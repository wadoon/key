...
/**
 * This class inherits from LDT and implements all method that are necessary to
 * handle integers, shorts and bytes. It caches the symbols declared in
 * integerHeader.key and offers methods to convert java number types to their
 * logic counterpart.
 */
...
public final class IntegerLDT extends LDT {
    ...
    public IntegerLDT(BCTermServices services) {
        super(NAME, services);
    ...
    /**
     * returns the function symbol for the given operation null if no function
     * is found for the given operator
     * 
    ...
    public Function getFunctionFor(
        ...
        if (op instanceof GreaterThan) {
            ...
            return isLong ? getJavaDivLong() : (isBigint ? getJDivision()
                    : getJavaDivInt());
        }
        else if (op instanceof Times) {
            return isLong ? getJavaMulLong() : (isBigint ? getMul()
                    : getJavaMulInt());
        }
        else if (op instanceof Plus) {
            return isLong ? getJavaAddLong() : (isBigint ? getAdd()
                    : getJavaAddInt());
        }
        else if (op instanceof Minus) {
            return isLong ? getJavaSubLong() : (isBigint ? getSub()
                    : getJavaSubInt());
        }
            ...
            return isLong ? getJavaUnaryMinusLong()
                    : (isBigint ? getNegativeNumberSign()
    ...
    @Override
    public Term translateLiteral(Literal lit, BCTermServices services) {
        int length = 0;
        ...
        if (lit instanceof IntLiteral) {
            if (literalString.startsWith("0") && !literalString.equals("0")) { // hex
                                                                               // or
                                                                               // octal
                                                                               // literal
                try {
            ...
            assert Character.toLowerCase(literalString.charAt(literalString
                    .length() - 1)) == 'l';
            try {
                final long l =
                        Long.decode(literalString.substring(0,
    ...
    public Expression translateTerm(Term t, ExtList children,
        ...
        throw new RuntimeException(
                "IntegerLDT: Cannot convert term to program: "
                        + t);
    }
