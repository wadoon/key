...
//						guardType = getSourceInfo().getType(tmp);
        ...
        if (enhancedFor.getGuard() instanceof Conditional && !(guardType instanceof ArrayType)) {
            ...
            if (tmpTR.getTypeArguments() != null) {
                ...
                iteratorType = ((ParameterizedType) guardType).getProgramModelInfo().getParameterizedType((ClassType) iteratorType, tmpTR.getTypeArguments());
            }
            conditional = true;
...
//			}
        ...
        if (arrayReferenceName != null) {
            ...
            if (iteratorType != null) {
                ...
                arrayReferenceName = VariableKit.createValidVariableName(sc.getSourceInfo(), enhancedFor.getASTParent(), "a" + counterA);
            counterA++;
        }
        ...
        for (Item it : items) {
            ...
            if (it.iteratorType == null) {
                ...
                if (!(it.iteratorType instanceof ParameterizedType) && it.guardType instanceof ParameterizedType) {
                    ...
                    if (((ParameterizedType) it.guardType).getTypeParameters() == null)
                        tmp.setTypeArguments(tmp2.getTypeArguments().deepClone());
                    else tmp.setTypeArguments(new ASTArrayList<TypeArgumentDeclaration>());
