...
public final class ASCII_UCodeESC_CharStream {
    ...
    public final char readChar() throws IOException {
        ...
        if (++this.bufpos == this.available)
            AdjustBuffSize();
        char c = (char) (0xFF & ReadByte());
        if ((c = (char) (0xFF & ReadByte())) == '\\') {
            UpdateLineColumn(c);
            int backSlashCnt = 1;
            label48:
            while (true) {
                if (++this.bufpos == this.available)
                    AdjustBuffSize();
                try {
                    this.buffer[this.bufpos] = c = (char) (0xFF & ReadByte());
                    if ((c = (char) (0xFF & ReadByte())) != '\\') {
                        UpdateLineColumn(c);
            ...
            try {
                while ((c = (char) (0xFF & ReadByte())) == 'u')
                    this.column++;
                this.buffer[this.bufpos] = c = (char) (hexval(c) << 12 | hexval((char) (0xFF & ReadByte())) << 8 | hexval((char) (0xFF & ReadByte())) << 4 | hexval((char) (0xFF & ReadByte())));
                this.column += 4;
            } catch (IOException e) {
