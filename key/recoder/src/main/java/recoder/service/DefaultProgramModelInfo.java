...
public abstract class DefaultProgramModelInfo extends AbstractService implements ProgramModelInfo, TuningParameters {
            ...
            add_methods: for (int j = 0; j < ms; j++) {
                ...
                if (isVisibleFor(m, ct)) {
                    ...
                    if ((c == jlo & mname == GETCLASS_NAME) && java5Allowed()) {
                    	...
                    	result.add(new DummyGetClassMethod(ct, this));
                    	result_size++;
                    } else {
                    	for (int k = 0; k < result_size; k++) {
    ...
    private boolean signaturesEquals(List<? extends Type> rsig,
		...
		for (int i = 0, s = rsig.size(); i < s; i++) {
			...
			if (t1 instanceof TypeParameter && t2 instanceof TypeParameter) {
				if (!TypeParameter.EqualsImplementor.equals((TypeParameter)t1, (TypeParameter)t2)) 
					return false;
	...
	public List<ClassType> removeErasedTypesFromList(
		...
		OUTER: for (int i = 0; i < classes.size(); i++) {
			// TODO 0.90 this assumes that the types are ordered in
			// the sense that RAW and non-RAW are grouped together
			// TODO 0.90 this can be improved !?
			// TODO 0.90 do we really need to reject "ErasedType"s only ?
	...
	private Type makeParameterizedType(TypeArgument ta) {
    	...
    	if (ta.getTypeArguments() == null || ta.getTypeArguments().size() == 0)
    		return bt;
    	return ParameterizedType.getParameterizedType((ClassType)bt, ta.getTypeArguments(),
    ...
    // TODO 0.90 - check if array handling is still okej
    ...
    public boolean isSubtype(ClassType a, ClassType b) {
        ...
        if (a instanceof ParameterizedType) {
        	// TODO 0.90 check type arguments (JLS ?15.12.12.2, ?5.1.9)
        ...
        if (b instanceof ErasedType) {
        	// TODO 0.90 is this completely valid??
        	b = ((ErasedType)b).getGenericType();
        }
        if (a instanceof ErasedType) {
        	// TODO 0.90 same here !?
        	a = ((ErasedType)a).getGenericType();
        }
        if (a instanceof TypeParameter && b instanceof TypeParameter) {
        	// TODO 0.90 check bounds! (see JLS)
        	result = true;
        }
    ...
    private final boolean paramMatches(Type ta, Type tb, boolean allowAutoboxing) {
    	...
    	if (tb instanceof TypeArgument.CapturedTypeArgument)
    		// TODO 0.90 this isn't the full deal...
    	...
    	if (ta instanceof TypeParameter && tb instanceof TypeParameter) {
    		...
    		if (((TypeParameter)ta).inheritanceEqual((TypeParameter)tb))
    			return true;
    	}
    	...
    	if (tb instanceof TypeParameter && ta instanceof PrimitiveType) {
    ...
    public ClassType getBoxedType(PrimitiveType unboxedType) {
        NameInfo ni = getNameInfo();
        switch (unboxedType) {
        case booleanType: return ni.getJavaLangBoolean();
        case byteType: return ni.getJavaLangByte();
        case charType: return ni.getJavaLangCharacter();
        case shortType: return ni.getJavaLangShort();
        case intType: return ni.getJavaLangInteger();
        case longType: return ni.getJavaLangLong();
        case floatType:  return ni.getJavaLangFloat();
        case doubleType:  return ni.getJavaLangDouble();
        }
    ...
    public boolean isVisibleFor(Member m, ClassType t) {
        ...
        if (m.isPublic()) {
        	...
        	// TODO 0.90 what if containing type is not visible to t ???
            return true;
        }
    ...
    // TODO 0.90 remove or at least make non-public
    ...
    public ClassType getBaseType(TypeArgument ta) {
    	if (ta.getWildcardMode() == WildcardMode.Any) {
    		// TODO 0.90 this handling is wrong:
    		// class A<T extends Number> { T getT() { return null; } { A<?> a = null; a.getT().byteValue(); }}
    		// should compile...
    	...
    	// TODO the two following cases should be mergable !?
    	...
    	if (ta instanceof TypeArgumentInfo) {
    		...
    		if (tai.isTypeVariable()) {
    			...
    			// TODO 0.92 this can appear when:
    			...
    			return getNameInfo().getJavaLangObject(); // TODO !!!!
    		} else {
    			ClassType ct = getNameInfo().getClassType(ta.getTypeName()); 
    			return targs == null ? ct : ParameterizedType.getParameterizedType(ct, targs, getServiceConfiguration().getImplicitElementInfo());
    		}
    	}
    	if (ta instanceof TypeArgumentDeclaration) {
    		...
    		// TODO 0.90 check if there's a better/cleaner solution !?
    		...
    		while (context.getASTParent() instanceof TypeArgumentDeclaration) {
    			// TODO 0.90 check if this get called!!! (Nested TypeArgumentDeclarations)
    			context = context.getASTParent();
    			context = context.getASTParent();
    		}
    ...
    private void internalFilterApplicableMethods(List<Method> list, String name, List<Type> signature, ClassType context, 
        ...
        name = name.intern(); // TODO necessary ??
    ...
    protected ClassType makeParameterizedArrayType(ClassType t, List<? extends TypeArgument> typeArgs) {
    	...
    	result = ParameterizedType.getParameterizedType(result, typeArgs,
    							getServiceConfiguration().getImplicitElementInfo());
    	for(int i = 0; i < dim; i++) {
	...
	public ClassType getCommonSupertype(ClassType ... types) {
		...
		ClassType result;
		List<ClassType> tml = new ArrayList<ClassType>();
	...
	public List<PrimitiveType> getAllSubtypes(PrimitiveType pt) {
		switch (pt) {
		case booleanType: return Collections.<PrimitiveType>emptyList(); 
		case byteType: return Collections.<PrimitiveType>emptyList();
		case shortType: return makeList(PrimitiveType.byteType);
		case longType: return makeList(PrimitiveType.intType,
				PrimitiveType.shortType, PrimitiveType.charType, 
				PrimitiveType.byteType);
		case intType: return makeList(PrimitiveType.shortType, 
				PrimitiveType.charType, PrimitiveType.byteType);
		case floatType: return makeList(PrimitiveType.longType,
				PrimitiveType.intType, PrimitiveType.shortType, 
				PrimitiveType.charType,	PrimitiveType.byteType);
		case doubleType: return makeList(PrimitiveType.floatType,
				PrimitiveType.longType,	PrimitiveType.intType, 
				PrimitiveType.shortType, PrimitiveType.charType,	
				PrimitiveType.byteType);
		case charType: return Collections.<PrimitiveType>emptyList();
		default: throw new Error(); // all cases covered
		}
	}

	public List<PrimitiveType> getAllSupertypes(PrimitiveType pt) {
		switch (pt) {
		case booleanType: return makeList(PrimitiveType.booleanType); 
		case byteType: return makeList(
				PrimitiveType.byteType, PrimitiveType.shortType, 
				PrimitiveType.intType, PrimitiveType.longType, 
				PrimitiveType.floatType, PrimitiveType.doubleType);
		case shortType: return makeList(
				PrimitiveType.shortType, PrimitiveType.intType,
				PrimitiveType.longType, PrimitiveType.floatType,
				PrimitiveType.doubleType);
		case longType: return makeList(
				PrimitiveType.longType, PrimitiveType.floatType,
				PrimitiveType.doubleType);
		case intType: return makeList(
				PrimitiveType.intType, PrimitiveType.longType,
				PrimitiveType.floatType, PrimitiveType.doubleType);
		case floatType: return makeList(
				PrimitiveType.floatType, PrimitiveType.doubleType);
		case doubleType: return makeList(PrimitiveType.doubleType); 
		case charType: return makeList(
				PrimitiveType.charType, PrimitiveType.intType,
				PrimitiveType.longType, PrimitiveType.floatType, 
				PrimitiveType.doubleType);
		default: throw new Error(); // all cases covered
		}
	...
	public List<PrimitiveType> getSubtypes(PrimitiveType pt) {
		...
		case shortType: return makeList(PrimitiveType.byteType);
		case longType: return makeList(PrimitiveType.intType);
		case intType: return makeList(PrimitiveType.shortType, PrimitiveType.charType);
		case floatType: return makeList(PrimitiveType.longType);
		case doubleType: return makeList(PrimitiveType.floatType); 
		case charType: return Collections.<PrimitiveType>emptyList();
		default: throw new Error(); // all cases covered
	...
	public List<PrimitiveType> getSupertypes(PrimitiveType pt) {
		switch (pt) {
		case booleanType: return Collections.<PrimitiveType>emptyList();
		case byteType: return makeList(PrimitiveType.shortType);
		case shortType: return makeList(PrimitiveType.intType);
		case longType: return makeList(PrimitiveType.floatType);
		case intType: return makeList(PrimitiveType.longType);
		case floatType: return makeList(PrimitiveType.doubleType);
		case doubleType: return Collections.<PrimitiveType>emptyList();
		case charType: return makeList(PrimitiveType.intType);
		default: throw new Error(); // all cases covered
		}
	}
	...
	public Type eraseType(Type t) {
		// JLS 3rd edition, § 4.6 
		// TODO 0.90 check if properly used everywhere + testcases.
		if (t instanceof ParameterizedType)
			t = ((ParameterizedType)t).getGenericType();
		// TODO 0.90 "The erasure of a nested type T.C is |T|.C."
		...
		if (t instanceof TypeParameter) {
			int bc = ((TypeParameter)t).getBoundCount();
			if (bc == 0)
				return getNameInfo().getJavaLangObject();
			return getClassTypeFromTypeParameter((TypeParameter)t, 0);
		}
		return t;
	}
	...
	List<Type> replaceAllTypeParameters(
		...
		for (Type t : replaceIn) {
			res.add(replaceAllTypeParameters(t, typeParams, replaceWith, goByName));
		}
		return res;
	}
	...
	Type replaceAllTypeParameters(
		...
		for (TypeParameter tp : typeParams) {
			...
			else if (targs != null) { // TODO 0.90 really "else" ?
				// TODO 0.90 clean this up!
				ArrayList<TypeParameter> single_tp = new ArrayList<TypeParameter>();
				single_tp.add(tp);
				ArrayList<TypeArgument> single_cp = new ArrayList<TypeArgument>();
		...
		if (targs != null)
			repl = ParameterizedType.getParameterizedType((ClassType)repl,  targs,
					getServiceConfiguration().getImplicitElementInfo());
		while (dim-- > 0)
	...
	public static class ResolvedTypeArgument implements TypeArgument {
		...
		// TODO 0.90 can this entire type be removed ?
		...
		public String getTypeName() {
			// TODO 0.90 make this better, this is a quick fix!!
			String res = type.getFullName();
			int idx = res.indexOf('.');
			while (idx > -1) {
				if (!Character.isJavaIdentifierStart(res.charAt(idx+1)))
					return type.getName(); // lokal type !?
		...
		public boolean semanticalEquality(TypeArgument o) {
			// TODO 0.90 is this okej ?
			if (o == this)
				return true;
			if(!(o instanceof ResolvedTypeArgument))
				return false;
			ResolvedTypeArgument ro = (ResolvedTypeArgument)o;
			if (ro.wm != wm || !type.equals(ro.type)) 
				return false;
			for (int i = 0; i < typeArgs.size(); i++)
				if (!typeArgs.get(i).semanticalEquality(ro.typeArgs.get(i)))
					return false;
			return true;
		}
	...
	<X extends Type> X replaceAllTypeParametersWithArgs(
		...
		// TODO 0.90 this is a quick solution only, no capture-conversion performed
		...
		int i = -1;
		// TODO 0.90 this may not be entirely proper code :-/
		OUTER: for (TypeParameter tp : typeParams) {
			i++;
			if (tp.equals(replaced)) {
				// bugfix for bug 2046005 
				// TODO 0.90 this is an ugly solution!!
		...
		if (targs != null) {
			...
			replaced = ParameterizedType.getParameterizedType((ClassType)replaced,  targs,
					getServiceConfiguration().getImplicitElementInfo());
		}
		while (dim-- > 0)
			replaced = replaced.createArrayType();
	...
	private TypeArgument replaceAllTypeParametersWithArgs(
		...
		if (bt instanceof TypeParameter) {
			...
			// TODO 0.90 we let it pass for now, but investigate!
			return replaceIn;
		}
