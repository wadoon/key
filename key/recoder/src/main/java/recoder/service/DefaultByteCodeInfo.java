...
public class DefaultByteCodeInfo extends DefaultProgramModelInfo implements ByteCodeInfo {
    ...
    public Type getType(ByteCodeElement bce) {
    	...
    	if (bce instanceof MethodInfo) {
    		...
    		if (mi.getTypeParameters() != null) {
    			for (TypeParameter tp : mi.getTypeParameters()) {
    				if (baseTypeName.equals(tp.getName())) {
    					result = tp;
    					while (dim-- > 0)
    	...
    	if (bce instanceof MemberInfo) {
    		...
    		if (typeArgs != null && typeArgs.size() != 0) {
    			if (result instanceof ArrayType) {
    				...
    				result = ParameterizedType.getParameterizedType((ClassType)result, typeArgs,
    ...
    public List<? extends ClassType> getTypes(ClassTypeContainer ctc) {
        ...
        if (ctc instanceof ByteCodeElement) {
            ...
            return (ctl == null) ? new ArrayList<ClassType>(0) : new ArrayList<ClassType>(ctl);
        } else {
                    	...
                    	Type t = null;
                   		...
                   		if (mi.getTypeArgumentsForParam(i) != null) {
                   			if (t instanceof ArrayType) {
                   				...
                   				t = ParameterizedType.getParameterizedType((ClassType)t, mi.getTypeArgumentsForParam(i),
                   						getServiceConfiguration().getImplicitElementInfo());
                   			}
                   		}
                    	res.add(t);
        ...
        if (sname != null) {
            ...
            if (ct == null) {
            	...
            	if (tais != null && tais.size() > 0)
            		ct = ParameterizedType.getParameterizedType(ct, tais, getServiceConfiguration().getImplicitElementInfo());
                list.add(ct);
            }
        }
