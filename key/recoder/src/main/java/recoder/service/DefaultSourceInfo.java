...
public class DefaultSourceInfo extends DefaultProgramModelInfo implements SourceInfo, ChangeHistoryListener, Formats {
     ...
     */
    private final Map<String, Type> name2primitiveType = new HashMap<String, Type>(INITIAL_SOURCE_INFO_NAME2PRIMITIVE_HASH_SET_SIZE);
    ...
    private ClassType getFromPackageImports(String name, List<Import> il, ClassType searchingFrom) {
        if (DEBUG)
    ...
    public Type getType(String name, ProgramElement context) {
        ...
        // check primitive types, (array types of primitive types (NOT IN 0.90 ANY MORE(YET?))),
        ...
        if (name.endsWith("]")) {
            ...
            // TODO 0.90 may be reworked for faster access (use baseType.createArrayType())
        ...
        while (s != null) {
            ...
            if (s instanceof TypeDeclaration) {
                ...
                if (newResult != null) {
                	// TODO 0.90 what is this - result is ALWAYS null!!!
                    if (result == null) {
                        if (DEBUG)
        ...
        if (result == null) {
        	...
        	// TODO 0.90 should be superfluous? NameInfo does this, now...
        ...
        if (expr instanceof Operator) { ///////////////// Operators
            ...
            if (expr instanceof UncollatedReferenceQualifier) {
            	...
            	if (java5Allowed()) {
        			...
        			tas.add(ta);
        			result = ParameterizedType.getParameterizedType(
        					getNameInfo().getJavaLangClass(), 
        					tas, getServiceConfiguration().getImplicitElementInfo());
                ...
                if (m != null) {
                    ...
                    if (result instanceof ClassType && ((ClassType)result).isInner()) {
                    	...
                    	if (mr.getReferencePrefix() != null) {
                    		...
                    		// TODO 0.90 the same with field references!!! 
                    		if (pref instanceof ParameterizedType) {
                    			result = ParameterizedType.getParameterizedType((ClassType)result, null, (ParameterizedType)pref, getServiceConfiguration().getImplicitElementInfo());
                    		}
                    	}
                    }
                }
            	...
            	// TODO 0.90 - is this still neccessary ?
    ...
    private ClassType inferType(Method m, MethodReference mr, TypeParameter typeParam) {
    		...
    		if (m.isVarArgMethod() && j == sig.size()-1) {
    			if (mr.getArguments().size() == sig.size()) {
    				// in this case, do NOT create an array type if it already is an array type...
    			 	Expression e = mr.getArguments().get(sig.size()-1);
    				...
    				for (int k = sig.size()-1; k < mr.getArguments().size(); k++) {
    					Expression e = mr.getArguments().get(k);
    	...
    	if (result.size() == 0) {
    		...
    		// take leftmost bound in that case
    		// TODO 0.90 - create intersection type??
    		if (typeParam.getBoundCount() == 0)
    			return getNameInfo().getJavaLangObject();
    		else {
    			// TODO 0.90 create method from this ??
    			if (typeParam instanceof TypeParameterInfo)
    				return getNameInfo().getClassType(typeParam.getBoundName(0));
    			else
    				return (ClassType)getType(((TypeParameterDeclaration)typeParam).getTypeReferenceAt(0));
    		}
    	}
    	else if (result.size() == 1) {
    		return result.get(0);
    	} else {
    		return getCommonSupertype(result.toArray(new ClassType[result.size()]));
    	}
    }
    ...
    List<ClassType> getTypeListFromTypeArgs(List<TypeArgumentDeclaration> targs) {
    	...
    	for (TypeArgumentDeclaration tad : targs) {
    		if (tad.getWildcardMode() != WildcardMode.None)
    			throw new RuntimeException(); // TODO 0.90 better exception...
            ...
            if (td instanceof EnumDeclaration) {
            	...
            	if (tp.getBounds() == null || tp.getBounds().size() == 0) {
            		...
            		for (TypeReference tr : tp.getBounds()) {
            			// TODO 0.90 why does the outcommented line not work??
...
//                    } else {
                    // TODO 0.90 check if replacement code is always okej...
                    // fixes bug 204167
...
//                    }
    ...
    // TODO check if this always works correctly (it's, at least, better than not doing this, see bug 2071287)...
    // labeled / switch statement??
    // NOTE: only supposed to be used for fixing bug 2071287
