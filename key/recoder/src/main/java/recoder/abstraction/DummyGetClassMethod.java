...
public final class DummyGetClassMethod implements Method {
	...
	private final ProgramModelInfo service;
	public DummyGetClassMethod(ClassType parent, ProgramModelInfo service) {
		this.parent = parent;
	...
	public ClassType getReturnType() {
		// build this together
		if (returnType == null) {
			ClassType ct = service.getServiceConfiguration().getNameInfo().getJavaLangClass();
			TypeArgument ta = new DefaultProgramModelInfo.ResolvedTypeArgument(
					WildcardMode.Extends, parent, null);
			ArrayList<TypeArgument> tas = new ArrayList<TypeArgument>(1);
			tas.add(ta);
			returnType = ParameterizedType.getParameterizedType(ct, tas, service.getServiceConfiguration().getImplicitElementInfo());
		}
		return returnType;
	}
