...
public interface TypeParameter extends ClassType {
	...
	public static class EqualsImplementor {
		public static boolean equals(TypeParameter t1, TypeParameter t2) {
			if (t1 == t2)
				return true;
...
//						find ridx
						int ridx = -1;
						List<? extends TypeArgument> rl = ((ParameterizedType)superCT).getAllTypeArgs();
						...
						if (ridx == -1)
							continue;
						tryUpwards = ((ParameterizedType)superCT).getGenericType().getTypeParameters().get(ridx); 
					}
