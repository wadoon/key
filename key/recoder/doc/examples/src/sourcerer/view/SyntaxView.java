	   ...
	   packages -> packages | units
		...
		public void update() {
			...
			if (!treeModelListeners.isEmpty()) {
				Enumeration<TreeModelListener> enum2 = treeModelListeners.elements();
				TreeModelEvent event = new TreeModelEvent(this, new TreePath(root));
				while (enum2.hasMoreElements()) {
					enum2.nextElement().treeStructureChanged(event);
				}
			}
		}
		...
		private void computeCache() {
			...
			for (int i = units.size() - 1; i >= 0; i -= 1) {
				...
				List<ModelElement> list = cache.get(p);
				if (list == null) {
		...
		public int getChildCount(Object parent) {
			...
			List<ModelElement> children = cache.get(parent);
		...
		public void valueForPathChanged(TreePath path, Object newValue) { /* nothing to do */ }
	}
