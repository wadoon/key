...
public class CommandParser {
    ...
    @Parameter(names = {"-c", "--coverage"},
               arity = 1,
               description = "target degree of code coverage for each method.\n" + CommandParser.INDENT + "Parameters (only one per session):\n" + CommandParser.INDENT + "statement\tstatement coverage (default)\n" + CommandParser.INDENT + "branch\t\tbranch coverage\n" + CommandParser.INDENT + "path\t\tpath coverage\n" + CommandParser.INDENT + "condition\tcondition coverage\n" + CommandParser.INDENT + "decision\tdecision coverage\n" + CommandParser.INDENT + "mcdc\t\tmodified condition/decision coverage")
     ...
     */
    @Parameter(names = {"-fr", "--framework"},
               variableArity = true,
               description = "the test frameworks to use.\n" + CommandParser.INDENT + "Parameters:\n" + CommandParser.INDENT + "[framework]\t specify framework to use\n" + CommandParser.INDENT + "legal frameworks: junit3, junit4")
