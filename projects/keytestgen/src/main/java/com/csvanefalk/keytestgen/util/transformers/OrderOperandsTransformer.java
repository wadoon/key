...
public class OrderOperandsTransformer extends AbstractTermTransformer {
    ...
    private void collectLiteralsFromTree(final Term term, final Operator operator, final List<Term> literals) {
    ...
    private Term constructTree(final PriorityQueue<String> sortedIdentifiers,
                               final Map<String, Term> mappings,
        ...
        if (sortedIdentifiers.size() == 0) {
            ...
            final Term rightChild = constructTree(sortedIdentifiers, mappings, operator);
    ...
    protected Term transformAnd(final Term term) throws TermTransformerException {
        ...
        if (comparison > 0) {
            return termFactory.createTerm(Junctor.AND, transformedSecondChild, transformedFirstChild);
        } else {
            return termFactory.createTerm(Junctor.AND, transformedFirstChild, transformedSecondChild);
        }
    }
    ...
    protected Term transformEquals(final Term term) throws TermTransformerException {
        ...
        final int comparison = firstChildName.compareTo(secondChildName);
        if (comparison > 0) {
            return termFactory.createTerm(term.op(), transformedSecondChild, transformedFirstChild);
        } else {
            return termFactory.createTerm(term.op(), transformedFirstChild, transformedSecondChild);
        }
    }
    ...
    protected Term transformOr(final Term term) throws TermTransformerException {
        ...
        final int comparison = firstChildName.compareTo(secondChildName);
        if (comparison > 0) {
            return termFactory.createTerm(Junctor.OR, transformedSecondChild, transformedFirstChild);
        } else {
            return termFactory.createTerm(Junctor.OR, transformedFirstChild, transformedSecondChild);
        }
    }
    ...
    private Term transformTree(final Term term) {
        ...
        if (TermParserTools.isOr(term) || TermParserTools.isAnd(term)) {
             ...
             */
            final Term newTerm = constructTree(sortedIdentifiers, identifierMapping, operator);
            return newTerm;
        } else {
