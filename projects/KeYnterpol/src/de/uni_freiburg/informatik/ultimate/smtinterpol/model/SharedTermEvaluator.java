...
public class SharedTermEvaluator {
	...
	public Term evaluate(SharedTerm st, Theory t) {
		if (st.validShared()) {
			if (st instanceof SharedAffineTerm) {
				SharedAffineTerm sat = (SharedAffineTerm) st;
				if (sat.toAffineTerm().isConstant()) {
					return sat.toAffineTerm().getConstant().toTerm(
							st.getSort());
				}
			}
