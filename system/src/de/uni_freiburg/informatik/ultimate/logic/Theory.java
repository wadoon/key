...
public class Theory {
	...
	public Theory(Logics logic) {
		m_BooleanSort = declareInternalSort("Bool", 0).getSort(null, new Sort[0]);
	...
	FunctionSymbol declareInternalFunction(String name, Sort[] paramTypes, Sort resultType, 
				int flags) {
	...
	private void createNumericOperators(final Sort sort, boolean isRational) {
		...
		if (isRational) {
			...
			declareInternalFunction("abs", sort1, sort, 0);
	...
	private void setLogic(Logics logic) {
		...
		case QF_IDL:
		case QF_LIA:
			m_NumericSort = declareInternalSort("Int", 0).getSort(null, new Sort[0]);
		...
		case QF_RDL:
			m_RealSort = m_NumericSort = 
				declareInternalSort("Real", 0).getSort(null, new Sort[0]);
		...
		case QF_UFLIRA:
			m_IsUFLogic = true;
			m_RealSort = declareInternalSort("Real", 0).getSort(null, new Sort[0]);
			m_NumericSort = declareInternalSort("Int", 0).getSort(null, new Sort[0]);
			createIRAOperators();
			break;
		case QF_AUFBV:
	...
	SortSymbol declareInternalSort(String name, int paramCount) {
		return defineSort(name, paramCount, null, SortSymbol.INTERNAL);
	}
			...
			TermVariable[] definitionVars, Term definition, int flags)
	{
		if ((flags & FunctionSymbol.INTERNAL) == 0) {
			...
			if (!m_IsUFLogic && paramTypes.length > 0)
				throw new IllegalArgumentException("Not allowed in this logic!");
		}
	...
	public FunctionSymbol skolemize(TermVariable tv,int counter) {
		return new FunctionSymbol("@" + tv.getName() + "_skolem_" + counter,null,
