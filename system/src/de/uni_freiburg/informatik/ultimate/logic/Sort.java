...
public class Sort {
	...
	public boolean isArraySort() {
		if (!m_Symbol.isIntern())
			return false;
		return getRealSort().getName().equals("Array");
	}
	public boolean isNumericSort() {
		if (!m_Symbol.isIntern())
			return false;
		String name = getRealSort().getName();
		return name.equals("Int") || name.equals("Real");
	}
	public boolean isDefinitelyInfinite() {
		if (!m_Symbol.isIntern())
			return false;
		String name = getRealSort().getName();
		if (name.equals("Int") || name.equals("Real"))
			return true;
		if (name.equals("Array"))
			return getArguments()[0].isDefinitelyInfinite();
		// Infinite if at least one parameter is infinte.
		for (Sort arg : getArguments())
			if (arg.isDefinitelyInfinite())
				return true;
		return false;
	}
