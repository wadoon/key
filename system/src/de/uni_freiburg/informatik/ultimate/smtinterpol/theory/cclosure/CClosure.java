...
//	boolean ematchingActive = false;
	...
	public CClosure(DPLLEngine engine,ConvertFormula converter) {
		this.engine = engine;
...
//		assert(checkCongruence());
	...
	public void fillInModel(Model model, Theory t, SharedTermEvaluator ste) {
		...
		if (!allTerms.isEmpty()) {
			...
			FlatTerm flatt0 = t0.getFlatTerm();
			if (flatt0 instanceof FlatFormula) {
				FlatFormula ff = (FlatFormula) flatt0;
				if (ff.isTrue()) {
					trueNode = t0;
