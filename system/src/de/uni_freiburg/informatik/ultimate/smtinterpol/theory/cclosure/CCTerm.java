...
import de.uni_freiburg.informatik.ultimate.smtinterpol.convert.FlatFormula;
...
public abstract class CCTerm extends SimpleListable<CCTerm> {
	...
	protected CCTerm(boolean isFunc, int parentPos, SharedTerm term) {
		this.isFunc = isFunc;
		this.ccpars = null;
		if (isFunc) {
	...
	private void propagateSharedEquality(CClosure engine, SharedTerm sterm) {
		 ...
		 */
		EqualityFormula eqForm = 
			(EqualityFormula) this.sharedTerm.createEquality(sterm);
	...
	private Clause mergeInternal(CClosure engine, CCTerm lhs, CCEquality reason) {
		...
		if (diseq == null && src.sharedTerm != null) {
			if (dest.sharedTerm != null) {
				...
				if (form.isFalse())
					...
					((EqualityFormula) form).
						createCCEquality(src.sharedTerm, dest.sharedTerm);
		...
		if (sharedTermConflict || diseq != null) {
			Clause conflict = sharedTermConflict 
				? engine.computeCycle(src.sharedTerm.toCCTerm(), 
									  dest.sharedTerm.toCCTerm())
