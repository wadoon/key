...
public class BoundConstraint extends DPLLAtom {
	...
	public Term getSMTFormula(Theory smtTheory, boolean useAuxVars) {
		...
		String comp = m_bound.meps == 0 ? "<=" : "<";
		Term zero = m_var.misint ? smtTheory.numeral(BigInteger.ZERO)
				: smtTheory.decimal(BigDecimal.ZERO);
		return smtTheory.term(comp,at.toSMTLib(smtTheory, m_var.misint, useAuxVars), zero);
	}
	...
	public Term getNegatedSMTFormula(Theory smtTheory, boolean useAuxVars) {
		MutableAffinTerm at = new MutableAffinTerm();
		at.add(Rational.ONE, m_var);
		at.add(m_ibound.ma.negate());
		String comp = m_ibound.meps == 0 ? ">=" : ">";
		Term zero = m_var.misint ? smtTheory.numeral(BigInteger.ZERO)
				: smtTheory.decimal(BigDecimal.ZERO);
		return smtTheory.term(comp,at.toSMTLib(smtTheory, m_var.misint, useAuxVars), zero);
	}
