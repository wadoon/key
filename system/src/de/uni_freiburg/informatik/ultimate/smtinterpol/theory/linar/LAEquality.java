...
public class LAEquality extends DPLLAtom {
	...
	public Term getSMTFormula(Theory smtTheory, boolean useAuxVars) {
		return getSMTFormula(smtTheory, false, useAuxVars);
	}
	
	@Override
	public Term getNegatedSMTFormula(Theory smtTheory, boolean useAuxVars) {
		return getSMTFormula(smtTheory, true, useAuxVars);
	}
	
	Term getSMTFormula(Theory smtTheory, boolean isNegated, boolean useAuxVars) {
		...
		Sort[] binfunc = {s,s};
		FunctionSymbol comp =  
				smtTheory.getFunction(isNegated ? "distinct" : "=", binfunc);
		return smtTheory.term(comp, at.toSMTLib(smtTheory, m_Var.misint, useAuxVars),
