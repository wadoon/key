...
public class Model implements de.uni_freiburg.informatik.ultimate.logic.Model {
	...
	public Model(ConvertFormula cf, Theory t) {
		m_Theory = t;
		// Extract Boolean model
		for (BooleanVarAtom atom : cf.getBooleanVars()) {
			ApplicationTerm at = (ApplicationTerm) atom.getSMTFormula(t);
			Term value;
			if (atom.getDecideStatus() != null)
				value = atom.getDecideStatus() == atom ? t.TRUE : t.FALSE;
			else
				value = atom.getPreferredStatus() == atom ? t.TRUE : t.FALSE;
			m_FuncVals.put(at.getFunction(), new Value(value));
		}
		// Extract different theories
		CClosure cc = cf.getCClosure();
		LinArSolve la = null;
		QuantifierTheory qf = null;
		for (ITheory theory : cf.getEngine().getAttachedTheories()) {
	...
	private Term evalInternalFunction(FunctionSymbol fun, Term[] args) {
		...
		if (name.equals("/")) {
			...
			Rational val = (Rational)((ConstantTerm) args[0]).getValue();
			for (int i = 1; i < args.length; ++i)
				val = val.div((Rational)((ConstantTerm) args[i]).getValue());
			return val.toTerm(fun.getReturnSort());
		}
		if (name.equals("<=")) {
			for (int i = 1; i < args.length; ++i) {
