...
public class RuleView extends JSplitPane implements TreeSelectionListener, java.io.Serializable {
    ...
    private String toString(Taclet tac) {
        ILogicPrinter lp =
                mediator.getProfile().getUIConfiguration().createLogicPrinter(
                        mediator.getProfile().getUIConfiguration().createDefaultProgramPrinter(),
                        mediator.getProfile().getUIConfiguration().createDefaultNotationInfo(),
                        null, true);
        lp.printTaclet(tac);
        return lp.toString();
    }
