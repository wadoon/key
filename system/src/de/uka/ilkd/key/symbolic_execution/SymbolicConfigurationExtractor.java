...
public class SymbolicConfigurationExtractor {
   ...
   public SymbolicConfigurationExtractor(Node node) {
      assert node != null;
      this.node = node;
      this.tb = node.proof().getServices().getTermBuilder();
   }
   ...
   public void analyse() throws ProofInputException {
      synchronized (this) {
         if (!isAnalysed()) {
            ...
            symbolicObjectsResultingInCurrentState.add(tb.NULL(getServices())); // Add null because it can happen that a object is null and this option must be included in equivalence class computation
   ...
   protected Set<Term> computeInitialObjectsToIgnore() {
      ...
      if (excVar instanceof ProgramVariable) {
         result.add(tb.var((ProgramVariable)excVar));
      }
                  ...
                  for (Term subUpdate : updateTerm.subs()) {
                     if (subUpdate.op() instanceof ElementaryUpdate) {
                        ...
                        if (eu.lhs() instanceof ProgramVariable) {
                           result.add(tb.var((ProgramVariable)eu.lhs()));
                        }
                     }
                  }
               }
   ...
   protected Sequent createSequentForEquivalenceClassComputation(Term pathCondition) {
      ...
      for (SequentFormula sf : originalSequent.succedent()) {
         Term term = sf.formula();
         if (Junctor.IMP.equals(term.op())) {
            Term newImplication = tb.imp(term.sub(0), tb.ff());
            newSuccedent = newSuccedent.insertLast(new SequentFormula(newImplication)).semisequent();
            // Updates are not required, because tb.apply(updates, true) is just true
         }
         else {
      ...
      for (Term first : symbolicObjects) {
         for (Term second : symbolicObjects) {
            if (!first.equals(second)) {
               Term equalTerm = tb.equals(first, second);
   ...
   protected void collectLocationsFromTerm(Term updateTerm, 
      ...
      if (updateTerm.op() instanceof UpdateJunctor) {
         ...
         if (SymbolicExecutionUtil.isHeapUpdate(getServices(), updateTerm)) {
            ...
            if (!isImplicitProgramVariable(var) && !objectsToIgnore.contains(tb.var(var))) {
   ...
   protected Term createLocationPredicateAndTerm(Set<ExtractLocationParameter> valueSelectParameter) {
      ...
      Function newPredicate = new Function(new Name(tb.newName(getServices(), "ConfigurationPredicate")), Sort.FORMULA, sorts);
      // Create formula which contains the value interested in.
      Term newTerm = tb.func(newPredicate, arguments);
      return newTerm;
   }
   ...
   protected ISymbolicConfiguration lazyComputeConfiguration(Node node,
      ...
      if (!locations.isEmpty()) {
         ...
         Term configurationCondition = tb.and(configuration);
         if (pathCondition != null) {
            configurationCondition = tb.and(configurationCondition, pathCondition);
         }
         ...
         ImmutableList<Term> newUpdates = ImmutableSLList.<Term>nil().append(tb.parallel(additionalUpdates));
   ...
   protected class ExtractLocationParameter {
      ...
      public Term createPreUpdate() {
         Term originalTerm = parentTerm != null ? parentTerm : tb.var(programVariable);
         return tb.elementary(getServices(), preVariable, originalTerm);
      }
      ...
      public Term createPreValueTerm() {
         if (parentTerm != null) {
            if (isArrayIndex()) {
               Term idx = tb.zTerm(getServices(), "" + arrayIndex);
               return ((JavaDLTermBuilder)tb).dotArr(getServices(), parentTerm, idx);
            }
            else {
               if (getServices().getJavaInfo().getArrayLength() == programVariable) {
                  ...
                  return tb.dot(getServices(), programVariable.sort(), createPreParentTerm(), function);
               }
            }
         }
         else {
            if (programVariable.isStatic()) {
               Function function = getServices().getTypeConverter().getHeapLDT().getFieldSymbolForPV((LocationVariable)programVariable, getServices());
               return ((JavaDLTermBuilder)tb).staticDot(getServices(), programVariable.sort(), function);
            }
            else {
