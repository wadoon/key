...
public abstract class AbstractKeYProgramModelMapping {
	...
	/**
	 * returns a matching ProgramElement (KeY) to a given ProgramElement
	 * (Recoder)
	 * 
	 * @param pe
	...
	/**
	 * returns the Recoder-equivalent to a given ProgramElement (KeY). If
	 * there's no RecodeR equivalent to program element pe, an assertion failure
	 * "Program Element <pe> not known" is emitted.
	 * 
	 * @param pe
	...
	/**
	 * returns the Recoder-equivalent to a given ModelElement (KeY). If there's
	 * no Recoder-equivalent to the ModelElement pe a debug message
	 * "Model Element <pe> not known" is printed.
	 * 
	 * @param pe
	 *            a ModelElement
	 */
	...
	/**
	 * As long as we do not support lemmata we need the source code of some
	 * 'java.lang' classes. These are parsed in using method parseSpecial of
	 * {@link Recoder2KeY}. To avoid multiple readings this method indicates
	 * whether the special have been parsed in or not.
	 * 
	 ...
	 * As long as we do not support lemmata we need the source code of some
	 * 'java.lang' classes. These are parsed in using method parseSpecial of
	 * {@link Recoder2KeY}. To avoid multiple readings this method sets a flag
	 * whether the special have been parsed in or not
	 * 
	 * @param b
