...
public class BlockContractBuiltInRuleApp extends AbstractBuiltInRuleApp {
	...
	public BlockContractBuiltInRuleApp tryToInstantiate(final Goal goal)
    {
        ...
        final BlockContractRule.Instantiation instantiation = BlockContractRule.instantiate(posInOccurrence().subTerm(), goal, (Services) services);
        final ImmutableSet<BlockContract> contracts = BlockContractRule.getApplicableContracts(instantiation, goal, (Services) services);
        block = instantiation.block;
        contract = SimpleBlockContract.combine(contracts, (Services) services);
        heaps = HeapContext.getModHeaps((Services) services, instantiation.isTransactional());
        return this;
	}
