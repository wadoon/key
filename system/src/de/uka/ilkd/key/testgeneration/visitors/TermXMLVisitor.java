 ...
 * @author christopher
 */
public class TermXMLVisitor
        extends KeYTestGenTermVisitor {

    /**
     * The name of the root tag of the XML document. If null or empty, it is excluded altogether
     */
    private final String rootTag;

    /**
     * Since {@link Visitor} does not support exceptional behavior, whereas the
     * {@link XMLEventWriter} demands it, we use an intermediary buffer to store events, and write
     * them only after the visitation process is completed.
     */
    private LinkedList<XMLEvent> elements = new LinkedList<XMLEvent>();

    /**
     * Use a stack in order to properly determine the order in which start and end tags should be
     * inserted for XML elements in the Term.
     */
    ...
    public String getXML() throws XMLVisitorException {
        ...
        try {
            ...
            if (rootTag != null && !rootTag.isEmpty()) {
                eventWriter.add(eventFactory.createStartElement("", "", rootTag));
                eventWriter.add(newline);
            }
    ...
    /**
     * Whenever a subtree is entered, create a tag corresponding to the type of the root element,
     * and push the name of the element on the stack in order to later generate an end tag.
     */
    @Override
    ...
    /**
     * Whenever a subtree is left, generate a closing tag corresponding to the one that was created
     * when the tree was first entered.
     */
    @Override
    ...
    /**
     * Add a tag, together with formatting, to the outputstream, indented by a specific number of
     * extra tabs.
     * 
     * @param tag
     *            the tag to insert
