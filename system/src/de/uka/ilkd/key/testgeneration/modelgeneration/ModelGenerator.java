 ...
 * KeYTestGen2 whenever the user does not specify anything else.
 *
 * The pathcondition to be instantiated is translated into a format
 * acceptable to an external SMT solvers. After that, the KeY SMT
 ...
 * Generator.
 * 
 * The Model Generator is a work in progress, and only basic functionality
 * has been implemented so far.
 * 
 * Future work:The Model generator will be able to provide a rich set of feedback to
 * the caller, for example giving information if a model for a given 
 * pathcondition cannot be found. It should also able to return data relevant
 * to benchmarking and optimization, such as execution time, bottlenecks, and
 *the like.
 */
public class ModelGenerator implements IModelGenerator {
    ...
    /**
     * Creates a default implementation of the ModelGenerator, which 
     * uses the Z3 solvers with default settings. 
     * 
    ...
    /**
     * Creates a custom implementation of the ModelGenerator. The user 
     * specifies which SMT solvers(s) and what settings should be used
     * 
     * TODO Currently only the Z3 solver will return a model, implment 
     * this for the other supported solvers as well
     * 
     * @param settings The settings for the SMT solvers used
     * 
    ...
    /**
     * Generates a model satisfying the pathcondition for a given
     * {link {@link IExecutionNode}. The model is encoded in XML format, 
     * and serves as a basis for creating executable test suites.
     * 
     * @param targetNode the node whose path condition we wish to find a model 
     * for
     * @param services related to the underlying implementation. 
     * See {@link Services}
     * 
    ...
    private HashMap<String,IModelContainer> createModel(SMTSolverResult result) throws ParseException {
        ...
        HashMap<String, ValueContainer> model = Z3ModelParser.parseModel(buffer.toString());
        
        HashMap<String,IModelContainer> finalModel = new HashMap<String, IModelGenerator.IModelContainer>();
        ...
        for(ValueContainer container : model.values()) {
            ...
            modelContainer.setValue(container.getValue());
            
            finalModel.put(modelContainer.getName(), modelContainer);
        }
     ...
     * 
     * @param the target path condition
     * 
    ...
    private  synchronized SMTProblem createSMTProblem(final IExecutionNode targetNode) 
        ...
        try {
            /*
             * The path condition has to be negated, in order to undo the negations that
             * will be carried out by the SMT interface.
             */
            Term newCondition = TermFactory.DEFAULT.createTerm(Junctor.NOT, targetNode.getPathCondition());
    ...
    private synchronized SMTSolverResult solveSMTProblem(
        ...
        do {
            ...
            try {
                ...
                /*
                 * In the event that the system fails due launchers being reused, dispose
                 * of them and create new ones.
                 */
    ...
    /**
     * Assert that the solvers associated with the ModelGenerator
     * are accessible.
     * 
     * @param solvers
     * @throws ModelGeneratorException
