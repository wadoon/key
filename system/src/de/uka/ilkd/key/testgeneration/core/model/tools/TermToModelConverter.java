...
public class TermToModelConverter {
    ...
    private static class VariableAssignmentResolvingVisitor extends
         ...
         * that the value of b is copied into a. For Integer types, there is no
         * need to do this explicitly, since the SMT interface will be taking
         * care of this, and we would thus only be performing the same work
         * twice. For Boolean types, however, which are not resolved by the SMT
         * interface, we do this explicitly.
         * <p>
            ...
            try {
                ...
                if (isPrimitiveType(leftOperand)) {
                    ...
                    if (isBoolean(leftOperand)) {
                         ...
                         * (TRUE or FALSE), we need to create a new such value
