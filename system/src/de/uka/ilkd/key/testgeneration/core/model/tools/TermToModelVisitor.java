...
class TermToModelVisitor extends KeYTestGenTermVisitor {
    ...
    public TermToModelVisitor(Model model, Services services,
        ...
        ClassDeclaration baseType = new ClassDeclaration(new Modifier[0], name,
                null, name, null, new MemberDeclaration[0], false, false);

        default_self = new LocationVariable(new ProgramElementName("$SELF$"),
                new KeYJavaType(baseType));
         ...
         */
        KeYJavaMethod method = mediator.getMethod();
        for (IProgramVariable parameter : method.getParameters()) {

            String identifier = parameter.name().toString();

            ModelVariable modelParameter = new ModelVariable(parameter,
                    identifier);
            ...
            if (primitiveTypes.contains(modelParameter.getType())) {
                value = resolvePrimitiveType(parameter);
            }
     ...
     */
    private Object resolvePrimitiveType(IProgramVariable variable) {
