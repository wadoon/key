...
/**
 * This class is responsible for starting external processes: 1. It creates the
 * process 2. Creates a pipe, that is used for communication. 3. Starts the
 * process and waits until the pipe has been closed or the process has been
 * stopped. Remark: It blocks the invoking thread. The parameter T of the class
 * can be used to define user-specific parameters.
 */
public class ExternalProcessLauncher<T> {
    private Process process;
    /**
     * lock for the process-object in order to guarantee synchronous access: If
     * you want to access on <code>process</code> then acquire first the lock!
     */
    ...
    public void launch(final String[] command, String initialMessage,
        ...
        try {
            ...
            try {
                ...
                pipe.start(process.getInputStream(), process.getOutputStream(),
                        process.getErrorStream(), listener);
            } finally {
                lockProcess.unlock();
            }
