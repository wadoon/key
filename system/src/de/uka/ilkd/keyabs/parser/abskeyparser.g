...
header {
  ...
  package de.uka.ilkd.keyabs.parser;
 ...
 */  
class ABSKeYParser extends Parser;
options {
    importVocab=ABSKeYLexer;
    k = 1;
...

{
    ...
    private ParserConfig<ABSServices> parserConfig;
     ...
     */
    public ABSKeYParser(ParserMode mode, TokenStream lexer) {
	this((lexer instanceof ABSKeYLexer)? ((ABSKeYLexer)lexer).getSelector() : ((DeclPicker)lexer).getSelector(), 2);
        this.selector = (lexer instanceof ABSKeYLexer)? ((ABSKeYLexer)lexer).getSelector() : ((DeclPicker)lexer).getSelector();
    ...
    public ABSKeYParser(ParserMode mode, TokenStream lexer, ABSServices services) {
        this(mode, lexer);
        this.keh = services.getExceptionHandler();
    }

    /* Most general constructor, should only be used internally */
    private ABSKeYParser(TokenStream lexer,
		     String filename,
    ...
    public ABSKeYParser(ParserMode mode, 
        ...
        this(mode, 
             lexer, 
             filename,
             (SchemaJavaReader) null,//new SchemaRecoder2KeY(services, nss),
	     services, 
	     nss, 
	     new HashMap());
    }
    ...
    public ABSKeYParser(ParserMode mode, 
    		     TokenStream lexer, 
                     String filename, 
                     ParserConfig<ABSServices> schemaConfig,
                     ParserConfig<ABSServices> normalConfig, 
                     HashMap taclet2Builder,
    ...
    public ABSKeYParser(ParserMode mode, TokenStream lexer, String filename) { 
        this(lexer, filename, null, null, mode);
        if (lexer instanceof DeclPicker) {
            this.capturer = (DeclPicker) lexer;
        }
     ...
     */
    public static Taclet parseTaclet(String s, ABSServices services) {
   	try {
	    ABSKeYParser p =
                new ABSKeYParser(ParserMode.TACLET,
                              new ABSKeYLexer(new StringReader(s),null),
                              "No file. ABSKeYParser.parseTaclet(\n" + s + ")\n",
                              services,
    ...
    private Operator getAttribute(Sort prefixSort, String attributeName) 
        ...
        throw new RuntimeException("TODO: Implement getAttribute in abskeyparser.g");
    }
  ...
  private KeYJavaType getTypeByClassName(String s) 
    throws KeYSemanticException {
        ...
        try {
	    kjt=getProgramInfo().getKeYJavaTypeByClassName(s);
    ...
    private boolean isPackage(String name){
        try {   
//            return getProgramInfo().isPackage(name);
        } catch(RuntimeException e){        
            // may be thrown in cases of invalid java identifiers
            return false;
        } 
        throw new RuntimeException("TODO: implement isPackage in abskeyparser");
    }
	...
	Debug.fail("ABSKeYParser.progVars(): this statement should not be reachable.");
	return null;
    }
...
top {Term a;} : a=formula {	 
   Debug.fail("ABSKeYParser: top() should not be called. Ever.");	 
}	 
;
