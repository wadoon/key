...
public class SortBuilder {
	...
	if (dataType.getKey().equals(ABS_INT_TYPE_NAME)) {
	    dataTypeSort = services.getTypeConverter().getIntegerLDT().targetSort();
	} else if (dataType.getKey().equals(ABS_BOOLEAN_TYPE_NAME)) {
	    dataTypeSort = services.getTypeConverter().getBooleanLDT().targetSort();
	}
	return dataTypeSort;
    }
	...
	for (InterfaceTypeUse ifdecl : interf.getValue()
	    ...
	    Sort extSort = iServices.getNamespaces().sorts()
		    .lookup(itfName);
	...
	for (DataTypeDecl d : info.getABSParserInfo().getParametricDatatypes()
	   ...
	   /* for (KeYJavaType s : initConfig.getServices().getProgramInfo().getAllKeYJavaTypes()) {
		...
		type = new ABSDatatype(name);
		dtSort = new SortImpl(name);
		initConfig.sortNS().addSafely(dtSort);
		abs2sort = new KeYJavaType(type, dtSort);
		services.getProgramInfo().rec2key().put(type, abs2sort);
	    }*/
	}
