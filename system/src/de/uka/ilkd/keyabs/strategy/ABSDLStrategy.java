...
package de.uka.ilkd.keyabs.strategy;
 ...
 * sequent.
 */
public class ABSDLStrategy extends AbstractFeatureStrategy {
    ...
    public static class Factory extends StrategyFactory {
        ...
        public Strategy create(Proof p_proof,
            ...
            return new ABSDLStrategy(p_proof, strategyProperties);
        }

        @Override
        public Name name() {
            return new Name(ABSDLStrategy);
        }
    }
    ...
    private class ArithTermFeatures {
        ...
        public ArithTermFeatures(IntegerLDT numbers) {
            ...
            notContainsDivMod = rec(any(),
                    add(add(not(divF), not(modF)), add(not(jdivF), not(jmodF))));
        }
    ...
    private class FormulaTermFeatures {
        ...
        public FormulaTermFeatures() {
            ...
            clauseSet = rec(andF,
                    or(opSub(Junctor.AND, any(), not(andF)), clause));
    ...
    public static final String ABSDLStrategy = "JavaCardDLStrategy";
    ...
    protected ABSDLStrategy(Proof p_proof, StrategyProperties strategyProperties) {

        super(p_proof);

        this.oneStepSimplifierRuleInstance = MiscTools
                .findOneStepSimplifier(p_proof);
    ...
    public final RuleAppCost computeCost(RuleApp app, PosInOccurrence pio,
        ...
        // (goal.node().serialNr() == 433 || goal.node().serialNr() == 421)*/) {
        // RuleAppCost result = costComputationF.compute ( app, pio, goal );
        // System.out.println("Cost for node " + goal.node().serialNr() + ": " +
        // result);
        // return result;
        // }
    ...
    public final boolean isApprovedApp(RuleApp app, PosInOccurrence pio,
            Goal goal) {
        // if(app.rule()==UseDependencyContractRule.INSTANCE) {
        // boolean result = !( approvalF.compute ( app, pio, goal ) instanceof
        // TopRuleAppCost );
        // System.out.println("Approval for node " + goal.node().serialNr() +
        // ": " + result);
        // return result;
        // }
    ...
    private Feature isBelow(TermFeature t) {
        ...
        return not(sum(superTerm, SuperTermGenerator.upwards(any()),
    ...
    private RuleSetDispatchFeature setupApprovalDispatcher(Proof p_proof) {
        ...
        final IntegerLDT numbers = p_proof.getServices().getTypeConverter()
                .getIntegerLDT();
    ...
    private Feature setupApprovalF(Proof p_proof) {
        ...
        if (depProp.equals(StrategyProperties.DEP_ON)) {
            depSpecF = ConditionalFeature.createConditional(
                    depFilter,
                    ifZero(new DependencyContractFeature(), longConst(0),
                            inftyConst()));
        } else {
    ...
    private RuleSetDispatchFeature setupCostComputationF(Proof p_proof) {
        ...
        bindRuleSet(
                d,
                "comprehensions",
                add(NonDuplicateAppModPositionFeature.INSTANCE, longConst(1000)));
        ...
        bindRuleSet(
                d,
                "simplify_prog",
                ifZero(isBelow(add(ff.forF, not(ff.atom))), longConst(200),
                        longConst(-100)));
        ...
        if (methProp.equals(StrategyProperties.METHOD_CONTRACT)) {
            ...
            // bindRuleSet(d, "method_expand", inftyConst()); //This seems to be
            // more correct, but the then some proofs from the example directory
            // do not work.
        ...
        if (queryAxProp.equals(StrategyProperties.QUERYAXIOM_ON)) {
            bindRuleSet(d, "query_axiom", longConst(0)); // Originally the
                                                         // QueryAxiom rule was
                                                         // assigned the
                                                         // strategy "simplify".
                                                         // Hence, the cost
    ...
    private void setupDefOpsExpandMod(RuleSetDispatchFeature d) {
        ...
        final Feature exSubsumedModulus = add(
                applyTF("divDenom", tf.literal),
                not(sum(superTerm,
                        SuperTermGenerator.upwardsWithIndex(sub(
                                or(tf.addF, tf.mulF), any())),
                        not(subsumedModulus))));
    ...
    private void setupDivModDivision(RuleSetDispatchFeature d) {
        ...
        final Feature isReduciblePolyE = sum(
                numTerm,
                SubtermGenerator.rightTraverse(instOf("divNum"), tf.addF),
                ifZero(applyTF(numTerm, tf.addF),
                        longConst(0),
                        sum(divCoeff, MultiplesModEquationsGenerator.create(
                                numTerm, denomLC), checkCoeffE)));
    ...
    private void setupFormulaNormalisation(RuleSetDispatchFeature d,
        ...
        bindRuleSet(
                d,
                "conjNormalForm",
                add(not(applyTF(FocusFormulaProjection.INSTANCE, ff.propJunctor)),
                        NotInScopeOfModalityFeature.INSTANCE, longConst(-150)));
        ...
        final Feature pullOutQuantifierAllowed = add(
                not(NotBelowQuantifierFeature.INSTANCE),
                onlyBelowQuanAndOr,
                applyTF(FocusProjection.create(0),
    ...
    private Feature setupGlobalF(Feature dispatcher, Proof p_proof) {//
        ...
        return SumFeature.createSum(new Feature[] {
                ...
                // splitF,
                dispatcher, NonDuplicateAppFeature.INSTANCE,
                // strengthenConstraints,
                AgeFeature.INSTANCE, oneStepSimplificationF,
                // smtF,
                methodSpecF, queryF, depSpecF, loopInvF, ifMatchedF });
    }
    ...
    private void setupInEqCaseDistinctions(RuleSetDispatchFeature d,
        ...
        final Feature posNegSplitting = forEach(
                intRel,
                SequentFormulasGenerator.antecedent(),
        ...
        final Feature strengthening = forEach(
                intRel,
                SequentFormulasGenerator.antecedent(),
                SumFeature.createSum(new Feature[] {
                        applyTF(intRel,
        ...
        final Feature rootInferences = forEach(
                intRel,
                SequentFormulasGenerator.antecedent(),
    ...
    private void setupInEqSimp(RuleSetDispatchFeature d, Proof p_proof,
            IntegerLDT numbers) {
                        ...
                        ifZero(MatchedIfFeature.INSTANCE,
                                SumFeature.createSum(new Feature[] {
                                        ...
                                        applyTF("contradRightBigger",
                                                tf.polynomial),
                                        applyTFNonStrict("contradCoeffSmaller",
                                                tf.posLiteral),
                                        applyTFNonStrict("contradCoeffBigger",
                                                tf.posLiteral),
    ...
    private void setupInEqSimpInstantiationWithoutRetry(
            RuleSetDispatchFeature d, Proof p_proof) {
        // category "direct inequations"
    ...
    private void setupMultiplyInequations(RuleSetDispatchFeature d,
        ...
        final Feature totallyBounded = not(sum(
                intRel,
        ...
        final Feature exactlyBounded = not(sum(
                intRel,
                SequentFormulasGenerator.sequent(),
                not(add(applyTF(intRel, tf.intRelation), InEquationMultFeature
    ...
    private void setupNewSymApproval(RuleSetDispatchFeature d,
            IntegerLDT numbers) {
        final TermBuffer antecFor = new TermBuffer();
        final Feature columnOpEq = applyTF(
                antecFor,
                opSub(tf.eq, opSub(tf.mul, tf.atom, tf.atLeastTwoLiteral),
                        tf.polynomial));
        final Feature biggerLeftSide = MonomialsSmallerThanFeature.create(
                instOf("newSymLeft"),
                subAt(antecFor, PosInTerm.TOP_LEVEL.down(0).down(0)), numbers);
        bindRuleSet(
                d,
                "polySimp_newSym",
    ...
    private void setupPolySimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        final Feature validEqApplication = add(
                not(eq(eqLeft, focus)),
    ...
    private void setupSplitting(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "beta", SumFeature.createSum(new Feature[] {
                noCutsAllowed,
    ...
    private void setupSplittingApproval(RuleSetDispatchFeature d) {
        ...
        final TermBuffer subFor = new TermBuffer();
        final Feature compareCutAllowed = ifZero(
                applyTF(subFor, ff.cutAllowed),
    ...
    private void setUpStringNormalisation(RuleSetDispatchFeature d,
        ...
        final CharListLDT charListLDT = services.getTypeConverter()
                .getCharListLDT();
        ...
        final TermFeature stringLiteral = rec(
                any(),
                or(or(op(charListLDT.getClEmpty()), op(charListLDT.getClCons())),
                        tf.charLiteral));

        Feature belowModOpPenality = ifZero(isBelow(ff.modalOperator),
                longConst(500));

        Feature ignoreDefOpsCharAtLetSymbols = ifZero(
                DirectlyBelowSymbolFeature.create(tf.eq, 0),
                ifZero(IntroducedSymbolBy.create(
                        sub(FocusProjection.create(1), 1), "defOpsCharAt",
                        "newSym"), inftyConst()));
        ...
        Feature ignoreDefOpsSubLetSymbols = ifZero(
                DirectlyBelowSymbolFeature.create(tf.eq, 0),
                ifZero(IntroducedSymbolBy.create(
                        sub(FocusProjection.create(1), 1), "defOpsSubstring",
                        "newSym"), inftyConst()));
    ...
    private void setupUserTaclets(RuleSetDispatchFeature d) {
        for (int i = 1; i <= StrategyProperties.USER_TACLETS_NUM; ++i) {
            final String userTacletsProbs = strategyProperties
                    .getProperty(StrategyProperties.USER_TACLETS_OPTIONS_KEY(i));
