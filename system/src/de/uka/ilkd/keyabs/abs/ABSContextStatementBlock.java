 ...
 * open brackets, try's and so on and omega is the rest of the program. Between
 * the prefix pi and the postfix omega there can stand an arbitrary program.
 * This pattern is realized using this class.
 */

public class ABSContextStatementBlock extends ABSStatementBlock implements
        IContextStatementBlock {
    ...
    /**
     * overrides the check of the superclass as unmatched elements will
     * disappear in the suffix of this ContextStatementBlock
     */
    @Override
    ...
    public MatchConditions match(SourceData source, MatchConditions matchCond) {
        ...
        if (src instanceof ProgramPrefix) {
            ...
            if (patternPrefixLength > srcPrefixLength) {
                Debug.out(
                        "Program match FAILED. Source has not enough prefix elements.",
                        this, source);
                return null;
            }
            ...
            // firstActiveStatement contains the ProgramPrefix in front of the
            // first active statement
            // start denotes the child where to start to match
            // in some cases firstActiveStatement already denotes the element to
            // match
            // (empty block, empty try block etc.) this is encoded by setting
            // start to -1
            int start = -1;
        ...
        matchCond = makeContextInfoComplete(matchCond, newSource, prefix, pos,
                relPos, src, services);
    ...
    /**
     * completes match of context block by adding the prefix end position and
     * the suffix start position
     */
    private MatchConditions makeContextInfoComplete(MatchConditions matchCond,
            SourceData newSource, ProgramPrefix prefix, int pos,
            PosInProgram relPos, ProgramElement src, IServices services) {

        final SVInstantiations instantiations = matchCond.getInstantiations();
        final ExecutionContext lastExecutionContext = instantiations
                .getExecutionContext();
        ...
        matchCond = matchCond.setInstantiations(instantiations.replace(
                prefixEnd, suffixStart, lastExecutionContext, src, services));
        return matchCond;
    }

    /**
     * computes the PosInProgram of the first element, which is not part of the
     * prefix
     * 
     * @param prefix
     ...
     * @return the PosInProgram of the first element, which is not part of the
     *         prefix
     */
    private PosInProgram matchPrefixEnd(final ProgramPrefix prefix, int pos,
            PosInProgram relPos) {
