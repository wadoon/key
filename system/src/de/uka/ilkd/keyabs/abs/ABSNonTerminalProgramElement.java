...
/**
 * Top level implementation of a Java {@link NonTerminalProgramElement}. taken
 * from COMPOST and changed to achieve an immutable structure
 */
public abstract class ABSNonTerminalProgramElement extends ABSProgramElement
        implements NonTerminalProgramElement {

    private int hashCode;

    public ABSNonTerminalProgramElement() {
    }
     ...
     */
    public ABSNonTerminalProgramElement(ExtList list) {
        super(list);
    }

    public ABSNonTerminalProgramElement(PositionInfo pos) {
        super(pos);
    }

    public ABSNonTerminalProgramElement(ExtList children, PositionInfo pos) {
        super(children, pos);
    }
     ...
     */
    protected int getArrayPos(ImmutableArray<ProgramElement> arr,
            ProgramElement el) {
    ...
    public boolean equalsModRenaming(SourceElement se, NameAbstractionTable nat) {
        ...
        final ABSNonTerminalProgramElement jnte = (ABSNonTerminalProgramElement) se;
    ...
    public MatchConditions match(SourceData source, MatchConditions matchCond) {
        ...
        final SourceData newSource = new SourceData(ntSrc, 0,
                source.getServices());
    ...
    /**
     * used by @link matchChildren to decide if a found match is valid or if
     * there are remaining source elements that have not been matched (in which
     ...
     * <code>source.getSource()</code>. The call <tt>source.next</tt> has to be
     * done in the @link ProgramElement#match method of the currently matched
     * child in case of a successful match. This is <em>not</em> done here
     * (rationale: schemavariables matching on lists can be implemented easy).
     * 
     * 
     * @param source
