...
from common_python.vu_common import (VLog, vsetdiff, 
                       is_str, is_list, is_dict, is_tuple, 
                       merge_dict)
from common_python.sageutil import get_vars, is_sage_expr
...
class Inv(object):
    ...
    @staticmethod
    def print_invs(ps, nice_format=False):
        """
        sage: var('y')
        y
        ...
        if ps:
            ps = sorted(ps, key=lambda p: p.p_str) #sort by alphabetical string first
...
class InvExp(Inv):
    ...
    def get_score(self):
        ...
        sage: InvIeq((1/2)*x**2 + 134.134234*y + 1 >= 0).get_score()
        12
        sage: InvEqt(x**3 + 2.432*x + 8 == 0).get_score()
        6
        sage: InvIeq(x**2+x+7 >= 0).get_score()
        3
        
        If cannot compute score, returns the strlen of the inv
