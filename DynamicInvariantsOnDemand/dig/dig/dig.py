...
from common_python.vu_common import (VLog, pause, is_str, is_dict, is_list, is_empty, get_cur_time)
...
class DIG(object):
    ...
    def __init__(self, filename, verbose=VLog.DEBUG):
        ...
        for f in [ dig_miscs, dig_inv, dig_refine, 
                   dig_arrays, dig_polynomials]:
    ...
    def _get_eqts_ieqs_max_min(self, deg, subset_siz=None,is_fixed=True):
        def worker_mpp_gen(Q):
            rs = self.get_ieqs_max_min_gen(
                terms=None,subset_siz=subset_siz)
            Q.put(rs)
        def worker_mpp_fixed(Q):
            rs = self.get_ieqs_max_min_fixed(
                terms=None,subset_siz=subset_siz)
            Q.put(rs)
        def worker_eqts(Q):
    ...
    def get_eqts_ieqs_max_min_fixed(self, deg):
        return self._get_eqts_ieqs_max_min(deg=deg,subset_siz=None,
                                           is_fixed=True)        
