...
public class PostconditionComposite extends AbstractTruthValueComposite {
   ...
   protected Triple<JavaDLTerm, PosInTerm<JavaDLTerm>, JavaDLTerm> computeTermToShow(IKeYSENode<?> node,
      ...
      if (uninterpretedPredicate != null) {
         ...
         PosInTerm<JavaDLTerm> predicatePosition = findUninterpretedPredicateTerm(node, term, uninterpretedPredicate, additionalPredicates);
         JavaDLTerm termWithoutPredicate = predicatePosition != null ? 
                                     ...
                                     term;
         return new Triple<JavaDLTerm, PosInTerm<JavaDLTerm>, JavaDLTerm>(INCLUDE_UPDATES ? keyNode.proof().getServices().getTermBuilder().applySequential(updates, termWithoutPredicate) : termWithoutPredicate, 
                                                  predicatePosition,
                                                  term);
      }
      else {
         return new Triple<JavaDLTerm, PosInTerm<JavaDLTerm>, JavaDLTerm>(INCLUDE_UPDATES ? keyNode.proof().getServices().getTermBuilder().applySequential(updates, term) : term, 
                                                  null,
                                                  null);
      }
   }
