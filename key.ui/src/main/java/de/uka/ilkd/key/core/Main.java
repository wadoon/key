...
public final class Main {
    ...
    @Option(names = "--show-properties", description = "")
    private boolean showProperties = false;

    @Option(names = "--auto", description = "")
    private boolean isAuto = false;

    @Option(names = "--last")
    private boolean isLast = false;

    @Option("--auto-loadonly")
    ...
    @Option("--no-pruning-closed", description="This parameter disables the possibility to prune in closed branches. It is meant as a fallback solution if storing all closed goals needs too much memory.")
    private boolean isNoPruningClosed = false;

    @Option("--keep-fileRepos", description="""
     * If this option is set, the (Disk)FileRepo does not delete its temporary
     * directories (can be
     * used for debugging).       
            """)
    private boolean isKeepFileRepos= "--keep-fileRepos";
    ...
    @Option("--tacletDir")
    private boolean tacletDir = false;

    @Option("--justify-rules")
    private boolean justifyRules = false;
    ...
    @Option(names = { "--verbose", "-v" }, multiple = true)
     ...
     * {@link #examplesDir} is defined
     * (not {@code null}). It is set in the Eclipse integration to {@code false},
     * because it is
    ...
    private static AbstractMediatorUserInterfaceControl createUserInterface(
        ...
        if (uiMode == UiMode.AUTO) {
            ...
            /*
             * explicitly enable pruning in closed branches for interactive mode (if not
             * manually
             * disabled)
             */
    ...
    private static List<File> preProcessInput(List<File> filesOnStartup) {
        ...
        if (riflFileName != null) {
            ...
            // final KeYRecoderExceptionHandler kexh =
            // ui.getMediator().getExceptionHandler();
            try {
